...
Version 1.0.3, September 2021 by AnonymousPlanet.
...
This guide is a non-profit open-source initiative, licensed under Creative Commons **Attribution-NonCommercial** 4.0 International ([cc-by-nc-4.0][] <sup>[[Archive.org]][27]</sup>). See the [license][License:] at the end of the document.
...
DNS stands for "Domain Name System"[^30] and is a service used by your browser (and other apps) to find the IP addresses of a service. It is a huge "contact list" (phone book for older people) that works like asking it a name and it returns the number to call. Except it returns an IP instead.
...
Because it is not encrypted, your ISP and/or any other adversary could still intercept (using a Man-in-the-middle attack[^89]) your request will know and possibly log what your IP was looking for. The same ISP can also tamper with the DNS responses even if you are using a private DNS. Rendering the use of a private DNS service useless.

As a bonus, many devices and apps will use hardcoded DNS servers bypassing any system setting you could set. This is for example the case with most (70%) Smart TVs and a large part (46%) of Game Consoles[^33]. For these devices, you will have to force them[^34] to stop using their hardcoded DNS service which could make them stop working properly.

A solution to this is to use encrypted DNS using DoH (DNS over HTTPS[^35]), DoT (DNS over TLS[^36]) with a private DNS server (this can be self-hosted locally with a solution like pi-hole[^37], remotely hosted with a solution like nextdns.io or using the solutions provider by your VPN provider or the Tor network). This should prevent your ISP or some go-between from snooping on your requests ... except it might not.
...
Unfortunately, the TLS protocol used in most HTTPS connections in most Browsers (Chrome/Brave/Ungoogled-Chromium among them) will leak the Domain Name again through SNI[^38] handshakes (this can be checked here at Cloudflare: <https://www.cloudflare.com/ssl/encrypted-sni/> <sup>[[Archive.org]][52]</sup> ). **As of the writing of this guide, only Firefox-based browsers supports ECH (Encrypted Client Hello**[^39] **previously known as eSNI**[^40]**) on some websites which will encrypt everything end to end (in addition to using a secure private DNS over TLS/HTTPS) and will allow you to hide your DNS requests from a third party**[^41]**.** And this option is not enabled by default either so you will have to enable it yourself.

![][53]

In addition to limited browser support, only Web Services and CDNs[^42] behind Cloudflare CDN support ECH/eSNI at this stage[^43]. This means that ECH and eSNI are not supported (as of the writing of this guide) by most mainstream platforms such as:
...
Some countries like Russia[^44] and China[^45] might (unverified despite the articles) block ECH/eSNI handshakes at the network level to allow snooping and prevent bypassing censorship. Meaning you will not be able to establish an HTTPS connection with a service if you do not allow them to see what it was.

The issues do not end here. Part of the HTTPS TLS validation is called OCSP[^46] and this protocol used by Firefox-based browsers will leak metadata in the form of the serial number of the certificate of the website you are visiting. An adversary can then easily find which website you are visiting by matching the certificate number[^47]. This issue can be mitigated by using OCSP stapling[^48]. Unfortunately, this is enabled but not enforced by default in Firefox/Tor Browser. But the website you are visiting must also be supporting it and not all do. Chromium-based browsers on the other hand use a different system called CRLSets[^49]'[^50] which is arguably better.
...
RFID stands for Radio-frequency identification[^54], it is the technology used for instance for contactless payments and various identification systems. Of course, your smartphone is among those devices and has RFID contactless payment capabilities through NFC[^55]. As with everything else, such capabilities can be used for tracking by various actors.
...
But that is not what all those Wi-Fi access points can do. Recently developed techs could even allow someone to track your movements accurately just based on radio interferences. What this means is that it is possible to track your movement inside a room/building based on the radio signals passing through. This might seem like a tinfoil hat conspiracy theory claim but here are the references[^59] with demonstrations showing this tech in action: <http://rfpose.csail.mit.edu/> <sup>[[Archive.org]][64]</sup> and the video here: <https://www.youtube.com/watch?v=HgDdaMy8KNE> <sup>[[Invidious]][65]</sup>
...
These have been used since at least since 2008 using an attack called "Jasager"[^60] and can be done by anyone using self-built tools or using commercially available devices such as Wi-Fi Pineapple[^61].
...
These devices can fit in a small bag and can take over the Wi-Fi environment of any place within their range. For instance, a Bar/Restaurant/Café/Hotel Lobby. These devices can force Wi-Fi clients to disconnect from their current Wi-Fi (using de-authentication, disassociation attacks[^62]) while spoofing the normal Wi-Fi networks at the same location. They will continue to perform this attack until your computer, or you decide to try to connect to the rogue AP.

These devices can then mimic a captive portal[^63] with the exact same layout as the Wi-Fi you are trying to access (for instance an Airport Wi-Fi registration portal). Or they could just give you unrestricted access internet that they will themselves get from the same place.
...
Tor and VPNs are not silver bullets. Many advanced techniques have been developed and studied to de-anonymize encrypted Tor traffic over the years[^64]. Most of those techniques are Correlation attacks that will correlate your network traffic in one way or another to logs or datasets. Here are some classic examples:

-   **Correlation Fingerprinting Attack:** As illustrated (simplified) below, this attack will fingerprint[^65] your encrypted traffic (like the websites you visited) just based on the analysis of your encrypted traffic (without decrypting it). It can do so with a whopping 96% success rate. Such fingerprinting can be used by an adversary that has access to your source network to figure out some of your encrypted activity (such as which websites you visited).

![][69]

-   **Correlation Timing Attacks:** As illustrated (simplified) below, an adversary that has access to network connection logs (IP or DNS for instance, remember that most VPN servers and most Tor nodes are known and publicly listed) at the source and the destination could correlate the timings to de-anonymize you without requiring any access to the Tor or VPN network in between. A real use case of this technique was done by the FBI in 2013 to de-anonymize[^66] a bomb threat hoax at Harvard University.
...
Be aware again that this might not be enough against a motivated global adversary[^67] with wide access to global mass surveillance. Such an adversary might have access to logs no matter where you are and could use those to de-anonymize you.
...
Lastly, do remember that using Tor can already be considered suspicious activity[^68], and its use could be considered malicious by some[^69].
...
-   iPhones and iPads (IOS 13 and above)[^70]'[^71]

-   Samsung Phones (Android 10 and above)[^72]

-   MacBooks (macOS 10.15 and above)[^73]

Such devices will continue to broadcast identity information to nearby devices even when offline using Bluetooth Low-Energy[^74]. They do not have access to the devices directly (which are not connected to the internet) but instead use BLE to find them through other nearby devices[^75]. They are using peer-to-peer short-range Bluetooth communication to broadcast their status through nearby online devices.
...
-   The smartphone OS makers (Google/Apple for Android/IOs) also keep logs of IMEI/IMSI identifications tied to Google/Apple accounts and which user has been using them. They too can trace back the history of the phone and to which accounts it was tied in the past[^84].

-   Government agencies around the world interested in your phone number can and do use[^85] special devices called "IMSI catchers"[^86] like the Stingray[^87] or more recently the Nyxcell[^88]. These devices can impersonate (to spoof) a cell phone Antenna and force a specific IMSI (your phone) to connect to it to access the cell network. Once they do, they will be able to use various MITM[^89] (Man-In-The-Middle Attacks) that will allow them to:
...
Some commercial devices will keep a record of MAC addresses roaming around for various purposes such as road congestion[^92].
...
Operating systems have protections in place to randomize those addresses but are still subject to vulnerabilities[^93].
...
All modern CPUs[^94] are now integrating hidden management platforms such as the now infamous Intel Management Engine[^95] and the AMD Platform Security Processor[^96].
...
These have already been affected by several security vulnerabilities in the past[^97] that allowed malware to gain control of target systems. These are also accused by many privacy actors including the EFF and Libreboot of being a backdoor into any system[^98].

There are some not so straightforward ways[^99] to disable the Intel IME on some CPUs and you should do so if you can. For some AMD laptops, you can disable it within the BIOS settings by disabling PSP.
...
If you are feeling a bit more adventurous, you could install your own BIOS using Libreboot[^100] or Coreboot[^284] if your laptop supports it (be aware that Coreboot does contain some propriety code unlike its fork Libreboot).
...
-   IOS/Apple:
    ...
    -   Apple does claim[^101] that they anonymize this data using differential privacy[^102] but you will have to trust them on that.
...
Data is being transmitted even if you opt-out[^104], processed, and stored indefinitely (most likely unencrypted[^105]) by various third parties[^106].
...
-   Your Fitness Devices and Apps[^107]? (Strava[^108]'[^109], Fitbit[^110], Garmin, Polar[^111], ...)

-   Your Smart Speaker? (Amazon Alexa[^112], Google Echo, Apple Homepod ...)

-   Your Smart Transportation? (Car? Scooter?)

-   Your Smart Tags? (Apple AirTag, Galaxy SmartTag, Tile...)

-   Your Car? (Yes, most modern cars have advanced logging/tracking features these days[^113])
...
Your metadata is all the information about your activities without the actual content of those activities. For instance, it is like knowing you had a call from an oncologist before then calling your family and friends successively. You do not know what was said during the conversation, but you can guess what it was just from the metadata[^114].

This metadata will also often include your location that is being harvested by Smartphones, Operating Systems (Android[^115]/IOS), Browsers, Apps, Websites. Odds are several companies are knowing exactly where you are at any time[^116] because of your smartphone[^117].

This location data has been used in many judicial cases[^118] already as part of "geofence warrants" [^119] that allow law enforcement to ask companies (such as Google/Apple) a list of all devices present at a certain location at a certain time. In addition, this location data is even sold by private companies to the military who can then use it conveniently[^120]. These warrants are becoming widely used by law enforcement[^121]'[^122].
...
The question is: Is there someone somewhere that would have both pieces of information available[^123] for correlation in a convenient database?

Have you heard of Edward Snowden[^124]? Now is the time to google him and read his book[^125]. Also read about XKEYSCORE[^126]'[^127], MUSCULAR[^128], SORM[^129], Tempora[^130] , and PRISM[^131].

See "We kill people based on Metadata"[^132] or this famous tweet from the IDF <https://twitter.com/idf/status/1125066395010699264> <sup>[[Archive.org]][92]</sup> <sup>[[Nitter]][93]</sup>.

See [Appendix N: Warning about smartphones and smart devices]

### Your Digital Fingerprint, Footprint, and Online Behavior:

This is the part where you should watch the documentary "The Social Dilemma"[^133] on Netflix as they cover this topic much better than anyone else IMHO.

This includes is the way you write (stylometry) [^134]'[^135], the way you behave[^136]'[^137]. The way you click. The way you browse. The fonts you use on your browser[^138]. Fingerprinting is being used to guess who someone is by the way that user is behaving. You might be using specific pedantic words or making specific spelling mistakes that could give you away using a simple Google search for similar features because you typed comparably on some Reddit post 5 years ago using a not so anonymous Reddit account[^139].
...
-   For example, as a basis of authentication, a user's typing speed, keystroke depressions, patterns of error (say accidentally hitting an "l" instead of a "k" on three out of every seven transactions) and mouse movements establish that person's unique pattern of behavior[^140]. Some commercial services such as TypingDNA (<https://www.typingdna.com/> <sup>[[Archive.org]][94]</sup>) even offer such analysis as a replacement for two-factor authentications.

-   This technology is also widely used in CAPTCHAS[^350] services to verify that you are "human" and can be used to fingerprint a user.
...
A real use and well-documented case of this was the arrest of the hacker Jeremy Hammond[^141] who shared over time several details about his past and was later discovered.

There are also a few cases involving OSINT at Bellingcat[^142]. Have a look at their very informative (but slightly outdated) toolkit here: <https://docs.google.com/spreadsheets/d/18rtqh8EG2q1xBo2cLNyhIDuK9jrPGwYr9DI2UncoqJQ/edit#gid=930747607> <sup>[[Archive.org]][100]</sup>
...
-   <https://web.archive.org/web/https://jakecreps.com/tag/osint-tools/>
...
Even if you are not looking at the camera, they can still figure out who you are[^149], make out your emotions[^150], analyze your gait[^151]'[^152]'[^153], read your lips[^154], analyze the behavior of your eyes[^155], and probably guess your political affiliation[^156]'[^157].
...
-   Even if you never made a profile on those platforms, you still have one without even knowing it[^158]'[^159]'[^160]'[^161]'[^162].
...
Governments already know who you are because they have your ID/Passport/Driving License pictures and often added biometrics (Fingerprints) in their database. Those same governments are integrating those technologies (often provided by private companies such as the Israeli AnyVision[^163], Clearview AI[^164], or NEC[^165]) in their CCTV networks to look for "persons of interest"[^166]. And some heavily surveilled states like China have implemented widespread use of Facial Recognition for various purposes[^167]'[^168] including possibly identifying ethnic minorities[^169]. A simple face recognition error by some algorithm can ruin your life[^170]'[^171].
...
The same goes with fingerprint authentication being mainstreamed by many smartphone makers to authenticate yourself. A simple picture where your fingers appear can be used to de-anonymize you[^172]'[^173]'[^174].

The same goes with your voice which can be analyzed for various purposes as shown in the recent Spotify patent[^175].

Even your iris can be used for identification in some places[^176].
...
-   Wear a facemask as they have been proven to defeat some face recognition technologies[^185] but not all[^186].
...
-   Consider wearing special sunglasses (expensive, unfortunately) called "Reflectacles" <https://www.reflectacles.com/> <sup>[[Archive.org]][122]</sup>. There was a small study showing their efficiency against IBM and Amazon facial recognition[^187].

-   All that might still be useless because of gait recognition mentioned earlier[^151] but there might be hope here if you have a 3D Printer: <https://gitlab.com/FG-01/fg-01> <sup>[[Archive.org]][123]</sup>
...
Phishing[^188] is a social engineering[^189] type of attack where an adversary could try to extract information from you by pretending or impersonating something/someone else.

A typical case is an adversary using a man-in-the-middle[^89] attack or a fake e-mail/call to ask for your credential for a service. This could for example be through e-mail or through impersonating financial services.
...
These could be simple pixel-sized images[^190] hidden in your e-mails that would call a remote server to try and get your IP address.

These could be exploiting a vulnerability in an outdated format or an outdated reader[^191]. Such exploits could then be used to compromise your system.
...
So, you are using Tor Browser or Brave Browser over Tor. You could be using those over a VPN for added security. But you should keep in mind that there are exploits[^192] (hacks) that could be known by an adversary (but unknown to the App/Browser provider). Such exploits could be used to compromise your system and reveal details to de-anonymize you such as your IP address or other details.

A real use case of this technique was the Freedom Hosting[^193] case in 2013 where the FBI inserted malware[^194] using a Firefox browser exploit on a Tor website. This exploit allowed them to reveal details of some users. More recently, there was the notable SolarWinds[^195] hack that breached several US government institutions by inserting malware into an official software update server.

In some countries, Malware is just mandatory and/or distributed by the state itself. This is the case for instance in China with WeChat[^196] which can then be used in combination with other data for state surveillance[^197].
...
To reflect these recommendations, this guide will therefore later guide you in the use of Virtualization (See [Appendix W: Virtualization][Appendix V1: Hardening your Browsers:]) so that even if your Browser/Apps get compromised by a skilled adversary, that adversary will find himself stuck in a sandbox[^198] without being able to access identifying information or compromise your system.

### Malicious USB devices:

There are readily available commercial and cheap "badUSB" [^199]devices that can take deploy malware, log your typing, geolocate you, listen to you or gain control of your laptop just by plugging them in. Here are some examples that you can already buy yourself:
...
Such devices can be implanted anywhere (charging cable, mouse, keyboard, USB key ...) by an adversary and can be used to track you or compromise your computer or smartphone. The most notable example of such attacks is probably Stuxnet[^200] in 2005.

While you could inspect a USB key physically, scan it with various utilities, check the various components to see if they are genuine, you will most likely never be able to discover complex malware embedded in genuine parts of a genuine USB key by a skilled adversary without advanced forensics equipment[^201].
...
Malware and backdoors can be embedded directly into your hardware components. Sometimes those backdoors are implemented by the manufacturer itself such as the IME in the case of Intel CPUs. And in other cases, such backdoors can be implemented by a third party that places itself between orders of new hardware and customer delivery[^202].

Such malware and backdoors can also be deployed by an adversary using software exploits. Many of those are called rootkits[^203] within the tech world. Usually, these types of malware are harder to detect and mitigate as they are implemented at a lower level than the userspace[^204] and often in the firmware[^205] of hardware components itself.

What is firmware? Firmware is a low-level operating system for devices. Each component in your computer probably has firmware including for instance your disk drives. The BIOS[^206]/UEFI[^207] system of your machine for instance is a type of firmware.
...
Such watermarks can be inserted by various products[^212]'[^213]'[^214]'[^215] using Steganography[^216] and can resist compression[^217] and re-encoding[^218]'[^219].

These watermarks are not easily detectable and could allow identification of the source despite all efforts.

In addition to watermarks, the camera used for filming (and therefore the device used for filming) a video can also be identified using various techniques such as lens identification[^220] which could lead to de-anonymization.
...
**Do not ever print in Color, usually, watermarks are not present without color toners/cartridges**[^222]**.**
...
There are also tutorials for using such techniques using Photo Editing tools such as GIMP such as <https://medium.com/@somdevsangwan/unblurring-images-for-osint-and-more-part-1-5ee36db6a70b> <sup>[[Archive.org]][143]</sup> followed by <https://medium.com/@somdevsangwan/deblurring-images-for-osint-part-2-ba564af8eb5d> <sup>[[Archive.org]][144]</sup>

![][145]

Finally, you will find plenty of deblurring resources here: <https://github.com/subeeshvasu/Awesome-Deblurring> <sup>[[Archive.org]][146]</sup>

Some online services could even help you do this automatically to some extent like MyHeritage.com enhance tool:

<https://www.myheritage.com/photo-enhancer> <sup>[[Archive.org]][147]</sup>
...
Contrary to widespread belief, Crypto transactions (such as Bitcoin and Ethereum) are not anonymous[^223]. Most cryptocurrencies can be tracked accurately through various methods[^224].

Remember what they say on their page: <https://bitcoin.org/en/you-need-to-know> <sup>[[Archive.org]][149]</sup> and <https://bitcoin.org/en/protect-your-privacy> <sup>[[Archive.org]][150]</sup>: "Bitcoin is not anonymous"

The main issue is not setting up a random Crypto wallet to receive some currency behind a VPN/Tor address (at this point, the wallet is anonymous). The issue is mainly when you want to convert Fiat money (Euros, Dollars ...) to Crypto and then when you want to cash in your Crypto. You will have few realistic options but to transfer those to an exchange (such as Coinbase/Kraken/Bitstamp/Binance). Those exchanges have known wallet addresses and will keep detailed logs (due to KYC[^225] financial regulations) and can then trace back those crypto transactions to you using the financial system[^226].

There are some cryptocurrencies with privacy/anonymity in mind like Monero but even those have some and warnings to consider[^227]'[^228].

Even if you use Mixers or Tumblers[^229] (services that specialize in "anonymizing" cryptocurrencies by "mixing them"), keep in mind this is only obfuscation[^230] and not actual anonymity[^231]. Not only are they only obfuscation but they could also put you in trouble as you might end up exchanging your crypto against "dirty" crypto that was used in various questionable contexts[^232].
...
There are specialized commercial forensics solutions available (Magnet Axiom[^233], Cellebrite Cloud[^234]) that will help an adversary analyze your cloud data with ease.

Notable Examples:

-   Apple iCloud: <https://support.apple.com/en-us/HT202303> <sup>[[Archive.org]][151]</sup> : "Messages in iCloud also uses end-to-end encryption. If you have iCloud Backup turned on**, your backup includes a copy of the key protecting your Messages**. This ensures you can recover your Messages if you lose access to iCloud Keychain and your trusted devices. ".

-   Google Drive and WhatsApp: <https://faq.whatsapp.com/android/chats/about-google-drive-backups/> <sup>[[Archive.org]][152]</sup>: "**Media and messages you back up aren't protected by WhatsApp end-to-end encryption while in Google Drive**. ".

-   Dropbox: <https://www.dropbox.com/privacy#terms> <sup>[[Archive.org]][153]</sup> "To provide these and other features, **Dropbox accesses, stores, and scans Your Stuff**. You give us permission to do those things, and this permission extends to our affiliates and trusted third parties we work with".

-   Microsoft OneDrive: <https://privacy.microsoft.com/en-us/privacystatement> <sup>[[Archive.org]][154]</sup>: Productivity and communications products, "When you use OneDrive, we collect data about your usage of the service, as well as the content you store, to provide, improve, and protect the services. **Examples include indexing the contents of your OneDrive documents so that you can search for them later and using location information to enable you to search for photos based on where the photo was taken**".
...
Your Browser and Device Fingerprints[^361] are set of properties/capabilities of your System/Browser. These are used on most websites for invisible user tracking but also to adapt the website user experience depending on their browser. For instance, websites will be able to provide a "mobile experience" if you are using a mobile browser or propose a specific language/geographic version depending on your fingerprint. Most of those techniques work with recent Browsers like Chromium-based[^235] browsers (such as Chrome/Edge) or Firefox[^236] unless taking specific measures.

You can find a lot of detailed information and publications about this on these resources:

-   <https://amiunique.org/links> <sup>[[Archive.org]][155]</sup>

-   <https://brave.com/brave-fingerprinting-and-privacy-budgets/> <sup>[[Archive.org]][156]</sup>
...
Forensics techniques are now very advanced and can reveal a staggering amount of information from your devices even if they are encrypted[^237]. These techniques are widely used by law enforcement all over the world and should be considered.

Here are some recent resources you should read about your smartphone:

-   UpTurn, The Widespread Power of U.S. Law Enforcement to Search Mobile Phones <https://www.upturn.org/reports/2020/mass-extraction/> <sup>[[Archive.org]][158]</sup>

-   New-York Times, The Police Can Probably Break Into Your Phone <https://www.nytimes.com/2020/10/21/technology/iphone-encryption-police.html> <sup>[[Archive.org]][159]</sup>

-   Vice, Cops Around the Country Can Now Unlock iPhones, Records Show <https://www.vice.com/en/article/vbxxxd/unlock-iphone-ios11-graykey-grayshift-police> <sup>[[Archive.org]][160]</sup>

I also highly recommend that you read some documents from a forensics examiner perspective such as:

-   EnCase Forensic User Guide, <http://encase-docs.opentext.com/documentation/encase/forensic/8.07/Content/Resources/External%20Files/EnCase%20Forensic%20v8.07%20User%20Guide.pdf> <sup>[[Archive.org]][161]</sup>

-   FTK Forensic Toolkit, <https://accessdata.com/products-services/forensic-toolkit-ftk> <sup>[[Archive.org]][162]</sup>

-   SANS Digital Forensics and Incident Response Videos, <https://www.youtube.com/c/SANSDigitalForensics/videos>

And finally, here is this very instructive detailed paper on the current state of IOS/Android security from the John Hopkins University: https://securephones.io/main.html[^238].
...
And there are reasons[^239]'[^240]'[^241]'[^242] for that:
...
-   You should be wary of Apps/Services using a "modified" or proprietary cryptographic method[^243].

-   By default, you should not trust any "Roll your own crypto" until it was audited, peer-reviewed, vetted, and accepted by the cryptography community[^244]'[^245].

-   There is no such thing as "military-grade crypto"[^246]'[^247]'[^248].
...
-   File/Disk Encryption:
    ...
    -   Prefer:
        ...
        -   Hardware Accelerated[^250]: AES (Rijndael) 256 Bits with HMAC-SHA-2 or HMAC-SHA-3 (This is what Veracrypt, Bitlocker, Filevault 2, KeepassXC, and LUKS use by default). Prefer SHA-3.

        -   Non-Hardware Accelerated: Same as accelerated above or if available consider:

            -   ChaCha20[^251] or XChaCha20 (You can use ChaCha20 with Kryptor <https://www.kryptor.co.uk>, unfortunately, it is not available with Veracrypt).
...
* **Warning: RSA and ED25519 are unfortunately not seen as "Quantum Resistant"**[^254] **and while they have not been broken yet, they probably will be broken someday into the future. It is just a matter of when rather than if RSA will ever be broken. So, these are preferred in those contexts due to the lack of a better possibility.**
...
Any of those providers can be forced to silently (without your knowing (using for example a court order with a gag order[^255] or a national security letter[^256]) log your activity to de-anonymize you. There have been several recent examples of those:

-   2021, ProtonMail, ProtonMail logged IP address of French activist after order by Swiss authorities[^257].

-   2021, WindScribe, Servers were not encrypted as they should have been allowing MITM attacks by authorities[^258].

-   2021, DoubleVPN servers, logs, and account info seized by law enforcement[^259].

-   2021, The Germany-based mail provider Tutanota was forced to monitor specific accounts for 3 months[^260].

-   2020, The Germany-based mail provider Tutanota was forced to implement a backdoor to intercept and save copies of the unencrypted e-mails of one user[^261] (they did not decrypt the stored e-mail).

-   2017, PureVPN was forced to disclose information of one user to the FBI[^262].

-   2014, an EarthVPN user was arrested based on logs provider to the Dutch Police[^263].

-   2014, HideMyAss user was de-anonymized, and logs were provided to the FBI[^264].

-   2013, Secure E-Mail provider Lavabit shuts down after fighting a secret gag order[^265].

Some providers have implemented the use of a Warrant Canary[^266] that would allow their users to find out if they have been compromised by such orders, but this has not been tested yet as far as I know.

Finally, it is now well known that some companies might be sponsored front ends for some state adversaries (see the Crypto AG story[^267] and Omnisec story[^268]).
...
(Illustration: an excellent movie I highly recommend: Das Leben der Anderen[^269])

Many advanced techniques can be used by skilled adversaries[^270] to bypass your security measures provided they already know where your devices are. Many of those techniques are detailed here <https://cyber.bgu.ac.il/advanced-cyber/airgap> <sup>[[Archive.org]][168]</sup> (Air-Gap Research Page, Cyber-Security Research Center, Ben-Gurion University of the Negev, Israel) and include:
...
-   **Attacks that require no malware:**
    ...
    -   Observing a light bulb from a distance to listen to the sound in the room[^271] **without any malware**: Demonstration: <https://www.youtube.com/watch?v=t32QvpfOHqw> <sup>[[Invidious]][185]</sup>. It should be noted that this type of attack is not new at all and there have been articles about such techniques as far back as 2013[^272] and that you can even buy devices to perform this yourself such as here: <http://www.gcomtech.com/ccp0-prodshow/laser-surveillance-laser-listening.html> <sup>[[Archive.org]][186]</sup>

Here is also a good video from the same authors to explain those topics: Black Hat, The Air-Gap Jumpers <https://www.youtube.com/watch?v=YKRtFgunyj4> <sup>[[Invidious]][187]</sup>

**Realistically, this guide will be of little help against such adversaries as such malware could be implanted on the devices by a manufacturer, anyone in the middle**[^273]**, or by anyone with physical access to the air-gapped computer but there are still some ways to mitigate such techniques:**
...
-   Have a look at the Whonix Documentation concerning Data Collection techniques here: <https://www.whonix.org/wiki/Data_Collection_Techniques> <sup>[[Archive.org]][188]</sup>

-   You might also enjoy looking at this service <https://tosdr.org/> <sup>[[Archive.org]][189]</sup> (Terms of Services, Didn't Read) that will give you a good overview of the various ToS of many services.

-   Have a look at <https://www.eff.org/issues/privacy> <sup>[[Archive.org]][190]</sup> for some more resources.

-   Have a look at <https://en.wikipedia.org/wiki/List_of_government_mass_surveillance_projects> <sup>[[Wikiless]][191]</sup> <sup>[[Archive.org]][192]</sup> to have an overview of all known mass-surveillance projects, current, and past.

-   Have a look at <https://www.gwern.net/Death-Note-Anonymity> <sup>[[Archive.org]][193]</sup> (even if you don't know about Death Note).
...
Zero-Trust Security[^370] ("Never trust, always verify").
...
-   If you want system-wide plausible deniability[^292]'[^274] despite the risks[^275]'[^295], consider:
...
-   Check some data yourself here on the Tor Project OONI[^276] (Open Observatory of Network Interference) website: <https://explorer.ooni.org/> <sup>[[Archive.org]][217]</sup>
...
This is the hardest part of the whole guide. It is a SPOF (Single Point of Failure). The places where you can still buy prepaid SIM cards without ID registration are getting increasingly limited due to various KYC type regulations[^277].
...
-   **Recommended**: Do not require any identification (even e-mail):
    ...
    -   (Iceland based) <https://crypton.sh> <sup>[[Tor Mirror]][220]</sup> <sup>[[Archive.org]][221]</sup>
...
-   Not accepting Monero but accepting Crypto:
    ...
    -   (**Unknown origin, untested**) <https://smspva.com/> <sup>[[Archive.org]][225]</sup>

There are some other possibilities listed here <https://cryptwerk.com/companies/sms/xmr/> <sup>[[Archive.org]][226]</sup>. **Use at your own risk.**
...
Please do not buy or use gimmicky self-encrypting devices such as these: <https://syscall.eu/blog/2018/03/12/aigo_part1/> <sup>[[Archive.org]][227]</sup>

Some might be very efficient[^278] but many are gimmicky gadgets that offer no real protection[^279].
...
So, what is Tor Browser? Tor Browser (<https://www.torproject.org/> <sup>[[Archive.org]][228]</sup>) is a web browser like Safari/Firefox/Chrome/Edge/Brave designed with privacy and anonymity in mind.

This browser is different from other browsers as it will connect to the internet through the Tor Network using Onion Routing. I first recommend that you watch this very nice introduction video by the Tor Project themselves: <https://www.youtube.com/watch?v=JWII85UlzKw> <sup>[[Invidious]][229]</sup>. After that, you should probably head over to their page to read their quick overview here: <https://2019.www.torproject.org/about/overview.html.en> <sup>[[Archive.org]][230]</sup>. Without going into too many technical details, Tor Browser is an easy and simple "fire and forget" solution to browse the web anonymously from pretty much any device. It is probably sufficient for most people and can be used from any computer or smartphone.
...
-   Head over to:
    ...
    -   F-Droid Store: It's not yet there but you can add it manually following the instructions at <https://support.torproject.org/tormobile/tormobile-7/> <sup>[[Archive.org]][231]</sup>
...
You will find details about each level here: <https://tb-manual.torproject.org/security-settings/> <sup>[[Archive.org]][232]</sup> but here is a summary:
...
While the official Tor Browser is not yet available for iOS, there is an alternative called Onion Browser endorsed by the Tor Project[^280].
...
Tails[^281] stands for **The Amnesic Incognito Live System**. It is a bootable Live Operating System running from a USB key that is designed for leaving no traces and forcing all connections through the Tor network.
...
**WARNING: Tails is not always up to date with their bundled software. And not always up to date with the Tor Browser updates either. You should always make sure you are using the latest version of Tails and you should use extreme caution when using bundled apps within Tails that might be vulnerable to exploits and reveal your location**[^282]**.**
...
You should also read Tails Documentation, Warnings, and limitations, before going further <https://tails.boum.org/doc/about/warnings/index.en.html> <sup>[[Archive.org]][233]</sup>
...
<https://tails.boum.org/install/index.en.html> <sup>[[Archive.org]][234]</sup>, pick your flavor and proceed.

If you're having an issue accessing Tor due to censorship or other issues, you can try using Tor Bridges by following this Tails tutorial: <https://tails.boum.org/doc/anonymous_internet/tor/index.en.html> <sup>[[Archive.org]][235]</sup> and find more information about these on Tor Documentation <https://2019.www.torproject.org/docs/bridges> <sup>[[Archive.org]][236]</sup>
...
When using Tor Browser, you should click the little shield Icon (upper right, next to the Address bar) and select your Security level (see <https://tb-manual.torproject.org/security-settings/> <sup>[[Archive.org]][232]</sup> for details). Basically, there are three.
...
-   **Do not use hidden volumes on SSD drives as this is not supported/recommended by Veracrypt**[^283]**.**
...
-   Download the latest HiddenVM release from <https://github.com/aforensics/HiddenVM/releases> <sup>[[Archive.org]][243]</sup>

-   Download the latest Whonix XFCE release from <https://www.whonix.org/wiki/VirtualBox/XFCE> <sup>[[Archive.org]][244]</sup>
...
If you can afford it, you might consider getting a Purism Librem laptop (<https://puri.sm> <sup>[[Archive.org]][245]</sup>) or System76 laptops (<https://system76.com/> <sup>[[Archive.org]][246]</sup>) while using Coreboot[^284] (where Intel IME is disabled from the factory).
...
-   <https://freundschafter.com/research/system-alternatives-without-intel-me-iamt-and-amd-psp-secure-technology/> <sup>[[Archive.org]][247]</sup>

-   <https://libreboot.org/docs/hardware/> <sup>[[Archive.org]][248]</sup>

-   <https://coreboot.org/status/board-status.html> <sup>[[Archive.org]][249]</sup>
...
-   AMD laptops could be more interesting as some provide the ability to disable AMD PSP (the AMD equivalent of Intel IME) from the BIOS/UEFI settings by default. And, because AFAIK, AMD PSP was audited and contrary to IME was not found to have any "evil" functionalities[^285]. However, if you are going for the Qubes OS Route consider Intel as they do not support AMD with their anti-evil-maid system[^286].
...
These settings can be accessed through the boot menu of your laptop. Here is a good tutorial from HP explaining all the ways to access the BIOS on various computers: <https://store.hp.com/us/en/tech-takes/how-to-enter-bios-setup-windows-pcs> <sup>[[Archive.org]][250]</sup>
...
-   Disable Secure Boot if you intend to use QubesOS as they do not support it out of the box[^287]. Keep it on if you intend to use Linux/Windows.
...
So, what is Secure Boot[^288]? In short, it is a UEFI security feature designed to prevent your computer from booting an operating system from which the bootloader was not signed by specific keys stored in the UEFI firmware of your laptop.

When the operating system (or the Bootloader[^289]) supports it, you can store the keys of your bootloader in your UEFI firmware, and this will prevent booting up any unauthorized Operating System (such as a live OS USB or anything similar).
...
-   Defcon 22, <https://www.youtube.com/watch?v=QDSlWa9xQuA> <sup>[[Invidious]][251]</sup>

-   BlackHat 2016, <https://www.youtube.com/watch?v=0fZdL3ufVOI> <sup>[[Invidious]][252]</sup>
...
Take a moment to set a firmware password according to the tutorial here: <https://support.apple.com/en-au/HT204455> <sup>[[Archive.org]][253]</sup>

You should also enable firmware password reset protection (available from Catalina) according to the documentation here: <https://support.apple.com/en-gb/guide/security/sec28382c9ca/web> <sup>[[Archive.org]][254]</sup>
...
Here is a good cheap method to make your laptop tamper-proof using Nail Polish (with glitter) <https://mullvad.net/en/help/how-tamper-protect-laptop/> <sup>[[Archive.org]][255]</sup> [^290] (with pictures).
...
This route will make extensive use of Virtual Machines[^291], they will require a host OS to run the Virtualization software. You have three recommended choices in this part of the guide:
...
Linux is also not necessarily the best choice for anonymity depending on your threat model. This is because using Windows will allow us to **conveniently** use Plausible Deniability[^292] (aka Deniable Encryption[^293]) easily at the OS level. Windows is also unfortunately at the same time a privacy nightmare[^294] but is the only easy to set up option for using OS-wide plausible deniability. Windows telemetry and telemetry blocking are also widely documented which should mitigate many issues.

**So, what is Plausible Deniability?** You can cooperate with an adversary requesting access to your device/data without revealing your true secret. All this using Deniable Encryption[^295].

A soft lawful adversary could ask for your encrypted laptop password. At first, you could refuse to give out any password (using your "right to remain silent", "right not to incriminate yourself") but some countries are implementing laws[^296]'[^297] to exempt this from such rights (because terrorists and "think of the children"). In that case, you might have to reveal the password or face jail time in contempt of court. This is where plausible deniability will come into play.
...
This also means you could set up your own advanced "plausible deniability" setup using any Host OS by storing for instance Virtual Machines on a Veracrypt hidden volume container (be careful of traces in the Host OS tho that would need to be cleaned if the host OS is persistent, see [Some additional measures against forensics][Some additional measures against forensics:] section later). There is a project for achieving this within Tails (<https://github.com/aforensics/HiddenVM> <sup>[[Archive.org]][237]</sup>) which would make your Host OS non-persistent and use plausible deniability within Tails.

In the case of Windows, plausible deniability is also the reason you should ideally have Windows 10 Home (and not Pro). This is because Windows 10 Pro natively offers a full-disk encryption system (Bitlocker[^298]) where Windows 10 Home offers no full-disk encryption at all. We will later use third-party open-source software for encryption that will allow full-disk encryption on Windows 10 Home. This will give you a good (plausible) excuse to use this software. While using this software on Windows 10 Pro would be suspicious.
...
Cold Boot attacks[^300] are trickier than the Evil Maid Attack but can be part of an Evil Maid attack as it requires an adversary to come into possession of your laptop while you are actively using your device or shortly afterward.

The idea is rather simple, as shown in this video[^301], an adversary could theoretically quickly boot your device on a special USB key that would copy the content of the RAM (the memory) of the device after you shut it down. If the USB ports are disabled or if they feel like they need more time, they could open it and "cool down" the memory using a spray or other chemicals (liquid nitrogen for instance) preventing the memory from decaying. They could then be able to copy its content for analysis. This memory dump could contain the key to decrypt your device. We will later apply a few principles to mitigate these.

In the case of Plausible Deniability, there have been some forensics studies[^302] about technically proving the presence of the hidden data with a simple forensic examination (without a Cold Boot/Evil Maid Attack) but these have been contested by other studies[^303] and by the maintainer of Veracrypt[^304] so I would not worry too much about those yet.
...
-   You should limit the use of Sleep stand-by and instead use Shutdown or Hibernate to prevent the encryption keys from staying in RAM when your computer goes to sleep. This is because sleep will maintain power in your memory for resuming your activity faster. Only hibernation and shutdown will actually clear the key from the memory[^305].

See also <https://www.whonix.org/wiki/Cold_Boot_Attack_Defense> <sup>[[Archive.org]][258]</sup> and <https://www.whonix.org/wiki/Protection_Against_Physical_Attacks> <sup>[[Archive.org]][259]</sup>

Here are also some interesting tools to consider for Linux users to defend against these:

-   <https://github.com/0xPoly/Centry> <sup>[[Archive.org]][260]</sup> (unfortunately unmaintained it seems so I made a fork and pull request updating for Veracrypt <https://github.com/AnonymousPlanet/Centry> <sup>[[Archive.org]][261]</sup> which should still work)

-   <https://github.com/hephaest0s/usbkill> <sup>[[Archive.org]][262]</sup> (unfortunately unmaintained as well it seems)

-   <https://github.com/Lvl4Sword/Killer> <sup>[[Archive.org]][263]</sup>

-   <https://askubuntu.com/questions/153245/how-to-wipe-ram-on-shutdown-prevent-cold-boot-attacks> <sup>[[Archive.org]][264]</sup>

-   (Qubes OS, Intel CPU only) <https://github.com/QubesOS/qubes-antievilmaid> <sup>[[Archive.org]][265]</sup>
...
-   Windows ShellBags that are stored within the Windows Registry silently storing various histories of accessed volumes/files/folders[^306].

-   Windows Indexing keeping traces of the files present in your user folder by default[^307].

-   Recent lists (aka Jump Lists) in Windows and various apps keeping traces of recently accessed documents[^308].

-   Many more traces in various logs, please see this convenient interesting poster for more insight: <https://www.sans.org/security-resources/posters/windows-forensic-analysis/170/download> <sup>[[Archive.org]][266]</sup>

###### macOS:

-   Gatekeeper[^309] and XProtect keeping track of your download history in a local database and file attributes.
...
Consider also reading **<https://www.whonix.org/wiki/Full_Disk_Encryption#Encrypting_Whonix_VMs>** <sup>[[Archive.org]][267]</sup>
...
You should always remember that despite the reputation, Linux mainstream distributions (Ubuntu for instance) are not necessarily better at security than other systems such as macOS and Windows. See this reference to understand why <https://madaidans-insecurities.github.io/linux.html> <sup>[[Archive.org]][268]</sup>.
...
-   Using LUKS:
    ...
    -   Without plausible deniability:
        ...
        -   (Recommended and easy) Encrypt as part of the installation process: <https://ubuntu.com/tutorials/install-ubuntu-desktop> <sup>[[Archive.org]][269]</sup>
        ...
        -   (Tedious but possible) Encrypt after installation: <https://help.ubuntu.com/community/ManualFullSystemEncryption> <sup>[[Archive.org]][270]</sup>
...
There are several ways to achieve plausible deniability on Linux[^310] and it is possible to achieve. Here are some more details about some of the ways I would recommend. All these options require some higher level of skills at using Linux.

##### The Detached Headers Way:

While not supported yet by this guide, it is possible to achieve a form of deniability on Linux using LUKS by using detached LUKS headers. For now, I will redirect you toward this page for more information: <https://wiki.archlinux.org/title/Dm-crypt/Specialties#Encrypted_system_using_a_detached_LUKS_header> <sup>[[Archive.org]][271]</sup>
...
-   If you are not sure, just make sure you did not enable any telemetry and follow this tutorial if needed <https://vitux.com/how-to-force-ubuntu-to-stop-collecting-your-data-from-your-pc/> <sup>[[Archive.org]][272]</sup>
...
-   Disable Bluetooth if enabled by following this guide <https://www.addictivetips.com/ubuntu-linux-tips/disable-bluetooth-in-ubuntu/> <sup>[[Archive.org]][273]</sup> or issuing the following command:

    -   ```sudo systemctl disable bluetooth.service --force```

-   Disable Indexing if enabled by default (Ubuntu >19.04) by following this guide <https://www.linuxuprising.com/2019/07/how-to-completely-disable-tracker.html> <sup>[[Archive.org]][274]</sup> or issuing the following commands:
...
-   <https://www.how2shout.com/linux/how-to-hibernate-ubuntu-20-04-lts-focal-fossa/> <sup>[[Archive.org]][275]</sup>

-   <http://www.lorenzobettini.it/2020/07/enabling-hibernation-on-ubuntu-20-04/> <sup>[[Archive.org]][276]</sup>

-   <https://blog.ivansmirnov.name/how-to-set-up-hibernate-on-ubuntu-20-04/> <sup>[[Archive.org]][277]</sup>

After Hibernate is enabled, change the behavior so that your laptop will hibernate when you close the lid by following this tutorial for Ubuntu 20.04 <http://ubuntuhandbook.org/index.php/2020/05/lid-close-behavior-ubuntu-20-04/> <sup>[[Archive.org]][278]</sup> and this tutorial for Ubuntu 18.04 <https://tipsonubuntu.com/2018/04/28/change-lid-close-action-ubuntu-18-04-lts/> <sup>[[Archive.org]][279]</sup>

Unfortunately, this will not clean the key from memory directly when hibernating. To avoid this at the cost of some performance, you might consider encrypting the swap file by following this tutorial: <https://help.ubuntu.com/community/EnableHibernateWithEncryptedSwap> <sup>[[Archive.org]][280]</sup>

These settings should mitigate cold boot attacks if you can hibernate fast enough.

#### Enable MAC address randomization:

-   Ubuntu, follow these steps <https://help.ubuntu.com/community/AnonymizingNetworkMACAddresses> <sup>[[Archive.org]][281]</sup>.

-   Any other distro: you will have to find the documentation yourself, but it should be quite similar to the Ubuntu tutorial.

-   Consider this tutorial which should still work: <https://josh.works/shell-script-basics-change-mac-address> <sup>[[Archive.org]][282]</sup>

#### Hardening Linux:

As a light introduction for new Linux users, consider <https://www.youtube.com/watch?v=Sa0KqbpLye4> <sup>[[Invidious]][283]</sup>

For more in-depth and advanced options, refer to:

-   This excellent guide: <https://madaidans-insecurities.github.io/guides/linux-hardening.html> <sup>[[Archive.org]][284]</sup>

-   This excellent wiki resource: <https://wiki.archlinux.org/title/Security> <sup>[[Archive.org]][285]</sup>

-   These excellent scripts are based on the guide and wiki above: <https://codeberg.org/SalamanderSecurity/PARSEC> <sup>[[Archive.org]][286]</sup>
...
As a light introduction for new macOS users, consider <https://www.youtube.com/watch?v=lFx5icuE6Io> <sup>[[Invidious]][287]</sup>

Now to go more in-depth in securing and hardening your macOS, I recommend reading this GitHub guide which should cover many of the issues: <https://github.com/drduh/macOS-Security-and-Privacy-Guide> <sup>[[Archive.org]][288]</sup>
...
First, you should set up a firmware password following this guide from Apple: <https://support.apple.com/en-us/HT204455> <sup>[[Archive.org]][289]</sup>

Unfortunately, some attacks are still possible and an adversary could disable this password so you should also follow this guide to prevent disabling the firmware password from anyone including Apple: <https://support.apple.com/en-gb/guide/security/sec28382c9ca/web> <sup>[[Archive.org]][254]</sup>
...
These are the infamous "unblockable telemetry" calls from macOS Big Sur disclosed here: <https://sneak.berlin/20201112/your-computer-isnt-yours/> <sup>[[Archive.org]][290]</sup>
...
But you should document yourself on the actual issue before acting. This page is a good place to start: <https://blog.jacopo.io/en/post/apple-ocsp/> <sup>[[Archive.org]][291]</sup>
...
You should enable full disk encryption on your Mac using Filevault according to this part of the guide: <https://github.com/drduh/macOS-Security-and-Privacy-Guide#full-disk-encryption> <sup>[[Archive.org]][288]</sup>
...
As a light introduction, consider watching <https://www.youtube.com/watch?v=vNRics7tlqw> <sup>[[Invidious]][292]</sup>
...
Alternatively, you could use this free piece of software: <https://technitium.com/tmac/> <sup>[[Archive.org]][293]</sup>
...
Veracrypt[^311] is the software I will recommend for full-disk encryption, file encryption, and plausible deniability. It is a fork of the well-known but deprecated and unmaintained TrueCrypt. It can be used for:
...
It is to my knowledge the only (convenient and usable by anyone) free, open-source, and openly audited[^312] encryption software that also provides plausible deniability for widespread use and it works with Windows Home Edition.

Go ahead and download and install Veracrypt from: <https://www.veracrypt.fr/en/Downloads.html> <sup>[[Archive.org]][294]</sup>

After installation, please take a moment to review the following options that will help mitigate some attacks:

-   Encrypt the memory with a Veracrypt option[^313] (settings > performance/driver options > encrypt RAM) at a cost of 5-15% performance. This setting will also disable hibernation (which does not actively clear the key when hibernating) and instead encrypt the memory altogether to mitigate some cold-boot attacks.
...
-   Enable the Veracrypt option to mount volumes as removable volumes (Settings > Preferences > Mount volume as removable media). This will prevent Windows from writing some logs about your mounts in the Event logs[^314] and prevent some local data leaks.
...
Basically, you can follow the instructions here: <https://support.microsoft.com/en-us/windows/turn-on-device-encryption-0c453637-bc88-5f74-5105-741561aae838> <sup>[[Archive.org]][295]</sup>
...
Unfortunately, this is not enough. With this setup, your Bitlocker key can just be stored as-is in the TPM chip of your computer. This is rather problematic as the key can be extracted in some cases with ease[^315]'[^316].

To mitigate this, we will have to enable a few more options as per the recommendations of Microsoft[^317]:
...
By default, Windows 10 might not offer you this possibility so you should enable it by following this Microsoft tutorial: <https://docs.microsoft.com/en-us/troubleshoot/windows-client/deployment/disable-and-re-enable-hibernation> <sup>[[Archive.org]][296]</sup>
...
-   Route C: Plausible Deniability Encryption of your Operating system (you will have a "hidden OS" and a "decoy OS" running on the laptop):
    ...
    -   Cons:
        ...
        -   No use with an SSD drive due to the requirement of disabling Trim[^318] Operations[^319]. This will severely degrade the performance/health of your SSD drive over time.

**As you can see, Route C only offers two privacy advantages over the others, and it will only be of use against a soft lawful adversary. Remember <https://en.wikipedia.org/wiki/Rubber-hose_cryptanalysis>** <sup>[[Wikiless]][241]</sup> <sup>[[Archive.org]][242]</sup>**.**
...
-   To rescue disk[^320] or not rescue disk, well that is up to you. I recommend making one (just in case), just make sure to store it outside your encrypted drive (USB key for instance or wait and see the end of this guide for guidance on safe backups). This rescue disk will not store your passphrase and you will still need it to use it.

-   Wipe mode:

    -   If you have no sensitive data yet on this laptop, select None

    -   If you have sensitive data on an SSD, Trim alone should take care of it[^321] but I would recommend one pass (random data) just to be sure.
...
Read the Veracrypt Documentation <https://www.veracrypt.fr/en/VeraCrypt%20Hidden%20Operating%20System.html> <sup>[[Archive.org]][297]</sup> (Process of Creation of Hidden Operating System part) and <https://www.veracrypt.fr/en/Security%20Requirements%20for%20Hidden%20Volumes.html> <sup>[[Archive.org]][298]</sup> (Security Requirements and Precautions Pertaining to Hidden Volumes).

This is how your system will look after this process is done:

![][299]

(Illustration from Veracrypt Documentation, <https://veracrypt.fr/en/VeraCrypt%20Hidden%20Operating%20System.html> <sup>[[Archive.org]][297]</sup>)
...
Remember to read <https://www.veracrypt.fr/en/VeraCrypt%20Hidden%20Operating%20System.html> <sup>[[Archive.org]][297]</sup>
...
-   To rescue disk[^326] or not rescue disk, well that is up to you. I recommend making one (just in case), just make sure to store it outside your encrypted drive (USB key for instance or wait and see the end of this guide for guidance on safe backups). This rescue disk will not store your passphrase and you will still need it to use it.
...
Before going to the next step, you should learn the way to mount your Outer Volume safely for writing content on it. This is also explained in this official Veracrypt Documentation <https://www.veracrypt.fr/en/Protection%20of%20Hidden%20Volumes.html> <sup>[[Archive.org]][301]</sup>
...
Take some time to read again the "Possible Explanations for Existence of Two Veracrypt Partitions on Single Drive" of the Veracrypt documentation here <https://www.veracrypt.fr/en/VeraCrypt%20Hidden%20Operating%20System.html> <sup>[[Archive.org]][297]</sup>
...
In this route, we will make extensive use of the free Oracle Virtualbox[^327] software. This is a virtualization software in which you can create Virtual Machines that emulate a computer running a specific OS (if you want to use something else like Xen, Qemu, KVM, or VMWARE, feel free to do so but this part of the guide covers Virtualbox only for convenience).

So, you should be aware that Virtualbox is not the virtualization software with the best track record in terms of security and some of the reported issues[^328] have not been completely fixed to this date[^329] and if you are using Linux with a bit more technical skills, you should consider using KVM instead by following the guide available at Whonix here <https://www.whonix.org/wiki/KVM> <sup>[[Archive.org]][302]</sup> and here <https://www.whonix.org/wiki/KVM#Why_Use_KVM_Over_VirtualBox.3F> <sup>[[Archive.org]][303]</sup>
...
**This solution however has one main drawback to consider: Interference with Tor Stream Isolation**[^330].
...
(Illustration from Marcelo Martins, <https://stakey.club/en/decred-via-tor-network/> <sup>[[Archive.org]][309]</sup>)

VPN/Proxy over Tor falls on the right-side[^331] meaning using a VPN/Proxy over Tor forces Tor to use one circuit for all activities instead of multiple circuits for each. This means that using a VPN/Proxy over Tor can reduce the effectiveness of Tor in some cases and should therefore be used only for some specific cases:
...
-   <https://www.whonix.org/wiki/Stream_Isolation> <sup>[[Archive.org]][310]</sup>

-   <https://tails.boum.org/contribute/design/stream_isolation/> <sup>[[Archive.org]][311]</sup>

-   <https://www.whonix.org/wiki/Tunnels/Introduction#Comparison_Table> <sup>[[Archive.org]][312]</sup>
...
-   <https://www.whonix.org/wiki/Comparison_Of_Tor_with_CGI_Proxies,_Proxy_Chains,_and_VPN_Services#Tor_and_VPN_Services_Comparison> <sup>[[Archive.org]][314]</sup>

-   <https://www.whonix.org/wiki/Why_does_Whonix_use_Tor> <sup>[[Archive.org]][315]</sup>

-   <https://www.researchgate.net/publication/324251041_Anonymity_communication_VPN_and_Tor_a_comparative_study> <sup>[[Archive.org]][316]</sup>

-   <https://gist.github.com/joepie91/5a9909939e6ce7d09e29#file-vpn-md> <sup>[[Archive.org]][317]</sup>

-   <https://schub.wtf/blog/2019/04/08/very-precarious-narrative.html> <sup>[[Archive.org]][318]</sup>
...
-   Tor Project: <https://gitlab.torproject.org/legacy/trac/-/wikis/doc/TorPlusVPN> <sup>[[Archive.org]][319]</sup>

-   Tails Documentation:

    -   <https://gitlab.tails.boum.org/tails/blueprints/-/wikis/vpn_support/> <sup>[[Archive.org]][320]</sup>

    -   <https://tails.boum.org/support/faq/index.en.html#index20h2> <sup>[[Archive.org]][321]</sup>

-   Whonix Documentation (in this order):

    -   <https://www.whonix.org/wiki/Tunnels/Introduction> <sup>[[Archive.org]][322]</sup>

    -   <https://www.whonix.org/wiki/Tunnels/Connecting_to_Tor_before_a_VPN> <sup>[[Archive.org]][323]</sup>

    -   <https://www.whonix.org/wiki/Tunnels/Connecting_to_a_VPN_before_Tor> <sup>[[Archive.org]][313]</sup>
...
This route will use Virtualization and Whonix[^332] as part of the anonymization process. Whonix is a Linux distribution composed of two Virtual Machines:
...
Later, you will create and run several Virtual Machines within Virtualbox for your sensitive activities. Virtualbox provides a feature called "Snapshots"[^333] that allow for saving the state of a VM at any point in time. If for any reason later you want to go back to that state, you can restore that snapshot at any moment.
...
This will allow you to turn your VMs into a kind of disposable "Live Operating Systems" (like Tails discussed earlier). Meaning that you will be able to erase all the traces of your activities within a VM by restoring a Snapshot to an earlier state. Of course, this will not be "as good" as Tails (where everything is stored in memory) as there might be traces of this activity left on your hard disk. Forensics studies have shown the ability to recover data from a reverted VM[^334]. Fortunately, there will be ways to remove those traces after the deletion or reverting to an earlier snapshot. Such techniques will be discussed in the [Some additional measures against forensics][Some additional measures against forensics:] section of this guide.

#### Download Virtualbox and Whonix utilities:

You should download a few things within the host OS:

-   The latest version of the Virtualbox installer according to your Host OS <https://www.virtualbox.org/wiki/Downloads> <sup>[[Archive.org]][326]</sup>

-   (Skip this if you cannot use Tor natively of through a VPN) The latest Whonix OVA file from <https://www.whonix.org/wiki/Download> <sup>[[Archive.org]][327]</sup> according to your preference (Linux/Windows, with a Desktop interface XFCE for simplicity or only with the text-client for advanced users)
...
For ideal security, you should follow the recommendations provided here for each Virtualbox Virtual Machine <https://www.whonix.org/wiki/Virtualization_Platform_Security#VirtualBox_Hardening> <sup>[[Archive.org]][328]</sup> :
...
Finally, also follow this recommendation to desync the clock you are your VM compared to your host OS <https://www.whonix.org/wiki/Network_Time_Synchronization#Spoof_the_Initial_Virtual_Hardware_Clock_Offset> <sup>[[Archive.org]][329]</sup>
...
Also, consider applying these mitigations from VirtualBox to mitigate Spectre[^335]/Meltdown[^336] vulnerabilities by running this command from the VirtualBox Program Directory. All of these are described here: <https://www.whonix.org/wiki/Spectre_Meltdown> <sup>[[Archive.org]][83]</sup> (be aware these can impact severely the performance of your VMs but should be done for best security).

Finally, consider the security advice from Virtualbox themselves here <https://www.virtualbox.org/manual/ch13.html> <sup>[[Archive.org]][330]</sup>
...
-   Import Whonix file Into Virtualbox following the instructions on <https://www.whonix.org/wiki/VirtualBox/XFCE> <sup>[[Archive.org]][244]</sup>

-   Start the Whonix VMs

Remember at this stage that if you are having issues connecting to Tor due to censorship or blocking, you should consider connecting using Bridges as explained in this tutorial <https://www.whonix.org/wiki/Bridges> <sup>[[Archive.org]][331]</sup>.

-   Update the Whonix VMs by following the instructions on <https://www.whonix.org/wiki/Operating_System_Software_and_Updates#Updates> <sup>[[Archive.org]][332]</sup>
...
**Important Note: You should also read these very good recommendations over there <https://www.whonix.org/wiki/DoNot>** <sup>[[Archive.org]][333]</sup> **as most of those principles will also apply to this guide. You should also read their general documentation here <https://www.whonix.org/wiki/Documentation>** <sup>[[Archive.org]][334]</sup> **which will also provide tons of advice like this guide.**
...
**It is also the only VM that will provide Stream Isolation pre-configured for most apps by default**[^337]**.**

If you want additional software on the Workstation (such as another Browser), follow their guide here <https://www.whonix.org/wiki/Install_Software> <sup>[[Archive.org]][335]</sup>

Consider running Whonix in Live Mode if for extra malware protection, See <https://www.whonix.org/wiki/Anti-Forensics_Precautions> <sup>[[Archive.org]][336]</sup>

Do not forget to apply the VM hardening recommendations here: [Virtualbox Hardening recommendations].

Consider using AppArmor on your Whonix Workstations by following this guide: <https://www.whonix.org/wiki/AppArmor> <sup>[[Archive.org]][337]</sup>

#### Linux (any distro):

**Be careful, any customization you make to the non-Whonix guest VMs (keyboard layout, language, time zone, screen resolution, or other) could be used to fingerprint your VMs later. See <https://www.whonix.org/wiki/VM_Fingerprinting>** <sup>[[Archive.org]][338]</sup>
...
Refer to this tutorial <https://www.whonix.org/wiki/Other_Operating_Systems> <sup>[[Archive.org]][339]</sup> for detailed instructions.
...
Go with the Official Windows 10 Pro VM and harden it yourself: see [Appendix C: Windows Installation Media Creation][300] and go with the ISO route.

There is also another option you might hear about which is Windows AME (Ameliorated) from the <https://ameliorated.info/> <sup>[[Archive.org]][340]</sup> project which is a special Windows 10 build stripped from all telemetry/advertising and update components. **I do not recommend this option due to the insecure nature of that release lacking the latest updates/patches and lacking the ability to update completely. I do believe the project general idea is good, but it is just too insecure/risky and contains unnecessary software picked by the dev.**
...
Basically, follow the tutorial here: <https://www.android-x86.org/documentation/virtualbox.html> <sup>[[Archive.org]][341]</sup>
...
-   Windows Host OS:
    ...
    -   Virtualbox Catalina Tutorial: <https://www.wikigain.com/install-macos-catalina-on-virtualbox-on-windows/> <sup>[[Archive.org]][342]</sup>

    -   Virtualbox Big Sur Tutorial: <https://www.wikigain.com/how-to-install-macos-big-sur-on-virtualbox-on-windows-pc/> <sup>[[Archive.org]][343]</sup>
...
You will need something to store your data (logins/passwords, identities, and TOTP[^338] information).

For this purpose, I strongly recommend KeePassXC because of its integrated TOTP feature. This is the ability to create entries for 2FA[^339] authentication with the authenticator feature.
...
-   Whonix: [https://www.whonix.org/wiki/KeePassXC][] <sup>[[Archive.org]][345]</sup>

-   Linux:

    -   Download from <https://keepassxc.org/download/> <sup>[[Archive.org]][346]</sup>

    -   Follow the tutorial here <https://keepassxc.org/docs/KeePassXC_GettingStarted.html#_linux> <sup>[[Archive.org]][347]</sup>

-   Windows:

    -   Download from <https://keepassxc.org/download/> <sup>[[Archive.org]][346]</sup>

    -   Follow the tutorial here [https://KeePassXC.org/docs/KeePassXC_GettingStarted.html#_microsoft_windows][] <sup>[[Archive.org]][347]</sup>

-   macOS:

    -   Download from <https://keepassxc.org/download/> <sup>[[Archive.org]][346]</sup>

    -   Follow the tutorial here <https://keepassxc.org/docs/KeePassXC_GettingStarted.html#_macos> <sup>[[Archive.org]][347]</sup>
...
**This way is not supported by the Whonix project**[^340] but I will go ahead and give this option anyway. IMHO this is helpful to prevent your Host OS from leaking any information while you are using the Whonix VMs.
...
Qubes OS is not a Linux distribution[^341] but a Xen distribution. It is different from Linux distributions because it will make extensive use of Virtualization and Compartmentalization so that any app will run in a different VM (Qube). As a bonus, Qubes OS integrates Whonix by default and allows for increased privacy and anonymity. It is highly recommended that you document yourself over Qubes OS principles before going this route. Here are some recommended resources:

-   Qubes OS Introduction, <https://www.qubes-os.org/intro/> <sup>[[Archive.org]][351]</sup>

-   Qubes OS Video Tours, <https://www.qubes-os.org/video-tours/> <sup>[[Archive.org]][352]</sup>

-   Qubes OS Getting Started, <https://www.qubes-os.org/doc/getting-started/> <sup>[[Archive.org]][353]</sup>

-   YouTube, Life Behind the Tinfoil: A Look at Qubes and Copperhead - Konstantin Ryabitsev, The Linux Foundation <https://www.youtube.com/watch?v=8cU4hQg6GvU> <sup>[[Invidious]][354]</sup>

-   YouTube, I used the reasonably-secure Qubes OS for 6 months and survived - Matty McFatty [@themattymcfatty] <https://www.youtube.com/watch?v=sbN5Bz3v-uA> <sup>[[Invidious]][355]</sup>

-   YouTube, Qubes OS: How it works, and a demo of this VM-centric OS <https://www.youtube.com/watch?v=YPAvoFsvSbg> <sup>[[Invidious]][356]</sup>

This OS is recommended by prominent figures such as Edward Snowden and Privacytools.io.

Qubes is the best option in this guide for people who are more comfortable with Linux and tech in general. But it has some downsides such as the lack of OS-wide plausible deniability, its hardware requirements, and its hardware compatibility. While you can run this on 4GB of RAM as per their requirements[^342], the recommended RAM is 16GB. I would recommend against using Qubes OS if you have less than 8GB of RAM. If you want a comfortable experience, you should have 16GB, if you want a particularly enjoyable experience, you should have 24GB or 32GB.
...
You should also check their hardware compatibility here <https://www.qubes-os.org/hcl/> <sup>[[Archive.org]][357]</sup> before proceeding. Your mileage might vary, and you might experience several issues about hardware compatibility that you will have to troubleshoot and solve yourself.

I think that if you can afford it and are comfortable with the idea of using Linux, you should go with this route as it is probably the best one in terms of security and privacy. The only disadvantage of this route is that it does not provide a way to enable OS-wide plausible deniability[^292], unlike the Whonix route.
...
**This solution however has one main drawback to consider: Interference with Tor Stream Isolation**[^343].
...
VPN/Proxy over Tor falls on the right-side[^344] meaning using a VPN/Proxy over Tor forces Tor to use one circuit for all activities instead of multiple circuits for each. This means that using a VPN/Proxy over Tor can reduce the effectiveness of Tor in some cases and should therefore be used only for some specific cases:
...
Note, if you're having issues accessing the Tor Network due to blocking/censorship, you could try using Tor Bridges (see Tor Documentation <https://2019.www.torproject.org/docs/bridges> <sup>[[Archive.org]][236]</sup> and Whonix Documentation <https://www.whonix.org/wiki/Bridges> <sup>[[Archive.org]][331]</sup>).
...
We will follow the instructions from their own guide <https://www.qubes-os.org/doc/installation-guide/> <sup>[[Archive.org]][360]</sup>:

(Secure Boot is not supported as per their FAQ: <https://www.qubes-os.org/faq/#is-secure-boot-supported> <sup>[[Archive.org]][361]</sup> so it should be disabled in the BIOS/UEFI settings.)
...
-   Install Qubes OS according to the installation guide:
    ...
    -   **If you want to use Tor or VPN over Tor: Check the** "**Enabling system and template updates over the Tor anonymity network using Whonix" during the last step. This will force all Qubes OS updates to go through Tor. While this will significantly reduce your update speed, it will increase your anonymity from the start.** (If you are having issues connecting to Tor due to censorship or blocking, consider using Tor Bridges as recommended earlier. Just follow the tutorial provided here: <https://www.whonix.org/wiki/Bridges> <sup>[[Archive.org]][331]</sup>)
...
-   Select Random to randomize your Mac Address
    ...
    -   **Warning: This setting should work in most cases but can be unreliable on some network adapters. Please refer to this documentation if you want to be sure: <https://github.com/Qubes-Community/Contents/blob/master/docs/privacy/anonymizing-your-mac-address.md>** <sup>[[Archive.org]][362]</sup>
...
Basically, AppArmor[^346] is an application sandboxing system. By default, it is not enabled but supported by Qubes OS.
...
-   About the Debian VMs:
    ...
    -   Head out and read <https://wiki.debian.org/AppArmor> <sup>[[Archive.org]][363]</sup>

-   About any other Linux VM:

    -   Head out and read:

        -   <https://wiki.archlinux.org/title/AppArmor> <sup>[[Archive.org]][364]</sup>

        -   <https://wiki.debian.org/AppArmor> <sup>[[Archive.org]][363]</sup>

-   About the Whonix VMs, you should consider enabling and using AppArmor, especially on the Whonix VMs of Qubes OS:

    -   First, you should head out and read <https://www.whonix.org/wiki/AppArmor> <sup>[[Archive.org]][337]</sup>

    -   Secondly, you should head out again and read <https://www.whonix.org/wiki/Qubes/AppArmor> <sup>[[Archive.org]][365]</sup>

##### SELinux:

SELinux[^347] is similar to AppArmor. The differences between SELinux and AppArmor are technical details into which we will not get.

Here is a good explanation of what it is: <https://www.youtube.com/watch?v=_WOKRaM-HI4> <sup>[[Invidious]][366]</sup>
...
So, head out and read <https://docs.fedoraproject.org/en-US/quick-docs/getting-started-with-selinux/> <sup>[[Archive.org]][367]</sup>
...
This is based on the tutorial provided by Qubes OS themselves (<https://github.com/Qubes-Community/Contents/blob/master/docs/configuration/vpn.md> <sup>[[Archive.org]][368]</sup>). If you are familiar with this process, you can follow their tutorial. Here is mine:
...
-   Test your Connectivity:
    ...
    -   If you are going for VPN over Tor, Test the VM connectivity to Tor by launching a Browser within the ProxyVM and going to <https://check.torproject.org> <sup>[[Archive.org]][369]</sup> (It should say you are connected to Tor)
...
> (These will block outbound traffic when the VPN is down, it is a kill switch, more information here <https://linuxconfig.org/how-to-create-a-vpn-killswitch-using-iptables-on-linux> <sup>[[Archive.org]][370]</sup> )
...
-   Test the ProxyVM VPN connectivity by starting a Browser within it and going to your VPN provider test page. It should now say you are connected to a VPN:
    ...
    -   Mullvad: <https://mullvad.net/en/check/> <sup>[[Archive.org]][371]</sup>

    -   IVPN: <https://www.ivpn.net/> <sup>[[Archive.org]][372]</sup> (check the top banner)

    -   ProtonVPN: Follow their instructions here <https://protonvpn.com/support/vpn-ip-change/> <sup>[[Archive.org]][373]</sup>
...
-   Create a Whonix Workstation Disposable VM (follow this tutorial <https://www.whonix.org/wiki/Qubes/DisposableVM> <sup>[[Archive.org]][374]</sup>)
...
If you want to use Brave: apply the instructions from <https://brave.com/linux/> <sup>[[Archive.org]][375]</sup> (Fedora 28+ section) and run the following commands:
...
Edit the Whonix Disposable VM template and follow instructions here <https://www.whonix.org/wiki/Install_Software> <sup>[[Archive.org]][335]</sup>
...
Now you will have to follow the instructions from here: <https://github.com/anbox/anbox-modules> <sup>[[Archive.org]][376]</sup>:
...
Now we will follow their other tutorial from here: <https://github.com/anbox/anbox/blob/master/docs/install.md> <sup>[[Archive.org]][377]</sup>:
...
Captcha[^350] stands for "Completely Automated Public Turing test to tell Computers and Humans Apart" are Turing tests[^351] puzzles you need to complete before accessing a form/website. You will mostly encounter those provided by Google (reCAPTCHA service[^352]) and Cloudflare (hCaptcha[^353]). hCaptcha is used on 15% of the internet by their own metrics[^354].
...
If you often use VPNs or Tor, you will quickly encounter many captchas everywhere[^355]. Quite often when using Tor, even if you succeed in solving all the puzzles (sometimes dozens in a row), you will still be denied after solving the puzzles.

See <https://gitlab.torproject.org/legacy/trac/-/wikis/org/doc/ListOfServicesBlockingTor> <sup>[[Archive.org]][306]</sup>

While most people think those puzzles are only about solving a little puzzle, it is important to understand that it is much more complex, and that modern Captchas uses advanced machine learning and risk analysis algorithms to check if you are human[^356]:

-   They check your browser, cookies, and browsing history using Browser fingerprinting[^357].

-   They track your cursor movements (speed, accuracy) and use algorithms to decide if it is "human/organic".

-   They track your behavior before/during/after the tests to ensure you are "human"[^358].
...
Most platforms (including the privacy-oriented ones such as Signal/Telegram/ProtonMail will require a phone number to register, and most countries now make it mandatory to submit a proof of ID to register[^360].
...
-   RiseUp: <https://riseup.net> <sup>[[Tor Mirror]][383]</sup> (It has come to my attention that the site now, unfortunately, requires an invitation from a current registered user)
...
These services will allow creating random aliases for your anonymous e-mail (on ProtonMail for example) and could increase your general privacy if you do not want to disclose that e-mail for any purpose. They are both recommended by privacytools.Io.
...
Sometimes some platforms will allow you to log in with a Tor IP but not sign-up (See <https://gitlab.torproject.org/legacy/trac/-/wikis/org/doc/ListOfServicesBlockingTor> <sup>[[Archive.org]][306]</sup>). Those platforms will keep a convenient permanent log of the IP you used during sign-up. And some will keep such logs indefinitely including all the IPs you used to log in (hi Facebook).
...
Browser and Device[^361] Fingerprinting are usually integrated into the Captcha services but also in other various services.

Many platforms (like Google[^362]) will check your browser for various capabilities and settings and block Browsers they do not like. This is one of the reasons I recommend using Chromium-based Browsers such as Brave Browser over Tor Browser within this VM.
...
While this might seem innocent, this is obviously an ID verification and de-anonymization method. This is just indirectly relying on third-party financial KYC[^225] regulations.

This is for instance now the case on YouTube for some European Users[^363] but also used by services like Amazon that requires a valid payment method for creating an account.
...
This option is often presented as the "default one", hiding away the "Sign-in with e-mail and password" with clever Dark Patterns[^364] and unfortunately sometimes needed.
...
This guide will not cover this one (yet) as it is mainly used on financial platforms (that will be able to identify you with other means anyway) and some dating apps like Tinder[^365]. Unfortunately, this method is now also sometimes being used on Facebook[^366] and Instagram as part of their verification methods (tho I did not face it yet so far).
...
Recently even platforms such as PornHub decided to implement similar measures in the future[^367].

This verification is extremely hard to defeat but possible. A method to possibly defeat those would be to use "deep fake" technology software such as the open-source FaceSwap <https://github.com/deepfakes/faceswap> <sup>[[Archive.org]][387]</sup> to generate the required verification pictures using a randomly computer-generated face that would be swapped over the picture of a complicit model (or a stock photo).
...
Some countries have even developed hotlines where you can report any subversive content[^368].
...
So what? Well instead of not trusting anyone or anything, I would advise to **"Trust but verify"**[^369] (or "Never trust, always verify" if you are more hardcore about it and want to apply Zero-Trust Security[^370]) instead.
...
This is the fun part where you will now create your identities from thin air. These identities do not exist but should be plausible and look "organic". They should ideally have a story, a "legend" (yes this is the real term for this[^371]).
...
-   Similarly, origin IP geolocation (your IP/location when you go to "whatsmyipaddress.com") should match your identity location as much as possible (You can pick this in the VPN client if you use the 3 layers approach or just create a new identity in Tor Browser or Brave Tor Tab until you get the appropriate Exit node, or configure Tor to restrict your Exit Nodes). You could exclude any exit IP that is not located in Western Europe/US/Canada/Japan/South Korea/Australia/New Zealand as you will have fewer issues. Ideally, you should get a European Union IP to get additional GDPR protection and if possible, a German exit IP due to their legal stance on using anonymous accounts on online platforms.

-   Brave Browser (Chromium-based) with a Private Tor Tab has (IMHO) a better acceptance level than Tor Browser (Firefox based). You will experience fewer issues with captchas and online platforms[^362] if you use Brave than if you use Tor Browser (feel free to try this yourself).

-   For every identity, you should have a matching profile picture associated with it. For this purpose, I recommend you just go to <https://thispersondoesnotexist.com/> <sup>[[Archive.org]][388]</sup> and generate a computer-generated profile picture (Do note that algorithms have been developed[^372]'[^373] to detect these and it might not work 100% of the time). You can also generate such pictures yourself from your computer if you prefer by using the open-source StyleGan project here <https://github.com/NVlabs/stylegan2> <sup>[[Archive.org]][389]</sup>. Just refresh the page until you find a picture that matches your identity in all aspects (age, sex, and ethnicity) and save that picture. It would be even better to have several pictures associated with that identity, but I do not have an "easy way" of doing that yet.

    -   **Bonus**, you could also make it more real by using this service (with an anonymous identity) <https://www.myheritage.com/deep-nostalgia> <sup>[[Archive.org]][390]</sup> to make a picture more lifelike. Here is an example:
...
You could also achieve the same result without using MyHeritage and by doing it yourself using for example <https://github.com/AliaksandrSiarohin/first-order-model> <sup>[[Archive.org]][392]</sup> but this will require more manual operations (**and requires an NVIDIA GPU**).
...
-   Always use TOTP 2FA (not SMS to prevent Sim Swapping attacks[^374] and to keep your identity working when your pre-paid card expires) using KeePassXC when available to secure your logins to various platforms.

-   Remember [Appendix A2: Guidelines for passwords and passphrases].

Here is also a good guide on this specific topic: <https://gendersec.tacticaltech.org/wiki/index.php/Complete_manual#.22Real.22_names> <sup>[[Archive.org]][393]</sup>
...
-   Whonix/Tails: Create/Edit a file ```/usr/local/etc/torrc.d/50_user.conf```[^375].

-   On Tor Browser: Edit the torrc file located at ```Browser/TorBrowser/Data/Tor```[^376].
...
**Please note that this is restricting Onion Routing could limit your Anonymity if you are too restrictive. You can see a visualized list of available Exit Nodes here: <https://www.bigdatacloud.com/insights/tor-exit-nodes>** <sup>[[Archive.org]][394]</sup>
...
On my side, I strongly condemn this type of real-name policy. See for instance this Wikipedia article giving some examples: <https://en.wikipedia.org/wiki/Facebook_real-name_policy_controversy> <sup>[[Wikiless]][395]</sup> <sup>[[Archive.org]][396]</sup>

Here are some more references about the German case for reference:

-   <https://slate.com/technology/2018/02/why-some-americans-are-cheering-germany-for-taking-on-facebooks-real-name-policy.html> <sup>[[Archive.org]][397]</sup>

-   <https://www.theverge.com/2018/2/12/17005746/facebook-real-name-policy-illegal-german-court-rules> <sup>[[Archive.org]][398]</sup>

-   <https://www.pcmag.com/news/german-court-rules-facebooks-real-name-policy-is-illegal> <sup>[[Archive.org]][399]</sup>

-   <https://www.vzbv.de/sites/default/files/downloads/2018/02/14/18-02-12_vzbv_pm_facebook-urteil_en.pdf> <sup>[[Archive.org]][400]</sup>

-   <https://www.pcmag.com/news/german-court-rules-facebooks-real-name-policy-is-illegal> <sup>[[Archive.org]][399]</sup>

-   <https://www.reuters.com/article/us-germany-facebook/german-court-rules-facebook-use-of-personal-data-illegal-idUSKBN1FW1FI> <sup>[[Archive.org]][401]</sup>
...
-   **Consider using the recommended tools on <https://privacytools.io/>** <sup>[[Archive.org]][404]</sup> **for better privacy instead of the usual mainstream ones.**

-   **Consider using the recommended tools on <https://www.whonix.org/wiki/Documentation>** <sup>[[Archive.org]][334]</sup> **as well instead of the usual mainstream ones such as E-mail providers: <https://www.whonix.org/wiki/E-Mail#Anonymity_Friendly_Email_Provider_List>** <sup>[[Archive.org]][405]</sup>

**The following overview does not mention the privacy practices of those platforms but only their requirements for registering an account. If you want to use privacy-aware tools and platforms, head on to <https://privacytools.io/>** <sup>[[Archive.org]][404]</sup>
...
-   Is this against their ToS? No, but yes <https://www.amazon.com/gp/help/customer/display.html?nodeId=202140280> <sup>[[Archive.org]][406]</sup>
...
-   Is this against their ToS? Yes <https://www.apple.com/legal/internet-services/icloud/en/terms.html> <sup>[[Archive.org]][407]</sup>
...
-   Is this against their ToS? Yes <https://www.binance.com/en/terms> <sup>[[Archive.org]][408]</sup>
...
-   Is this against their ToS? No <https://briarproject.org/privacy-policy/> <sup>[[Archive.org]][409]</sup>
...
-   Is this against their ToS? No <https://discord.com/terms> <sup>[[Archive.org]][410]</sup>
...
-   Is this against their ToS? No <https://element.io/terms-of-service> <sup>[[Archive.org]][411]</sup>
...
-   Is this against their ToS? Yes <https://www.facebook.com/terms.php> <sup>[[Archive.org]][412]</sup>
...
-   Can you create accounts through Tor? Yes, but it is very difficult and their onion address[^377] will not help. In most cases, you'll just have a random error at sign-up and your account suspended after sign-in."
...
-   Is this against their ToS? No <https://docs.github.com/en/free-pro-team@latest/github/site-policy/github-terms-of-service> <sup>[[Archive.org]][413]</sup>
...
-   Is this against their ToS? No <https://about.gitlab.com/handbook/legal/subscription-agreement/> <sup>[[Archive.org]][414]</sup>
...
-   Is this against their ToS? No <https://policies.google.com/terms> <sup>[[Archive.org]][415]</sup>
...
If your account gets suspended [^380] (this can happen on sign-up, after signing-up or after using it in some Google services), you can still get it unsuspended by submitting[^381] an appeal/verification (which will again require your Phone number and possibly an e-mail contact with Google support with the reason). Suspension of the account does not disable the e-mail forwarding, but the suspended account will be deleted after a while.
...
It is also possible that Google will require an ID check through indirect financial KYC or ID picture check if you try to access/publish mature content on their platform[^382].

#### HackerNews:

-   Is this against their ToS? No <https://www.ycombinator.com/legal/#tou> <sup>[[Archive.org]][416]</sup>
...
-   Is this against their ToS? **Maybe?** I am not sure <https://help.instagram.com/581066165581870?ref=dp> <sup>[[Archive.org]][417]</sup>
...
-   Is this against their ToS? No <https://jami.net/privacy-policy/> <sup>[[Archive.org]][418]</sup>
...
-   Is this against their ToS? No <https://www.ivpn.net/tos/> <sup>[[Archive.org]][419]</sup>
...
-   Is this against their ToS? Yes <https://www.kraken.com/legal> <sup>[[Archive.org]][420]</sup>
...
LinkedIn will then require you to go through a verification process that will, unfortunately, require you to send an ID proof (identity card, passport, driver's license). This ID verification is processed by a company called Jumio[^383] that specializes in ID proofing. This is most likely a dead end as this would force you to develop some strong Photoshop skills.
...
-   Is this against their ToS? No, it looks fine <https://onlyfans.com/terms> <sup>[[Archive.org]][426]</sup>
...
-   Is this against their ToS? No <https://ProtonMail.com/terms-and-conditions> <sup>[[Archive.org]][427]</sup>
...
They are recommended by privacytools.io[^384]. Their only apparent issue is that they do require (in most cases) a phone number or another e-mail address for registration (when you try to register from a VPN or Tor at least).

They claim they do not store/link the phone/e-mail associated with the registration but only store a hash that is not linked to the account[^385]. If their claim is true and the hash is not linked to your account, and that you followed my guide about the phone number, you should be reasonably safe from tracking.
...
-   Is this against their ToS? No <https://protonvpn.com/terms-and-conditions> <sup>[[Archive.org]][428]</sup>
...
-   Is this against their ToS? No <https://www.redditinc.com/policies> <sup>[[Archive.org]][429]</sup>
...
-   Is this against their ToS? Yes <https://slashdotmedia.com/terms-of-use/> <sup>[[Archive.org]][430]</sup>
...
-   Is this against their ToS? No <https://telegram.org/tos> <sup>[[Archive.org]][431]</sup>
...
Their appeal process is just sending an e-mail to <recover@telegram.org> <sup>[[Archive.org]][432]</sup> stating your phone number and issue and hope they answer.
...
-   Is this against their ToS? No <https://tutanota.com/terms/> <sup>[[Archive.org]][433]</sup>
...
This time you will need to submit an appeal using a form[^386], provide a good reason and wait for the appeal to be processed by Twitter. During that process, you may receive an e-mail (on ProtonMail) asking you to reply to a customer service ticket to prove that you do have access to your e-mail and that it is you. This will be directed toward your Gmail address but will arrive on your ProtonMail.
...
-   Is this against their ToS? No <https://www.twitch.tv/p/en/legal/terms-of-service/> <sup>[[Archive.org]][434]</sup>
...
-   Is this against their ToS? **Yes** <https://www.whatsapp.com/legal/updates/terms-of-service-eea> <sup>[[Archive.org]][435]</sup>
...
You are going to have to find a separate way to post there using at least seven proxies[^387] that are not known by 4chan blocking system (hint: Anonymous VPS using Monero is probably your best option).
...
End-to-end Encryption[^388] (aka e2ee) is a rather simple concept. It just means only you and your destination know each-others public encryption keys and no one in between that would be eavesdropping would be able to decrypt the communication.
...
More on this topic on this YouTube video: <https://www.youtube.com/watch?v=zSQtyW_ywZc> <sup>[[Invidious]][438]</sup>
...
Zero-Access Encryption[^390] at rest is used when you store data at some provider (let us say your chat history or chat backups) but this history or backup is encrypted on your side and cannot be read or decrypted by the provider hosting it.
...
Such a feature would have prevented important hacks such as the Cambridge Analytica scandal[^391] if it were implemented.
...
-   The Sealed Sender option[^392].

-   The Private Contact Discovery[^393].

-   The Private Group System[^394].
...
-   Wikipedia, <https://en.wikipedia.org/wiki/Comparison_of_instant_messaging_protocols> <sup>[[Wikiless]][439]</sup> <sup>[[Archive.org]][440]</sup>

-   Wikipedia, <https://en.wikipedia.org/wiki/Comparison_of_cross-platform_instant_messaging_clients> <sup>[[Wikiless]][441]</sup> <sup>[[Archive.org]][442]</sup>

-   Secure Messaging Apps <https://www.securemessagingapps.com/> <sup>[[Archive.org]][443]</sup>

-   ProtonMail Blog, <https://protonmail.com/blog/whatsapp-alternatives/> <sup>[[Archive.org]][444]</sup>

-   Whonix Documentation, Instant Messenger Chat <https://www.whonix.org/wiki/Chat> <sup>[[Archive.org]][445]</sup>

-   Have a look at <https://securechatguide.org/featuresmatrix.html> <sup>[[Archive.org]][446]</sup> which is also a good comparison table for messaging apps.
...
I will recommend these options in that order (as also recommend by privacytools.io[^395]'[^396] except for Session):

-   macOS:

    -   Native Tor Onion Routing Support (**preferred**):

        -   OnionShare version >2.3 (<https://onionshare.org/> <sup>[[Tor Mirror]][447]</sup> <sup>[[Archive.org]][448]</sup>)**
    ...
    -   Non-Native Tor Support (needs additional steps for ideal anonymity to proxy it through Tor through Virtualization or Proxying):
        ...
        -   Element/Matrix.org (<https://element.io/> <sup>[[Archive.org]][449]</sup>)

        -   Jami (<https://jami.net/> <sup>[[Archive.org]][450]</sup>)*
...
-   Linux:
    ...
    -   Native Tor Onion Routing Support (**preferred**):
        ...
        -   Briar (<https://briarproject.org/> <sup>[[Archive.org]][451]</sup>)*
...
-   iOS:
    ...
    -   Due to the lack of any better option (Briar is not on iOS) and general Tor support, only one option can still be possible here and it is normally not recommended by this guide but the only option: Session Messenger: <https://getsession.org/> <sup>[[Archive.org]][452]</sup>. Why is it not recommended these days within the privacy community? Well, it is because they recently[^397] dropped two key security features from their protocol: Perfect Forward Secrecy and Deniability which are considered rather essential in most other apps. Yet Session has been audited[^398] with satisfactory results but that audit took place before those changes. We also currently lack sufficient information on LokiNet (the Onion Routing Network used by Session) to endorse it. Session is still recommended by some like Techlore[^399].
...
If you must do so, you should take some steps because you cannot trust any organization to protect your anonymity[^400]:
...
For this, I strongly recommend the use of SecureDrop[^401] (<https://securedrop.org/> <sup>[[Archive.org]][453]</sup>) which is an open-source project from the Freedom of the Press Foundation.

-   Ideally, you should use SecureDrop over Tor and you will find a curated list of those here <https://github.com/alecmuffett/real-world-onion-sites#securedrop> <sup>[[Archive.org]][454]</sup>
...
-   Check regularly the eventual appearance of compromise of any of your identities using <https://haveibeenpwned.com/> <sup>[[Archive.org]][455]</sup> (obviously from a safe environment).
...
The process is fairly simple and all you will need is to follow Veracrypt tutorial here: <https://www.veracrypt.fr/en/Beginner%27s%20Tutorial.html> <sup>[[Archive.org]][456]</sup>
...
Qubes OS recommends using their own utility for backups as documented here <https://www.qubes-os.org/doc/backup-restore/> <sup>[[Archive.org]][457]</sup>. But it is just a hassle and provides limited added value unless you just want to back up a single Qube. So instead, I am also recommending just making a full image with Clonezilla which will remove all the hassle and bring you back a working system in a few simple steps.
...
For smaller files, consider Cryptpad.fr as recommended by Privacytools.io at <https://privacytools.io/providers/cloud-storage/> <sup>[[Archive.org]][461]</sup> (limited to 1GB total).
...
If you just want to save information (text), I will recommend the use of secure and private pastebins[^403]. Mostly I will stick to the ones recommended by privacytools.io (<https://privacytools.io/providers/paste/> <sup>[[Archive.org]][462]</sup> ):
...
To that, the answer is very simple and a clear consensus for everyone: <https://syncthing.net/> <sup>[[Archive.org]][463]</sup>
...
So, the fact is that it is very unlikely[^409]'[^410] and difficult for a forensic examiner to be able to recover data from a Trimmed SSD but it is not completely impossible either[^411]'[^412]'[^413] if they are fast enough and have access to extensive equipment, skills, and motivation.
...
-   Easy options for less experienced users:
    ...
    -   Buy PartedMagic[^414] for 11$ and use it to erase any disk.
...
Trim should be sufficient in most cases and you could just use the blkdiscard command to force an entire device trim as explained here: <https://wiki.archlinux.org/index.php/Solid_state_drive#Trim_an_entire_device> <sup>[[Archive.org]][471]</sup>
...
-   Option A: Follow one of these tutorials:
    ...
    -   <https://linuxhint.com/completely_wipe_hard_drive_ubuntu/> <sup>[[Archive.org]][474]</sup>

    -   <https://linoxide.com/linux-command/commands-wipe-disk-linux/> <sup>[[Archive.org]][475]</sup>

    -   <https://wiki.archlinux.org/index.php/Securely_wipe_disk> <sup>[[Archive.org]][476]</sup>

I recommend using dd or shred for this purpose.

-   Option B: Install and use BleachBit <https://www.bleachbit.org/download/linux> <sup>[[Archive.org]][472]</sup> or follow this EFF tutorial <https://ssd.eff.org/en/module/how-delete-your-data-securely-linux> <sup>[[Archive.org]][477]</sup>
...
-   Option B: Use external tools such as:
    ...
    -   Eraser (open-source): <https://eraser.heidi.ie/download/> <sup>[[Archive.org]][478]</sup>

    -   KillDisk Free: <http://killdisk.com/killdisk-freeware.htm> <sup>[[Archive.org]][479]</sup>
...
Unfortunately, the macOS Recovery disk utility will not be able to perform a secure erase of your SSD drive as stated in Apple documentation <https://support.apple.com/en-gb/guide/disk-utility/dskutl14079/mac> <sup>[[Archive.org]][480]</sup>.
...
-   <https://support.apple.com/guide/disk-utility/erase-and-reformat-a-storage-device-dskutl14079/mac> <sup>[[Archive.org]][481]</sup>

-   <https://support.apple.com/guide/disk-utility/encrypt-protect-a-storage-device-password-dskutl35612/mac> <sup>[[Archive.org]][482]</sup> or using Veracrypt full disk encryption.
...
Follow this tutorial: <https://support.apple.com/guide/disk-utility/erase-and-reformat-a-storage-device-dskutl14079/mac> <sup>[[Archive.org]][481]</sup> and use the secure erase option from Disk Utility which should work fine on HDD and Thumb drives.
...
-   Right-click a file/folder:
    ...
    -   BleachBit: Shred with BleachBit (or see this tutorial from the EFF <https://ssd.eff.org/en/module/how-delete-your-data-securely-windows> <sup>[[Archive.org]][484]</sup>)
...
-   Decrypt and re-encrypt (using LUKS for instance following this tutorial <https://wiki.archlinux.org/index.php/dm-crypt/Device_encryption#Re-encrypting_devices> <sup>[[Archive.org]][485]</sup>) the whole drive to overwrite all free space after data deletion. This will ensure overwriting of all the free space.

-   Trim using ```fstrim --all``` and then fill up the entire free space of the disk using a utility such as:

    -   BleachBit <https://www.bleachbit.org/download/linux> <sup>[[Archive.org]][472]</sup>
    ...
    -   Use the old school dd method (taken from this answer <https://superuser.com/questions/19326/how-to-wipe-free-disk-space-in-linux> <sup>[[Archive.org]][473]</sup>) run these commands on the drive you want to fill:
...
-   You can do this the graphical way with BleachBit following this tutorial from the EFF: <https://ssd.eff.org/en/module/how-delete-your-data-securely-linux> <sup>[[Archive.org]][477]</sup>

-   Or you can do this from the command line following this tutorial: <https://linuxhint.com/completely_wipe_hard_drive_ubuntu/> <sup>[[Archive.org]][474]</sup> (For this purpose I recommend wipe and shred).
...
-   Decrypt and re-encrypt (using LUKS using this tutorial <https://wiki.archlinux.org/index.php/dm-crypt/Device_encryption#Re-encrypting_devices> <sup>[[Archive.org]][485]</sup> or Veracrypt from the graphical interface for instance) the whole drive to overwrite all free space after data deletion. This will ensure overwriting of all the free space.
...
-   Fill the free space using one of those methods:
    ...
    -   Use the old school dd method (taken from this answer <https://superuser.com/questions/19326/how-to-wipe-free-disk-space-in-linux> <sup>[[Archive.org]][473]</sup>) run these commands:
...
Please follow this documentation to Trim within Qubes OS: <https://github.com/Qubes-Community/Contents/blob/master/docs/configuration/disk-trim.md> <sup>[[Archive.org]][486]</sup>
...
-   Decrypt and re-encrypt the whole drive to overwrite all free space after data deletion. This will ensure overwriting of all the free space. I didn't find a reliable tutorial on how to do this safely on Qubes OS but it's possible this Tutorial could work as well <https://wiki.archlinux.org/index.php/dm-crypt/Device_encryption#Re-encrypting_devices> <sup>[[Archive.org]][485]</sup> (at your own risk, this has not been tested yet).

-   Refer to this Documentation (<https://github.com/Qubes-Community/Contents/blob/master/docs/configuration/disk-trim.md> <sup>[[Archive.org]][486]</sup>) and then trim using "fstrim --all" and then fill up the entire free space of the disk using a utility such as:
...
If Trim is supported and enabled by your Linux Distribution for your external SSD drive. There should be no issue in securely deleting data normally and just issue a "fstrim --all" from the terminal to trim the drive. Refer to this Documentation (<https://github.com/Qubes-Community/Contents/blob/master/docs/configuration/disk-trim.md> <sup>[[Archive.org]][486]</sup>) to enable trim on a drive.
...
-   If your file system is APFS, you do not need to worry about Trim, it happens asynchronously as the OS writes data[^420] according to their documentation.
...
-   If your file system is HFS+, you could run First Aid on your System Drive from the Disk Utility which should perform a Trim operation in the details (<https://support.apple.com/en-us/HT210898> <sup>[[Archive.org]][487]</sup>)

![][488]

#### System/Internal, External HDD drive or a Thumb Drive:

Unfortunately, Apple has removed the secure erase options from the trash bin even for HDD drives[^421]. So, you are left with using other tools:

-   Permanent Eraser <http://www.edenwaith.com/products/permanent%20eraser/> <sup>[[Archive.org]][489]</sup>

-   From the terminal, you can use the "rm --P filename" command which should erase the file and overwrite it as explained in this EFF tutorial <https://ssd.eff.org/en/module/how-delete-your-data-securely-macos> <sup>[[Archive.org]][490]</sup>.
...
Consider also reading this documentation if you're going with Whonix <https://www.whonix.org/wiki/Anti-Forensics_Precautions> <sup>[[Archive.org]][336]</sup> as well as their general hardening tutorial for all platforms here <https://www.whonix.org/wiki/System_Hardening_Checklist> <sup>[[Archive.org]][491]</sup>

### Removing Metadata from Files/Documents/Pictures:

#### Pictures and videos:

On Windows, macOS, and Linux I would recommend ExifTool (<https://exiftool.org/> <sup>[[Archive.org]][492]</sup>) and/or ExifCleaner (<https://exifcleaner.com/> <sup>[[Archive.org]][493]</sup>) that allows viewing and/or removing those properties.

**ExifTool is natively available on Tails and Whonix Workstation.**

##### ExifCleaner:

Just install it from <https://exifcleaner.com/> <sup>[[Archive.org]][493]</sup>, run and drag and drop the files into the GUI.
...
Here is a tutorial to remove metadata from a Picture using OS provided tools: <https://www.purevpn.com/internet-privacy/how-to-remove-metadata-from-photos> <sup>[[Archive.org]][494]</sup>

##### Cloaking/Obfuscating to prevent picture recognition:

Consider the use of Fawkes <https://sandlab.cs.uchicago.edu/fawkes/> <sup>[[Archive.org]][495]</sup> (<https://github.com/Shawn-Shan/fawkes> <sup>[[Archive.org]][496]</sup>) to cloak the images from picture recognition tech on various platforms.

Or if you want online versions, consider:

-   <https://lowkey.umiacs.umd.edu/> <sup>[[Archive.org]][497]</sup>

-   <https://adversarial.io/> <sup>[[Archive.org]][498]</sup>

#### PDF Documents:

##### PDFParanoia (Linux/Windows/macOS/QubesOS):

Consider using <https://github.com/kanzure/pdfparanoia> <sup>[[Archive.org]][499]</sup> which will remove metadata and watermarks on any PDF.
...
First, here is a tutorial to remove metadata from Office documents: <https://support.microsoft.com/en-us/office/remove-hidden-data-and-personal-information-by-inspecting-documents-presentations-or-workbooks-356b7b5d-77af-44fe-a07f-9aa4d085966f> <sup>[[Archive.org]][500]</sup>. Make sure however that you do use the latest version of Office with the latest security updates.

Alternatively, on Windows, macOS, Qubes OS, and Linux I would recommend ExifTool (<https://exiftool.org/> <sup>[[Archive.org]][492]</sup>) and/or ExifCleaner (<https://exifcleaner.com/> <sup>[[Archive.org]][493]</sup>) that allows viewing and/or removing those properties
...
In addition, on Windows, macOS, Qubes OS, and Linux I would recommend ExifTool (<https://exiftool.org/> <sup>[[Archive.org]][492]</sup>) and/or ExifCleaner (<https://exifcleaner.com/> <sup>[[Archive.org]][493]</sup>) that allows viewing and/or removing additional properties
...
Another option good tool IMHO to remove metadata from various documents is the open-source mat2 recommended by privacytools.io[^422] (<https://0xacab.org/jvoisin/mat2> <sup>[[Archive.org]][501]</sup>) which you can use on Linux quite easily. I never managed to make it work properly within Windows due to various dependencies issues despite the provided instructions. It is however very straightforward to install and use on Linux.
...
Mat2 is also pre-installed on the Whonix Workstation VM[^423] and available on Tails by default[^424].
...
Note that it's possible to run Whonix in Live mode leaving no traces when you shut down the VMs, consider reading their documentation here <https://www.whonix.org/wiki/VM_Live_Mode> <sup>[[Archive.org]][503]</sup> and here <https://www.whonix.org/wiki/Warning#Whonix_.E2.84.A2_Persistence_vs_Live_vs_Amnesic> <sup>[[Archive.org]][213]</sup>.
...
Most of the info from this section can also be found at this nice guide <https://github.com/drduh/macOS-Security-and-Privacy-Guide> <sup>[[Archive.org]][288]</sup>
...
Lastly, you can also disable Gatekeeper altogether by issuing the following command in the terminal[^425]:

-   ```sudo spctl --master-disable```

Refer to this section of this guide for further information <https://github.com/drduh/macOS-Security-and-Privacy-Guide#gatekeeper-and-xprotect> <sup>[[Archive.org]][288]</sup>
...
See this section of this guide for guidance on where to find and how to delete such artifacts: <https://github.com/drduh/macOS-Security-and-Privacy-Guide#metadata-and-artifacts> <sup>[[Archive.org]][288]</sup>

Many of those can be deleted using various commercial third-party tools but I would personally recommend using the free and well-known Onyx which you can find here: <https://www.titanium-software.fr/en/onyx.html> <sup>[[Archive.org]][504]</sup>. Unfortunately, it is closed-source, but it is notarized, signed, and has been trusted for many years.
...
-   If your file system is HFS+ (or any other than APFS), you could run First Aid on your System Drive from the Disk Utility which should perform a Trim operation in the details (<https://support.apple.com/en-us/HT210898> <sup>[[Archive.org]][487]</sup>).

![][488]

### Linux (Qubes OS):

Please consider their guidelines <https://github.com/Qubes-Community/Contents/blob/master/docs/security/security-guidelines.md> <sup>[[Archive.org]][505]</sup>

If you are using Whonix on Qubes OS, please consider following some of their guides:

-   Whonix System Hardening guide <https://www.whonix.org/wiki/System_Hardening_Checklist> <sup>[[Archive.org]][491]</sup>

-   Enabling App Armor on Qubes <https://www.whonix.org/wiki/Qubes/AppArmor> <sup>[[Archive.org]][365]</sup>

-   Also, consider the use of Linux Kernel Guard <https://www.whonix.org/wiki/Linux_Kernel_Runtime_Guard_LKRG> <sup>[[Archive.org]][506]</sup>
...
Consider the use of Linux Kernel Guard as an added measure <https://www.whonix.org/wiki/Linux_Kernel_Runtime_Guard_LKRG> <sup>[[Archive.org]][506]</sup>
...
Windows Event logs will keep many various pieces of information that could contain traces of your activities such as the devices that were mounted (including Veracrypt NTFS volumes for instance[^314]), your network connections, app crash information, and various errors. It is always best to clean those up regularly. Do not do this on the Decoy OS.
...
-   Launch Regedit using this tutorial: <https://support.microsoft.com/en-us/windows/how-to-open-registry-editor-in-windows-10-deab38e6-91d6-e0aa-4b7c-8878d9e07b11> <sup>[[Archive.org]][507]</sup>
...
As explained earlier, Shellbags are basically histories of accessed volumes/files on your computer. Remember that shellbags are exceptionally useful sources of information for forensics[^306] and you need to clean those. Especially if you mounted any "hidden volume" anywhere. Again, you should not do this on the Decoy OS:

-   Download Shellbag Analyzer & Cleaner from <https://privazer.com/en/download-shellbag-analyzer-shellbag-cleaner.php> <sup>[[Archive.org]][508]</sup>
...
-   Download and install PrivaZer from <https://privazer.com/en/download.php> <sup>[[Archive.org]][509]</sup>
    ...
    -   (If you did select Free Space cleaning) Within Clean Options **(Be careful with this option as it will erase all the free space on the selected partition, especially if you are running the decoy OS. Do not erase the free space or anything else on the second partition as you risk destroying your Hidden OS)**
        ...
        -   If you have an SSD drive:
            ...
            -   Secure Overwriting Tab: I would just pick Normal Deletion + Trim (Trim itself should be enough[^321]). Secure Deletion with Trim[^318] (1 pass) might be redundant and overkill here if you intend to overwrite the free space anyway.
...
-   Get and install the latest version from BleachBit here <https://www.bleachbit.org/download> <sup>[[Archive.org]][510]</sup>
...
-   <https://justdeleteme.xyz/> <sup>[[Archive.org]][511]</sup>

-   <https://justgetmydata.com/> <sup>[[Archive.org]][512]</sup>
...
Go to their "Remove outdated content from Google Search" page here: <https://search.google.com/search-console/remove-outdated-content> <sup>[[Archive.org]][513]</sup> and submit a request accordingly.
...
Go to their "Content Removal" page here: <https://www.bing.com/webmasters/tools/contentremoval> <sup>[[Archive.org]][514]</sup> and submit a request accordingly.
...
DuckDuckGo does not store a cached version of pages[^426] and will instead forward you to a Google/Bing cached version if available.
...
Once have your Yandex account, head to the Yandex Webmaster tools <https://webmaster.yandex.com> <sup>[[Archive.org]][515]</sup> and then select Tools and Delete URL <https://webmaster.yandex.com/tools/del-url/> <sup>[[Archive.org]][516]</sup>
...
Yes, Yahoo Search still exists but as per their help page <https://help.yahoo.com/kb/SLN4530.html> <sup>[[Archive.org]][517]</sup>, there is no way to remove information or refresh information besides waiting. This could take 6 to 8 weeks.
...
-   <https://inteltechniques.com/workbook.html> <sup>[[Archive.org]][520]</sup>
...
A commonly used technique that combines the idea of a Dead Drop[^428] and Secure Communication Obfuscation[^429] through Steganography[^430] and/or Kleptography[^431] and has many names such as Koalang[^432] or "Talking Around" or even "Social Steganography". This technique is very old and still widely used nowadays by teenagers to bypass parental control. It is hiding in plain sight.
...
Wait, what is OPSEC? Well, OPSEC means Operations Security[^433]. The basic definition is: "OPSEC is the process of protecting individual pieces of data that could be grouped together to give the bigger picture ".
...
-   Read those tips here <https://www.whonix.org/wiki/DoNot> <sup>[[Archive.org]][333]</sup>
...
See current goals and donate at <https://anonymousplanet.org/donations.html> <sup>[[Mirror]][536]</sup> <sup>[[Archive.org]][537]</sup> <sup>[[Tor Mirror]][538]</sup> or directly by sending Monero (XMR) to this Address: ```4549BGJrEPBfpiPRL9CVGzGMgJnC1Dzf8EXLVfY8Ukrnj7LzkTV611dGf9tuQHiSQjbixsNWiffNiV5fPB3LkyF7UXi3vwQ```
...
-   **Very Special Thanks to Edward Snowden and who inspired me to write this guide (buy and read his book please <https://en.wikipedia.org/wiki/Permanent_Record_(autobiography)>** <sup>[[Wikiless]][546]</sup> <sup>[[Archive.org]][547]</sup>**)**
...
As written earlier in this guide and as noted by Privacytools.io[^435], Windows 10 is a privacy nightmare. And disabling everything during and after the installation using the settings available to you is not enough. The amount of telemetry data collected by Microsoft is staggering and could defeat your attempts at keeping secrets. You will need to download and use a couple of utilities to (hopefully) force Windows 10 into not sending data back to Microsoft.
...
-   Download and install W10Privacy from <https://www.w10privacy.de/english-home/> <sup>[[Archive.org]][548]</sup>
...
-   Download and run WindowsSpyBlocker from <https://crazymax.dev/WindowsSpyBlocker/download/> <sup>[[Archive.org]][549]</sup>
...
-   Also, consider using ShutUp10 from <https://www.oo-software.com/en/shutup10> <sup>[[Archive.org]][550]</sup>
...
**As a bonus, it could be interesting to also consider Hardening your Windows Host OS somewhat. See <https://github.com/beerisgood/windows10_hardening>** <sup>[[Archive.org]][551]</sup>[]{#_Appendix_C:_Windows .anchor} (This is a security guide, not a privacy guide. If you use this guide, do not enable Hyper-V as it does not play well with Virtualbox and do not enable features that were specifically disabled for privacy reasons earlier. Such as SmartScreen, cloud protection...)

# Appendix C: Windows Installation Media Creation

These are the steps to create a Windows 10 (21H1) Installation Media using this tool and instructions:

<https://www.microsoft.com/en-us/software-download/windows10> <sup>[[Archive.org]][552]</sup>
...
-   System Rescue:
    ...
    -   Create a System Rescue USB disk following these instructions <https://www.system-rescue.org/Installing-SystemRescue-on-a-USB-memory-stick/> <sup>[[Archive.org]][553]</sup> (download the ISO and write to a USB stick with Rufus).
...
-   SATA SSD:
    ...
    -   ATA Secure Erase:
        ...
        -   Follow one of these tutorials
            ...
            -   <https://wiki.archlinux.org/index.php/Solid_state_drive/Memory_cell_clearing> <sup>[[Archive.org]][554]</sup>

            -   <https://ata.wiki.kernel.org/index.php/ATA_Secure_Erase> <sup>[[Archive.org]][555]</sup>

            -   <https://tinyapps.org/docs/wipe_drives_hdparm.html> <sup>[[Archive.org]][556]</sup>

    -   ATA Sanitize:

        -   Follow this tutorial <https://tinyapps.org/docs/ata_sanitize_hdparm.html> <sup>[[Archive.org]][557]</sup>
...
-   NVMe SSD:
    ...
    -   Follow one of these tutorials:
        ...
        -   <https://tinyapps.org/docs/nvme-secure-erase.html> <sup>[[Archive.org]][558]</sup>

        -   <https://tinyapps.org/docs/nvme-sanitize.html> <sup>[[Archive.org]][559]</sup>

# Appendix E: Clonezilla

-   Get Clonezilla by just following these instructions: <https://clonezilla.org/liveusb.php> <sup>[[Archive.org]][560]</sup> (I recommend the Alternative version AMD64 that should work with most recent laptops)

-   Boot from Clonezilla

-   Follow these steps to make a backup: <https://clonezilla.org/show-live-doc-content.php?topic=clonezilla-live/doc/01_Save_disk_image> <sup>[[Archive.org]][561]</sup>
...
-   You are done, if you need to restore, follow these instructions: <https://clonezilla.org/show-live-doc-content.php?topic=clonezilla-live/doc/02_Restore_disk_image> <sup>[[Archive.org]][562]</sup>
...
If you are experiencing issues connecting to Tor due to Censorship or Blocking, you might consider using Tor bridges as explained here: <https://bridges.torproject.org/> <sup>[[Archive.org]][563]</sup>
...
-   Native Tools:
    ...
    -   Windows 10 Disk Cleanup Utility: <https://support.microsoft.com/en-us/windows/disk-cleanup-in-windows-10-8a96ff42-5751-39ad-23d6-434b4d5b9a68> <sup>[[Archive.org]][564]</sup>
...
-   Windows 10 Optimize Utility (Defrag on HDD Drives): <https://support.microsoft.com/en-us/windows/defragment-your-windows-10-pc-048aefac-7f1f-4632-d48a-9700c4ec702a> <sup>[[Archive.org]][565]</sup>
...
-   Third-Party Tools:
    ...
    -   The open-source utility BleachBit <https://www.bleachbit.org/> <sup>[[Archive.org]][566]</sup>

    -   The closed-source utility PrivaZer <https://privazer.com/> <sup>[[Archive.org]][567]</sup>
...
-   The Windows USN journal which stores plenty of information[^437].

-   The Windows System Resource Usage Monitor (SRUM)[^438].

-   Various histories of various programs (such as the recent lists).

-   Various logs

-   The free (unallocated) space of your hard drive[^439].
...
Several utilities are recommended (like the old unmaintained DBAN[^440] or System Rescue CD (<https://www.system-rescue.org/> <sup>[[Archive.org]][568]</sup>)) for this but I will recommend the use of ShredOS.

Feel free to go with DBAN instead if you want (using this tutorial: <https://www.lifewire.com/how-to-erase-a-hard-drive-using-dban-2619148> <sup>[[Archive.org]][569]</sup>), the process is basically the same but will not work out of the box with UEFI laptops.

If you want to go with System-Rescue, just head to their website and follow the instructions.

## Windows:

-   Download ShredOS from <https://github.com/PartialVolume/shredos.2020.02> <sup>[[Archive.org]][570]</sup>

-   Unzip the ISO file

-   Download Rufus from <https://rufus.ie/> <sup>[[Archive.org]][571]</sup>
...
-   Follow instructions on <https://github.com/PartialVolume/shredos.2020.02> <sup>[[Archive.org]][570]</sup>
...
-   SanDisk DashBoard: <https://kb.sandisk.com/app/answers/detail/a_id/15108/~/dashboard-support-information> <sup>[[Archive.org]][572]</sup>

-   Seagate SeaTools: <https://www.seagate.com/support/downloads/seatools/> <sup>[[Archive.org]][573]</sup>

-   Samsung Magican: <https://www.samsung.com/semiconductor/minisite/ssd/download/tools/> <sup>[[Archive.org]][574]</sup>

-   Kingston SSD Manager: <https://www.kingston.com/unitedstates/en/support/technical/ssdmanager> <sup>[[Archive.org]][575]</sup>

-   Lenovo:

    -   Most likely native utility available within the BIOS/UEFI, please check

    -   Drive Erase Utility: <https://support.lenovo.com/us/en/downloads/ds019026-thinkpad-drive-erase-utility-for-resetting-the-cryptographic-key-and-erasing-the-solid-state-drive-thinkpad> <sup>[[Archive.org]][576]</sup>

-   Crucial Storage Executive: <https://www.crucial.com/support/storage-executive> <sup>[[Archive.org]][577]</sup>

-   Western Digital Dashboard: <https://support.wdc.com/downloads.aspx?p=279> <sup>[[Archive.org]][578]</sup>

-   HP: Follow instructions on <https://store.hp.com/us/en/tech-takes/how-to-secure-erase-ssd> <sup>[[Archive.org]][579]</sup>

-   Transcend SSD Scope: <https://www.transcend-info.com/Support/Software-10/> <sup>[[Archive.org]][580]</sup>

-   Dell:

    -   Most likely native utility available within the BIOS/UEFI, please check <https://www.dell.com/support/kbdoc/en-us/000134997/using-the-dell-bios-data-wipe-function-for-optiplex-precision-and-latitude-systems-built-after-november-2015?lwp=rt> <sup>[[Archive.org]][581]</sup>

## Tools that provide only support from running OS (for external drives).

-   Toshiba Storage Tools: <https://www.toshiba-storage.com/downloads/> <sup>[[Archive.org]][582]</sup>
...
Some might be very efficient[^441] but many are gimmicky gadgets.
...
**Use the manufacturer-provided tools to check and perform these operations** ... It is pretty much the only way to be sure it is not only supported but actually works. Some utilities can tell you whether it is supported or not like CrystalDiskInfo[^442] but will not actually check if it is working. See [Appendix J: Manufacturer tools for Wiping HDD and SSD drives][Appendix J: Manufacturer tools for Wiping HDD and SSD drives:].
...
Follow this good tutorial: <https://www.glump.net/howto/desktop/enable-trim-on-an-external-ssd-on-linux> <sup>[[Archive.org]][583]</sup>

### ATA/NVMe Operations (Secure Erase/Sanitize):

**It is not "recommended". Please read the disclaimers here <https://ata.wiki.kernel.org/index.php/ATA_Secure_Erase>** <sup>[[Archive.org]][555]</sup> **and here <https://wiki.archlinux.org/index.php/Solid_state_drive/Memory_cell_clearing>** <sup>[[Archive.org]][554]</sup>
...
See also this tutorial <https://code.mendhak.com/securely-wipe-ssd/> <sup>[[Archive.org]][584]</sup>
...
According to Apple Documentation[^432], Trim is supported on APFS (asynchronously) and HFS+ (through period trim or first-aid).
...
If your disk supports it but it is not enabled in macOS. You could try issuing a "sudo trimforce enable" command from the Terminal and see if it enables Trim on your external SSD. And then again check the first aid command if it is not APFS (see this Tutorial for info <https://www.lifewire.com/enable-trim-for-ssd-in-os-x-yosemite-2260789> <sup>[[Archive.org]][585]</sup>)

If it does not work, I am not aware of any reliable method to enable TRIM besides the commercial utility Trim Enabler here <https://cindori.org/trimenabler/> <sup>[[Archive.org]][586]</sup> which claims support for external drives.
...
Download the latest Debian testing amd64 netinst ISO from <https://www.debian.org/CD/netinst/> <sup>[[Archive.org]][587]</sup>
...
-   Lenovo ThinkPads: <https://support.lenovo.com/be/en/solutions/migr-68369> <sup>[[Archive.org]][588]</sup>

-   HP (all): <https://support.hp.com/gb-en/document/c06204100> <sup>[[Archive.org]][589]</sup>

-   Dell (all): <https://www.dell.com/support/kbdoc/en-us/000146892/dell-data-wipe> <sup>[[Archive.org]][590]</sup>

-   Acer (Travelmate only): <https://us.answers.acer.com/app/answers/detail/a_id/41567/~/how-to-use-disk-sanitizer-on-acer-travelmate-notebooks> <sup>[[Archive.org]][591]</sup>
...
-   **Encrochat: <https://en.wikipedia.org/wiki/EncroChat>** <sup>[[Wikiless]][594]</sup> <sup>[[Archive.org]][595]</sup>

-   **Sky ECC: <https://en.wikipedia.org/wiki/Sky_ECC>** <sup>[[Wikiless]][596]</sup> <sup>[[Archive.org]][597]</sup>
...
There are three VPN companies recommended by privacytools.io (<https://privacytools.io/providers/vpn/> <sup>[[Archive.org]][598]</sup>) that accept cash payments: Mullvad, iVPN, and ProtonVPN.

In addition, I will also mention a newcomer to watch: Safing SPN <https://safing.io/> <sup>[[Archive.org]][599]</sup>) which (while still in the alpha stage at the time of this writing) which also accepts cash and has a very distinct new concept for a VPN which provides benefits similar to Tor Stream isolation with their "SPN"). Note that Safing SPN is not available on macOS at the moment. This possibility is "provisional" and at your own risk, but I think was worth mentioning.
...
-   Wearing gloves while manipulating anything to avoid leaving fingerprints[^447] and touch DNA[^448].
...
-   Do not lick the envelope or the stamps[^449] if you use them to avoid leaving DNA traces.
...
This does offer some disadvantages as Monero is not perfect as explained earlier in this guide and some global adversaries could maybe still track you. You will need to get Monero from an Exchange using the normal financial system and then pick a hosting (list here <https://www.getmonero.org/community/merchants/#exchanges> <sup>[[Archive.org]][600]</sup>)
...
There are plenty of tutorials on how to do this like this one <https://proprivacy.com/vpn/guides/create-your-own-vpn-server> <sup>[[Archive.org]][601]</sup>
...
-   (Windows/Linux/macOS) <https://linuxize.com/post/how-to-setup-ssh-socks-tunnel-for-private-browsing/> <sup>[[Archive.org]][602]</sup>

-   (Windows/Linux/macOS) <https://www.digitalocean.com/community/tutorials/how-to-route-web-traffic-securely-without-a-vpn-using-a-socks-tunnel> <sup>[[Archive.org]][603]</sup>

-   (Windows) <https://www.forwardproxy.com/2018/12/using-putty-to-setup-a-quick-socks-proxy/> <sup>[[Archive.org]][604]</sup>

-   (Linux/macOS) <https://ma.ttias.be/socks-proxy-linux-ssh-bypass-content-filters/> <sup>[[Archive.org]][605]</sup>
...
-   Download and install Putty from <https://www.putty.org/> <sup>[[Archive.org]][606]</sup>
...
In that case, my last resort recommendation is to connect safely **from a distance** to a Public Wi-Fi (See [Find some safe places with decent public Wi-Fi][Find some safe places with decent public Wi-Fi:]) using your laptop and Tails "unsafe browser". See <https://tails.boum.org/contribute/design/Unsafe_Browser/> <sup>[[Archive.org]][608]</sup>.
...
Consider reading this older but still relevant guide <https://archive.flossmanuals.net/bypassing-censorship/index.html> <sup>[[Archive.org]][609]</sup>
...
**I would recommend the AWUS036 series in the Alfa brand of adapters (see <https://www.alfa.com.tw/>** <sup>[[Archive.org]][611]</sup>**).** But you could also go with some other brands if you want such as the TP-Link TL-WN722 (see <https://www.tp-link.com/us/home-networking/usb-adapter/tl-wn722n/> <sup>[[Archive.org]][612]</sup>).

See this post for a comparison of various adapters: <https://www.wirelesshack.org/best-kali-linux-compatible-usb-adapter-dongles.html> <sup>[[Archive.org]][613]</sup> (Usually those antennas are used by Penetration Testers to probe Wi-Fis from a distance and are often discussed within the scope of the Kali Linux distribution).
...
**Do not forget tho that this will only delay a motivated adversary. Your signal can be triangulated easily by a motivated adversary in a matter of minutes once they reach the physical location of the Wi-Fi you're connecting to (for instance using a device such as AirCheck <https://www.youtube.com/watch?v=8FV2QZ1BPnw>** <sup>[[Invidious]][614]</sup>**, also see their other products here <https://www.netally.com/products/>** <sup>[[Archive.org]][615]</sup>**). These products can easily be deployed on mobile units (in a Car for instance) and pinpoint your location in a matter of minutes.**
...
-   Whonix Tutorial (should work with any VPN provider): <https://www.whonix.org/wiki/Tunnels/Connecting_to_a_VPN_before_Tor> <sup>[[Archive.org]][313]</sup> (use the Linux configurations below to get the necessary configuration files)

-   Windows Tutorials:

    -   Mullvad: <https://mullvad.net/en/help/install-mullvad-app-windows/> <sup>[[Archive.org]][616]</sup>

    -   iVPN: <https://www.ivpn.net/apps-windows> <sup>[[Archive.org]][617]</sup>

    -   Safing: <https://docs.safing.io/portmaster/install/windows> <sup>[[Archive.org]][618]</sup>

    -   ProtonVPN: <https://protonvpn.com/support/protonvpn-windows-vpn-application/> <sup>[[Archive.org]][619]</sup>

-   macOS:

    -   Mullvad: <https://mullvad.net/en/help/install-and-use-mullvad-app-macos/> <sup>[[Archive.org]][620]</sup>

    -   IVPN: <https://www.ivpn.net/apps-macos/> <sup>[[Archive.org]][621]</sup>

    -   Safing: Not available on macOS

    -   ProtonVPN: <https://protonvpn.com/support/protonvpn-mac-vpn-application/> <sup>[[Archive.org]][622]</sup>

-   Linux:

    -   Mullvad: <https://mullvad.net/en/help/install-mullvad-app-linux/> <sup>[[Archive.org]][623]</sup>

    -   iVPN: <https://www.ivpn.net/apps-linux/> <sup>[[Archive.org]][624]</sup>

    -   Safing: <https://docs.safing.io/portmaster/install/linux> <sup>[[Archive.org]][625]</sup>

    -   ProtonVPN: <https://protonvpn.com/support/linux-vpn-setup/> <sup>[[Archive.org]][626]</sup>
...
-   Windows:
    ...
    -   iVPN: <https://www.ivpn.net/knowledgebase/general/do-you-offer-a-kill-switch-or-vpn-firewall/> <sup>[[Archive.org]][627]</sup>

    -   ProtonVPN: <https://protonvpn.com/support/what-is-kill-switch/> <sup>[[Archive.org]][628]</sup>

    -   Mullvad: <https://mullvad.net/en/help/using-mullvad-vpn-app/#killswitch> <sup>[[Archive.org]][629]</sup>
...
-   macOS:
    ...
    -   ProtonVPN same as Windows with the client, the option should be in the provided VPN client <https://protonvpn.com/blog/macos-vpn-kill-switch/> <sup>[[Archive.org]][630]</sup>

-   Linux:

    -   Mullvad:

        -   <https://mullvad.net/en/help/wireguard-and-mullvad-vpn/> <sup>[[Archive.org]][631]</sup>

        -   <https://mullvad.net/en/help/linux-openvpn-installation/> <sup>[[Archive.org]][632]</sup>

    -   ProtonVPN: <https://github.com/ProtonVPN/linux-cli/blob/master/USAGE.md#kill-switch> <sup>[[Archive.org]][633]</sup>

    -   iVPN:

        -   <https://www.ivpn.net/knowledgebase/linux/linux-wireguard-kill-switch/> <sup>[[Archive.org]][634]</sup>

        -   <https://www.ivpn.net/knowledgebase/linux/linux-kill-switch-using-the-uncomplicated-firewall-ufw/> <sup>[[Archive.org]][635]</sup>

# Appendix S: Check your network for surveillance/censorship using OONI

So, what is OONI? OONI stands for Open Observatory of Network Interference and is a sub-project of the Tor Project[^276].
...
The problem is that your network providers will be able to see those tests and your attempts at connecting to various services if the network is monitored. The other issue is that there are solutions to prevent OONI from working properly[^450].
...
The probe can be found here: <https://ooni.org/install/> <sup>[[Archive.org]][636]</sup> for various platforms (iOS, Android, Windows, macOS, and Linux).

# Appendix T: Checking files for malware

## Integrity (if available):

Usually, integrity checks[^451] are done using hashes of files (usually stored within checksum files). Older files could use CRC[^452], more recently MD5[^453] but those present several weaknesses (CRC, MD5[^454]) that make them unreliable for file integrity checks (which does not mean they are not still widely used in other contexts).

This is because they do not prevent Collision[^455] well enough and could allow an adversary to create a similar but malicious file that would still produce in the same CRC or MD5 hash despite having different content.

For this reason, it is usually recommended to use SHA-based[^456] hashes and the most used is probably the SHA-2[^457] based SHA256 for verifying file integrity. SHA is much more resistant to collisions[^458] than CRC and MD5. And collisions with SHA256 or SHA512 are rare and hard to compute for an adversary.
...
Integrity is one thing. Authenticity is another thing. This is a process where you can verify some information is authentic and from the expected source. This is usually done by signing information (using GPG[^461] for instance) using public-key cryptography[^462].
...
-   Install GPG for your OS:
    ...
    -   Windows: gpg4win (<https://www.gpg4win.org/> <sup>[[Archive.org]][637]</sup>)

    -   macOS: GPGTools (<https://gpgtools.org/> <sup>[[Archive.org]][638]</sup>)
...
-   <https://support.torproject.org/tbb/how-to-verify-signature/> <sup>[[Archive.org]][639]</sup>

-   <https://tails.boum.org/install/vm-download/index.en.html> <sup>[[Archive.org]][640]</sup> (See Basic OpenPGP verification).

-   <https://www.whonix.org/wiki/Verify_the_Whonix_images> <sup>[[Archive.org]][641]</sup>
...
You might be asking yourself, what about Anti-Virus solutions? Well, no ... these are not perfect solutions against many modern malware and viruses using polymorphic code[^463]. But it does not mean they cannot help against less sophisticated and known attacks. It depends on how to use them as AV software can become an attack vector in itself.
...
Some will just argue against them broadly like Whonix[^464] but this topic is being discussed and disputed even at Whonix[^465] by other members of their community.
...
-   Yes, there are viruses and malware for Linux[^466]'[^467]'[^468]'[^469]'[^470]

-   Yes, there are viruses and malware for macOS[^478]'[^471][^472]'[^473][^474]
...
-   Do use Open-Source non-real-time offline Anti-Virus/Anti-Malware tools as an added measure to scan some files such as:
    ...
    -   Windows/Linux/macOS/Qubes OS: ClamAV (<https://www.clamav.net/> <sup>[[Archive.org]][642]</sup>)

    -   Linux/Qubes OS: RFXN Linux Malware Detect (<https://github.com/rfxn/linux-malware-detect> <sup>[[Archive.org]][643]</sup>)

    -   Linux/Qubes OS: Chkrootkit (<http://www.chkrootkit.org/> <sup>[[Archive.org]][644]</sup>)
...
-   You could also use online services for **non-sensitive files*** such as VirusTotal (<https://www.virustotal.com/gui/>) or Hybrid-analysis (<https://hybrid-analysis.com/>).
    ...
    -   You could also just check the VirusTotal database for the hash of your file if you don't want to send it over (see <https://developers.virustotal.com/v3.0/docs/search-by-hash> <sup>[[Archive.org]][645]</sup> (See the [Integrity (if available):] section again for guidance on how to generate hashes).

    -   Other tools are also available for non-sensitive files and a convenient list is right here: <https://github.com/rshipp/awesome-malware-analysis#online-scanners-and-sandboxes> <sup>[[Archive.org]][646]</sup>

* **Please be aware that while VirusTotal might seem very practical for scanning various files, their "privacy policy" is problematic (see <https://support.virustotal.com/hc/en-us/articles/115002168385-Privacy-Policy>** <sup>[[Archive.org]][647]</sup>**) and states:**
...
Again, regarding the PDFs of this guide and as explained in the README of my repository, you could check for anomalies using PDFID which you can download at <https://blog.didierstevens.com/programs/pdf-tools/> <sup>[[Archive.org]][648]</sup>:
...
-   **Qubes OS:** Consider using <https://github.com/QubesOS/qubes-app-linux-pdf-converter> <sup>[[Archive.org]][649]</sup> which will convert your PDF into a flattened image file. This should theoretically remove any malicious code in it. Note that this will also render the PDF formatting useless (such as links, headings, bookmarks, and references).

-   **(Deprecated) Linux/Qubes OS** (or possibly macOS through Homebrew or Windows through Cygwin): Consider not using <https://github.com/firstlookmedia/pdf-redact-tools> <sup>[[Archive.org]][650]</sup> which will also turn your PDF into a flattened image file. Again, this should theoretically remove any malicious code in it. Again, this will also render the PDF formatting useless (such as links, headings, bookmarks, and references). **Note that this tool is deprecated and relies on a library called "ImageMagick" which is known for several security issues**[^475]**. You should not use this tool even if it is recommended in some other guides.**

-   **Windows/Linux/Qubes/OS/macOS:** Consider using <https://github.com/firstlookmedia/dangerzone> <sup>[[Archive.org]][651]</sup> which was inspired by Qubes PDF Converted above and does the same but is well maintained and works on all OSes. This tool also works with Images, ODF files, and Office files (Warning: On Windows, this tool requires Docker-Desktop installed and this might (will) interfere with Virtualbox and other Virtualization software because it requires enabling Hyper-V. VirtualBox and Hyper-V do not play nice together[^476]. Consider installing this within a Linux VM for convenience instead of a Windows OS).

#### Other types of files:

Here are some various resources for this purpose where you will find what tool to use for what type:

-   **For Documents/Pictures:** Consider using <https://github.com/firstlookmedia/dangerzone> <sup>[[Archive.org]][651]</sup> which was inspired by Qubes PDF Converted above and does the same but is well maintained and works on all OSes. This tool also works with Images, ODF files, and Office files (Warning: On Windows, this tool requires Docker-Desktop installed and this might (will) interfere with Virtualbox and other Virtualization software because it requires enabling Hyper-V. VirtualBox and Hyper-V do not play nice together[^477]. Consider installing this within a Linux VM for convenience instead of a Windows OS).

-   **For Videos:** Be extremely careful, use an up-to-date player in a sandboxed environment. Remember <https://www.vice.com/en/article/v7gd9b/facebook-helped-fbi-hack-child-predator-buster-hernandez> <sup>[[Archive.org]][652]</sup>

-   This practical cheat sheet from SANS: <https://digital-forensics.sans.org/media/analyzing-malicious-document-files.pdf> <sup>[[Archive.org]][653]</sup> (warning, many of those tools might be harder to use on Windows and you might consider using them from a Linux OS such as Tails, Whonix Workstation, or a Linux distribution of your choice as explained later in this guide. There are also other guides out there[^478] that might be of use).

-   This GitHub repository with various resources on malware analysis: <https://github.com/rshipp/awesome-malware-analysis> <sup>[[Archive.org]][646]</sup>
...
There are plenty of methods you could use to bypass those restrictions locally. One of them would be to use portable apps[^479]. Those apps do not require installation on your system and can be run from a USB key or anywhere else.
...
**This is my recommended/preferred choice for a Browser within your guest VMs. This is not my recommended choice for a Browser within your Host OS where I strictly recommend Tor Browser as they recommend it themselves**[^480]**.**

Why Brave despite the controversies[^481]?
...
-   You will enjoy native ad-blocking where none is available in others by default without adding extensions[^482].

-   Performance is arguably better than Firefox[^483].

-   Brave is arguably better at fingerprinting resistance than others[^484].

-   Security of Chromium-based Browser is arguably better than Firefox[^485]'[^486]. Within the context of this guide, security should be privileged to prevent any vulnerability or exploit from gaining access to the VM.
...
-   Brave was found to be sending no identifiable telemetry compared to other browsers[^487].
...
-   Better Security than Firefox as it is Chromium-based[^488]'[^489].
...
-   Better Security than Firefox as it is Chromium-based[^490]'[^491].

-   Better Performance than Firefox.

-   The whole traffic will be router through Tor anyway.

-   Can benefit from additional security using Microsoft Defender Application Guard (MDAG)[^492]. Note that this feature cannot be enabled in a Virtualbox VM unfortunately.
...
-   In addition to being out of the Chromium world, it is also completely out of the Google world (despite the Mozilla Foundation being almost entirely funded by Google[^493]).
...
Security (especially sandboxing) of Firefox is arguably weaker than Chromium-based browsers[^494].
...
1.  <https://wiki.archlinux.org/title/Firefox/Privacy> <sup>[[Archive.org]][657]</sup> **(most recommended)**

2.  <https://www.privacytools.io/browsers/#about_config> <sup>[[Archive.org]][658]</sup>

3.  <https://proprivacy.com/privacy-service/guides/firefox-privacy-security-guide> <sup>[[Archive.org]][659]</sup>
...
-   <https://chrisx.xyz/blog/yet-another-firefox-hardening-guide/> <sup>[[Archive.org]][660]</sup>

-   <https://ebin.city/~werwolf/posts/firefox-hardening-guide/> <sup>[[Archive.org]][661]</sup>

# Appendix W: Virtualization

So, you might ask yourself, what is Virtualization[^495]?
...
In those cases, it might be necessary to use Tor bridges to connect to the Tor network (see Tor Documentation <https://2019.www.torproject.org/docs/bridges> <sup>[[Archive.org]][236]</sup> and Whonix Documentation <https://www.whonix.org/wiki/Bridges> <sup>[[Archive.org]][331]</sup>).

Bridges are special Tor entry nodes that are not listed on the Tor public directory. Some of those are running on people running the Snowflake Browser extension[^496] while others are running on various servers around the world. Most of those bridges are running some type of obfuscation method called obfs4[^497].

Here is the definition from the Tor Browser Manual[^498]: "obfs4 makes Tor traffic look random and prevents censors from finding bridges by Internet scanning. obfs4 bridges are less likely to be blocked than its predecessor, obfs3 bridges".
...
As per their definition from their manual[^499]: "meek transports make it look like you are browsing a major web site instead of using Tor. meek-azure makes it look like you are using a Microsoft web site". This is a type of "domain fronting" [^500].
...
First, you should proceed with the following checklist to make sure you cannot circumvent Tor Blocking (double-check) and try to use Tor Bridges (<https://bridges.torproject.org/> <sup>[[Archive.org]][563]</sup>):
...
Finally, you could also request a bridge request by e-mail to <bridges@torproject.org> with the subject empty and the body being: "get transport obfs4" or "get transport meek". There is some limitation with this method tho as it is only available from a Gmail e-mail address or a Riseup.net (<https://riseup.net/> <sup>[[Tor Mirror]][383]</sup>) e-mail address.
...
-   Download and install Tor Browser according to the instructions from <https://www.torproject.org/download/> <sup>[[Archive.org]][664]</sup>
...
-   At this point, still before connecting, you should click the little shield Icon (upper right, next to the Address bar) and select your Security level (see <https://tb-manual.torproject.org/security-settings/> <sup>[[Archive.org]][232]</sup> for details). Basically, there are three.
...
**Bitcoin in itself is not anonymous at all (Remember [Your Cryptocurrencies transactions][Your Cryptocurrencies transactions:]) and you should never ever purchase Bitcoin from an exchange and then use these directly for purchasing services anonymously. This will not work, and you can be traced easily.**
...
-   You might want to consider the use of Wasabi (<https://wasabiwallet.io/> <sup>[[Archive.org]][668]</sup>) for your BTC transactions using their "CoinJoin feature"[^502] to further cover your tracks. This would mean swapping your Monero for BTC to a Wasabi Wallet instead of a normal Wallet. And then using that Wasabi Wallet for your BTC transactions using their CoinJoin feature.
...
-   **Njalla <https://njal.la/> (my personal favorite but quite expensive, recommended by Privacytools.Io (<https://privacytools.io/providers/hosting/>** <sup>[[Archive.org]][669]</sup>**).**
...
-   Tor Project: <https://community.torproject.org/relay/community-resources/good-bad-isps/> <sup>[[Archive.org]][670]</sup>

-   Privacytools.io: <https://privacytools.io/providers/hosting/> <sup>[[Archive.org]][669]</sup>

Lastly, you could pick one (at your own risk) from the list here that does accept Monero: <https://www.getmonero.org/community/merchants/#hosting> <sup>[[Archive.org]][600]</sup>
...
My opinion (and the one of many[^503]'[^504]'[^505]'[^506]'[^507]'[^508]) is that passphrases are generally better than passwords. So instead of thinking of better passwords, forget them altogether and use passphrases instead (when possible). Or just use a password manager with very long passwords (such as KeePassXC, the preferred password manager in this guide).

The well-known shown-below XKCD <https://xkcd.com/936/> <sup>[[Archive.org]][671]</sup> is still valid despite some people disputing it (See <https://www.explainxkcd.com/wiki/index.php/936:_Password_Strength> <sup>[[Archive.org]][672]</sup>). Yes, it is quite old now and is a little bit outdated and might be misinterpreted. But generally, it is still valid and a good argument for using passphrases instead of passwords.

![][673]

(Illustration by Randall Munroe, xkcd.com, licensed under CC BY-NC 2.5)

Here are some recommendations (based on Wikipedia[^509]):
...
Watch this insightful video by Computerphile: <https://www.youtube.com/watch?v=3NjQ9b3pgIg> <sup>[[Invidious]][674]</sup>
...
I will not go into too many details. Just pick one from the privacytools.io list: <https://privacytools.io/providers/search-engines> <sup>[[Archive.org]][675]</sup> <sup>[[Tor Mirror]][676]</sup>
...
-   The OxyMonster case (<https://arstechnica.com/tech-policy/2018/06/dark-web-vendor-oxymonster-turns-out-to-be-a-frenchman-with-luscious-beard/> <sup>[[Archive.org]][677]</sup>):
...
-   <https://wikipedia.org/wiki/Forensic_linguistics> <sup>[[Wikiless]][678]</sup> <sup>[[Archive.org]][679]</sup>: Gives a brief rundown of the basics of forensic linguistics, not too informative.

-   <https://wikipedia.org/wiki/Writeprint> <sup>[[Wikiless]][680]</sup> <sup>[[Archive.org]][681]</sup>: Gives a brief and informative rundown of forensic linguistics applied to internet investigations.

-   <https://wikipedia.org/wiki/Stylometry> <sup>[[Wikiless]][682]</sup> <sup>[[Archive.org]][683]</sup>: Gives a brief overview of Stylometry.

-   <https://wikipedia.org/wiki/Content_similarity_detection> <sup>[[Wikiless]][684]</sup> <sup>[[Archive.org]][685]</sup>: I would recommend reading this, quite informative.

-   <https://wikipedia.org/wiki/Author_profiling> <sup>[[Wikiless]][686]</sup> <sup>[[Archive.org]][687]</sup>: Read through this as well if you are interested in this topic.

-   <https://wikipedia.org/wiki/Native-language_identification> <sup>[[Wikiless]][688]</sup> <sup>[[Archive.org]][689]</sup>: This is less important if you use a translator, but if you do not use a translator to communicate on forums that are not in your native language, consider giving this a quick read through.

-   <https://wikipedia.org/wiki/Computational_linguistics> <sup>[[Wikiless]][690]</sup> <sup>[[Archive.org]][691]</sup>: Only read through this if this topic is interesting to you.

-   <https://regmedia.co.uk/2017/09/27/gal_vallerius.pdf> <sup>[[Archive.org]][692]</sup>: Explains how authorities used forensic linguistics to help arrest OxyMonster (pages 13 -- 14).

-   <https://wikipedia.org/wiki/Ted_Kaczynski#After_publication> <sup>[[Wikiless]][693]</sup> <sup>[[Archive.org]][694]</sup>: May have an IQ of 167, but he was caught primarily based on forensic linguistics.

-   <https://i.blackhat.com/USA-19/Wednesday/us-19-Wixey-Im-Unique-Just-Like-You-Human-Side-Channels-And-Their-Implications-For-Security-And-Privacy.pdf> <sup>[[Archive.org]][695]</sup>: Explains how your writing style can be used to track you, I highly recommend reading through these slides, or watching the accompanying presentation on YouTube.

-   <https://media.defcon.org/DEF%20CON%2026/DEF%20CON%2026%20presentations/DEFCON-26-Matt-Wixey-Betrayed-by-the-Keyboard-Updated.pdf> <sup>[[Archive.org]][696]</sup>: Explains how your writing style can be used to track you, I highly recommend reading through these slides, or watching the accompanying presentation on YouTube, this is quite similar to the last presentation.

-   <https://i.blackhat.com/us-18/Wed-August-8/us-18-Wixey-Every-ROSE-Has-Its-Thorn-The-Dark-Art-Of-Remote-Online-Social-Engineering.pdf> <sup>[[Archive.org]][697]</sup>: This goes over how to potentially spot deception through the internet, and presents a checklist to see how trustworthy someone is. I would advise reading the slides or watching the presentation on YouTube.
...
-   For Reddit, use a LibReddit instance (<https://github.com/spikecodes/libreddit> <sup>[[Archive.org]][698]</sup>)

-   For Maps, consider using <https://www.openstreetmap.org>

-   For Translation, consider using a SimplyTranslate (<https://git.sr.ht/~metalune/simplytranslate_web> <sup>[[Archive.org]][699]</sup>) at <https://translate.metalune.xyz/>
...
-   Original: [https://anonymousplanet.org][] <sup>[[Archive.org]][700]</sup> <sup>[[Archive.today]][701]</sup>
...
-   Light Theme: <https://anonymousplanet.org/guide.pdf> <sup>[[Mirror]][702]</sup> <sup>[[Archive.org]][703]</sup> <sup>[[Tor Mirror]][704]</sup>

-   Dark Theme: <https://anonymousplanet.org/guide-dark.pdf> <sup>[[Mirror]][705]</sup> <sup>[[Archive.org]][706]</sup> <sup>[[Tor Mirror]][707]</sup>

-   Both at CryptPad.fr <https://cryptpad.fr/drive/#/2/drive/view/Ughm9CjQJCwB8BIppdtvj5zy4PyE-8Gxn11x9zaqJLI/>

OpenDocument Text (ODT) version at: <https://anonymousplanet.org/guide.odt> <sup>[[Mirror]][708]</sup> <sup>[[Archive.org]][709]</sup> <sup>[[Tor Mirror]][710]</sup>
...
If you want to compare the older version of the ODT format with a newer version, use the LibreWriter compare features as explained here: <https://help.libreoffice.org/7.1/en-US/text/shared/guide/redlining_doccompare.html> <sup>[[Archive.org]][711]</sup>
...
The anonymity of Monero depends on its crypto algorithms. If you do use Monero from a KYC Exchange. You can be almost certain that you are safe today. But you might not be in the long-term future if Monero algorithms are ever broken[^510] (think Quantum Computing). Do keep in mind that KYC regulations might force operators (such as Crypto Exchanges) to keep your financial records for up to 10 years and that you, therefore, need Monero algorithms to not be broken for the next 10 years as well. **Use at your own risk, sending cash payments to providers accepting cash (through the postal service) is always a better solution if/when possible.**

You may want to watch this insightful video for more details: [**https://www.youtube.com/watch?v=j02QoI4ZlnU**][] <sup>[[Invidious]][712]</sup>

Also please consider reading: **<https://github.com/monero-project/monero/blob/master/docs/ANONYMITY_NETWORKS.md#privacy-limitations>** <sup>[[Archive.org]][713]</sup>

---

# References:

[^1]: English translation of German Telemedia Act <https://www.huntonprivacyblog.com/wp-content/uploads/sites/28/2016/02/Telemedia_Act__TMA_.pdf> <sup>[[Archive.org]][714]</sup>. Section 13, Article 6, "The service provider must enable the use of Telemedia and payment for them to occur anonymously or via a pseudonym where this is technically possible and reasonable. The recipient of the service is to be informed about this possibility. ".

[^2]: Wikipedia, Real-Name System Germany <https://en.wikipedia.org/wiki/Real-name_system#Germany> <sup>[[Wikiless]][402]</sup> <sup>[[Archive.org]][403]</sup>

[^3]: Wikipedia, Don't be evil <https://en.wikipedia.org/wiki/Don%27t_be_evil> <sup>[[Wikiless]][715]</sup> <sup>[[Archive.org]][716]</sup>

[^4]: YouTube, <https://www.youtube.com/watch?v=6DGNZnfKYnU> <sup>[[Invidious]][717]</sup>

[^5]: Wikipedia, OSINT <https://en.wikipedia.org/wiki/Open-source_intelligence> <sup>[[Wikiless]][718]</sup> <sup>[[Archive.org]][719]</sup>

[^6]: YouTube Internet Historian Playlist, HWNDU <https://www.youtube.com/playlist?list=PLna1KTNJu3y09Tu70U6yPn28sekaNhOMY> <sup>[[Invidious]][720]</sup>

[^7]: Wikipedia, 4chan <https://en.wikipedia.org/wiki/4chan> <sup>[[Wikiless]][721]</sup> <sup>[[Archive.org]][722]</sup>

[^8]: PIA, See this good article on the matter <https://www.privateinternetaccess.com/blog/how-does-privacy-differ-from-anonymity-and-why-are-both-important/> <sup>[[Archive.org]][723]</sup> (disclaimer: this is not an endorsement or recommendation for this commercial service).
...
[^10]: This World of Ours, James Mickens <https://scholar.harvard.edu/files/mickens/files/thisworldofours.pdf> <sup>[[Archive.org]][724]</sup>

[^11]: XKCD, Security <https://xkcd.com/538/> <sup>[[Archive.org]][725]</sup>

[^12]: Wikipedia, Threat Model <https://en.wikipedia.org/wiki/Threat_model> <sup>[[Wikiless]][726]</sup> <sup>[[Archive.org]][727]</sup>

[^13]: Bellingcat <https://www.bellingcat.com/> <sup>[[Archive.org]][728]</sup>

[^14]: Wikipedia, Doxing <https://en.wikipedia.org/wiki/Doxing> <sup>[[Wikiless]][729]</sup> <sup>[[Archive.org]][730]</sup>

[^15]: YouTube, Internet Historian, The Bikelock Fugitive of Berkeley <https://www.youtube.com/watch?v=muoR8Td44UE> <sup>[[Invidious]][731]</sup>

[^16]: BBC News, Tor Mirror <https://www.bbc.com/news/technology-50150981> <sup>[[Archive.org]][732]</sup>

[^17]: GitHub, Real World Onion websites <https://github.com/alecmuffett/real-world-onion-sites> <sup>[[Archive.org]][733]</sup>

[^18]: Tor Project, Who Uses Tor <https://2019.www.torproject.org/about/torusers.html.en> <sup>[[Archive.org]][734]</sup>

[^19]: Whonix Documentation, The importance of Anonymity <https://www.whonix.org/wiki/Anonymity> <sup>[[Archive.org]][735]</sup>

[^20]: Geek Feminism, <https://geekfeminism.wikia.org/wiki/Who_is_harmed_by_a_%22Real_Names%22_policy%3F> <sup>[[Archive.org]][736]</sup>

[^21]: Tor Project, Tor Users <https://2019.www.torproject.org/about/torusers.html.en> <sup>[[Archive.org]][734]</sup>

[^22]: PrivacyHub, Internet Privacy in the Age of Surveillance <https://www.cyberghostvpn.com/privacyhub/internet-privacy-surveillance/> <sup>[[Archive.org]][737]</sup>

[^23]: PIA Blog, 50 Key Stats About Freedom of the Internet Around the World <https://www.privateinternetaccess.com/blog/internet-freedom-around-the-world-in-50-stats/> <sup>[[Archive.org]][738]</sup>

[^24]: Wikipedia, IANAL <https://en.wikipedia.org/wiki/IANAL> <sup>[[Wikiless]][739]</sup> <sup>[[Archive.org]][740]</sup>

[^25]: Wikipedia, Trust but verify <https://en.wikipedia.org/wiki/Trust,_but_verify> <sup>[[Wikiless]][741]</sup> <sup>[[Archive.org]][742]</sup>

[^26]: Wikipedia, IP Address, <https://en.wikipedia.org/wiki/IP_address> <sup>[[Wikiless]][743]</sup> <sup>[[Archive.org]][744]</sup>

[^27]: Wikipedia; Data Retention <https://en.wikipedia.org/wiki/Data_retention> <sup>[[Wikiless]][745]</sup> <sup>[[Archive.org]][746]</sup>

[^28]: Wikipedia, Tor Anonymity Network <https://en.wikipedia.org/wiki/Tor_(anonymity_network)> <sup>[[Wikiless]][747]</sup> <sup>[[Archive.org]][748]</sup>

[^29]: Wikipedia, VPN <https://en.wikipedia.org/wiki/Virtual_private_network> <sup>[[Wikiless]][749]</sup> <sup>[[Archive.org]][750]</sup>

[^30]: Wikipedia, DNS <https://en.wikipedia.org/wiki/Domain_Name_System> <sup>[[Wikiless]][751]</sup> <sup>[[Archive.org]][752]</sup>

[^31]: Wikipedia, DNS Blocking <https://en.wikipedia.org/wiki/DNS_blocking> <sup>[[Wikiless]][753]</sup> <sup>[[Archive.org]][754]</sup>

[^32]: CensoredPlanet <https://censoredplanet.org/> <sup>[[Archive.org]][218]</sup>

[^33]: ArXiv, Characterizing Smart Home IoT Traffic in the Wild <https://arxiv.org/pdf/2001.08288.pdf> <sup>[[Archive.org]][755]</sup>

[^34]: Labzilla.io, Your Smart TV is probably ignoring your Pi-Hole <https://labzilla.io/blog/force-dns-pihole> <sup>[[Archive.org]][756]</sup>

[^35]: Wikipedia, DNS over HTTPS: <https://en.wikipedia.org/wiki/DNS_over_HTTPS> <sup>[[Wikiless]][757]</sup> <sup>[[Archive.org]][758]</sup>

[^36]: Wikipedia, DNS over TLS, <https://en.wikipedia.org/wiki/DNS_over_TLS> <sup>[[Wikiless]][759]</sup> <sup>[[Archive.org]][760]</sup>

[^37]: Wikipedia, Pi-Hole <https://en.wikipedia.org/wiki/Pi-hole> <sup>[[Wikiless]][761]</sup> <sup>[[Archive.org]][762]</sup>

[^38]: Wikipedia, SNI <https://en.wikipedia.org/wiki/Server_Name_Indication> <sup>[[Wikiless]][763]</sup> <sup>[[Archive.org]][764]</sup>

[^39]: Wikipedia, ECH, <https://en.wikipedia.org/wiki/Server_Name_Indication#Encrypted_Client_Hello> <sup>[[Wikiless]][763]</sup> <sup>[[Archive.org]][764]</sup>

[^40]: Wikipedia, eSNI <https://en.wikipedia.org/wiki/Server_Name_Indication#Encrypted_Client_Hello> <sup>[[Wikiless]][763]</sup> <sup>[[Archive.org]][764]</sup>

[^41]: Usenix.org, On the Importance of Encrypted-SNI (ESNI) to Censorship Circumvention <https://www.usenix.org/system/files/foci19-paper_chai_0.pdf> <sup>[[Archive.org]][765]</sup>

[^42]: Wikipedia, CDN <https://en.wikipedia.org/wiki/Content_delivery_network> <sup>[[Wikiless]][766]</sup> <sup>[[Archive.org]][767]</sup>

[^43]: Cloudflare, Good-bye ESNI, hello ECH! <https://blog.cloudflare.com/encrypted-client-hello/> <sup>[[Archive.org]][768]</sup>

[^44]: ZDNET, Russia wants to ban the use of secure protocols such as TLS 1.3, DoH, DoT, ESNI <https://www.zdnet.com/article/russia-wants-to-ban-the-use-of-secure-protocols-such-as-tls-1-3-doh-dot-esni/> <sup>[[Archive.org]][769]</sup>

[^45]: ZDNET, China is now blocking all encrypted HTTPS traffic that uses TLS 1.3 and ESNI <https://www.zdnet.com/article/china-is-now-blocking-all-encrypted-https-traffic-using-tls-1-3-and-esni/> <sup>[[Archive.org]][770]</sup>

[^46]: Wikipedia, OCSP <https://en.wikipedia.org/wiki/Online_Certificate_Status_Protocol> <sup>[[Wikiless]][771]</sup> <sup>[[Archive.org]][772]</sup>

[^47]: Madaidans Insecurities, Why encrypted DNS is ineffective <https://madaidans-insecurities.github.io/encrypted-dns.html> <sup>[[Archive.org]][773]</sup>

[^48]: Wikipedia, OCSP Stapling <https://en.wikipedia.org/wiki/OCSP_stapling> <sup>[[Wikiless]][774]</sup> <sup>[[Archive.org]][775]</sup>

[^49]: Chromium Documentation, CRLSets <https://dev.chromium.org/Home/chromium-security/crlsets> <sup>[[Archive.org]][776]</sup>

[^50]: ZDNet, Chrome does certificate revocation better <https://www.zdnet.com/article/chrome-does-certificate-revocation-better/> <sup>[[Archive.org]][777]</sup>

[^51]: KUL, Encrypted DNS=⇒Privacy? A Traffic Analysis Perspective <https://www.esat.kuleuven.be/cosic/publications/article-3153.pdf> <sup>[[Archive.org]][778]</sup>

[^52]: ResearhGate, Oblivious DNS: Practical Privacy for DNS Queries <https://www.researchgate.net/publication/332893422_Oblivious_DNS_Practical_Privacy_for_DNS_Queries> <sup>[[Archive.org]][779]</sup>

[^53]: Nymity.ch, The Effect of DNS on Tor's Anonymity <https://nymity.ch/tor-dns/> <sup>[[Archive.org]][780]</sup>

[^54]: Wikipedia, RFID <https://en.wikipedia.org/wiki/Radio-frequency_identification> <sup>[[Wikiless]][62]</sup> <sup>[[Archive.org]][63]</sup>

[^55]: Wikipedia, NFC <https://en.wikipedia.org/wiki/Near-field_communication> <sup>[[Wikiless]][781]</sup> <sup>[[Archive.org]][782]</sup>

[^56]: Samsonite Online Shop, RFID accessories, <https://shop.samsonite.com/accessories/rfid-accessories/> <sup>[[Archive.org]][783]</sup>

[^57]: Google Android Help, Android Location Services <https://support.google.com/accounts/answer/3467281?hl=en> <sup>[[Archive.org]][784]</sup>

[^58]: Apple Support, Location Services and Privacy <https://support.apple.com/en-us/HT207056> <sup>[[Archive.org]][785]</sup>

[^59]: State University of New York, Towards 3D Human Pose Construction Using Wi-Fi <https://cse.buffalo.edu/~lusu/papers/MobiCom2020.pdf> <sup>[[Archive.org]][786]</sup>

[^60]: Digi.Ninja, Jasager <https://digi.ninja/jasager/> <sup>[[Archive.org]][787]</sup>

[^61]: Hak5 Shop, Wi-Fi Pineapple <https://shop.hak5.org/products/wifi-pineapple> <sup>[[Archive.org]][788]</sup>

[^62]: Wikipedia, Deautentication Attack <https://en.wikipedia.org/wiki/Wi-Fi_deauthentication_attack> <sup>[[Wikiless]][789]</sup> <sup>[[Archive.org]][790]</sup>

[^63]: Wikipedia, Capture Portal <https://en.wikipedia.org/wiki/Captive_portal> <sup>[[Wikiless]][791]</sup> <sup>[[Archive.org]][792]</sup>

[^64]: HackerFactor Blog, Deanonymizing Tor Circuits <https://www.hackerfactor.com/blog/index.php?/archives/868-Deanonymizing-Tor-Circuits.html> <sup>[[Archive.org]][793]</sup>

[^65]: KU Leuven, Website Fingerprinting through Deep Learning <https://distrinet.cs.kuleuven.be/software/tor-wf-dl/> <sup>[[Archive.org]][794]</sup>

[^66]: DailyDot, How Tor helped catch the Harvard bomb threat suspect <https://www.dailydot.com/unclick/tor-harvard-bomb-suspect/> <sup>[[Archive.org]][795]</sup>

[^67]: ArsTechnica, How the NSA can break trillions of encrypted Web and VPN connections <https://arstechnica.com/information-technology/2015/10/how-the-nsa-can-break-trillions-of-encrypted-web-and-vpn-connections/> <sup>[[Archive.org]][796]</sup>

[^68]: ArsTechnica, Does Tor provide more benefit or harm? New paper says it depends <https://arstechnica.com/gadgets/2020/11/does-tor-provide-more-benefit-or-harm-new-paper-says-it-depends/> <sup>[[Archive.org]][797]</sup>

[^69]: ResearchGate, The potential harms of the Tor anonymity network cluster disproportionately in free countries <https://www.pnas.org/content/early/2020/11/24/2011893117> <sup>[[Archive.org]][798]</sup>

[^70]: CryptoEngineering, How does Apple (privately) find your offline devices? <https://blog.cryptographyengineering.com/2019/06/05/how-does-apple-privately-find-your-offline-devices/> <sup>[[Archive.org]][799]</sup>

[^71]: Apple Support <https://support.apple.com/en-us/HT210515> <sup>[[Archive.org]][800]</sup>

[^72]: XDA, Samsung's Find My Mobile app can locate Galaxy devices even when they're offline <https://www.xda-developers.com/samsung-find-my-mobile-app-locate-galaxy-devices-offline/> <sup>[[Archive.org]][801]</sup>

[^73]: Apple Support, If your Mac is lost or stolen <https://support.apple.com/en-us/HT204756> <sup>[[Archive.org]][802]</sup>

[^74]: Wikipedia, BLE <https://en.wikipedia.org/wiki/Bluetooth_Low_Energy> <sup>[[Wikiless]][803]</sup> <sup>[[Archive.org]][804]</sup>

[^75]: Cryptography Engineering Blog, How does Apple (privately) find your offline devices? <https://blog.cryptographyengineering.com/2019/06/05/how-does-apple-privately-find-your-offline-devices/> <sup>[[Archive.org]][799]</sup>

[^76]: Wikipedia, IMEI <https://en.wikipedia.org/wiki/International_Mobile_Equipment_Identity> <sup>[[Wikiless]][805]</sup> <sup>[[Archive.org]][806]</sup>

[^77]: Wikipedia, IMSI <https://en.wikipedia.org/wiki/International_mobile_subscriber_identity> <sup>[[Wikiless]][807]</sup> <sup>[[Archive.org]][808]</sup>

[^78]: Android Documentation, Device Identifiers <https://source.android.com/devices/tech/config/device-identifiers> <sup>[[Archive.org]][809]</sup>

[^79]: Google Privacy Policy, Look for IMEI <https://policies.google.com/privacy/embedded?hl=en-US> <sup>[[Archive.org]][810]</sup>

[^80]: Wikipedia, IMEI and the Law <https://en.wikipedia.org/wiki/International_Mobile_Equipment_Identity#IMEI_and_the_law> <sup>[[Wikiless]][805]</sup> <sup>[[Archive.org]][806]</sup>

[^81]: Bellingcat, The GRU Globetrotters: Mission London <https://www.bellingcat.com/news/uk-and-europe/2019/06/28/the-gru-globetrotters-mission-london/> <sup>[[Archive.org]][811]</sup>

[^82]: Bellingcat,"V" For "Vympel": FSB's Secretive Department "V" Behind Assassination Of Georgian Asylum Seeker In Germany <https://www.bellingcat.com/news/uk-and-europe/2020/02/17/v-like-vympel-fsbs-secretive-department-v-behind-assassination-of-zelimkhan-khangoshvili/> <sup>[[Archive.org]][812]</sup>

[^83]: Wikipedia, CCTV <https://en.wikipedia.org/wiki/Closed-circuit_television> <sup>[[Wikiless]][813]</sup> <sup>[[Archive.org]][814]</sup>

[^84]: Apple, Transparency Report, Device Requests <https://www.apple.com/legal/transparency/device-requests.html> <sup>[[Archive.org]][815]</sup>

[^85]: The Intercept, How Cops Can Secretly Track Your Phone <https://theintercept.com/2020/07/31/protests-surveillance-stingrays-dirtboxes-phone-tracking/> <sup>[[Tor Mirror]][816]</sup> <sup>[[Archive.org]][817]</sup>

[^86]: Wikipedia, IMSI Catcher <https://en.wikipedia.org/wiki/IMSI-catcher> <sup>[[Wikiless]][818]</sup> <sup>[[Archive.org]][819]</sup>

[^87]: Wikipedia, Stingray <https://en.wikipedia.org/wiki/Stingray_phone_tracker> <sup>[[Wikiless]][820]</sup> <sup>[[Archive.org]][821]</sup>

[^88]: Gizmodo, Cops Turn to Canadian Phone-Tracking Firm After Infamous 'Stingrays' Become 'Obsolete' <https://gizmodo.com/american-cops-turns-to-canadian-phone-tracking-firm-aft-1845442778> <sup>[[Archive.org]][822]</sup>

[^89]: Wikipedia, MITM <https://en.wikipedia.org/wiki/Man-in-the-middle_attack> <sup>[[Wikiless]][823]</sup> <sup>[[Archive.org]][824]</sup>

[^90]: Purism, Librem 5 <https://shop.puri.sm/shop/librem-5/> <sup>[[Archive.org]][825]</sup>

[^91]: Wikipedia, MAC Address <https://en.wikipedia.org/wiki/MAC_address> <sup>[[Wikiless]][826]</sup> <sup>[[Archive.org]][827]</sup>

[^92]: Acyclica Road Trend Product Sheet, <https://amsignalinc.com/data-sheets/Acyclica/Acyclica-RoadTrend-Product-Sheet.pdf> <sup>[[Archive.org]][828]</sup>

[^93]: ResearchGate, Tracking Anonymized Bluetooth Devices <https://www.researchgate.net/publication/334590931_Tracking_Anonymized_Bluetooth_Devices/fulltext/5d3308db92851cd04675a469/Tracking-Anonymized-Bluetooth-Devices.pdf> <sup>[[Archive.org]][829]</sup>

[^94]: Wikipedia, CPU <https://en.wikipedia.org/wiki/Central_processing_unit> <sup>[[Wikiless]][830]</sup> <sup>[[Archive.org]][831]</sup>

[^95]: Wikipedia, Intel Management Engine <https://en.wikipedia.org/wiki/Intel_Management_Engine> <sup>[[Wikiless]][832]</sup> <sup>[[Archive.org]][833]</sup>

[^96]: Wikipedia, AMD Platform Security Processor <https://en.wikipedia.org/wiki/AMD_Platform_Security_Processor> <sup>[[Wikiless]][834]</sup> <sup>[[Archive.org]][835]</sup>

[^97]: Wikipedia, IME, Security Vulnerabilities <https://en.wikipedia.org/wiki/Intel_Management_Engine#Security_vulnerabilities> <sup>[[Wikiless]][832]</sup> <sup>[[Archive.org]][833]</sup>

[^98]: Wikipedia, IME, Assertions that ME is a backdoor <https://en.wikipedia.org/wiki/Intel_Management_Engine#Assertions_that_ME_is_a_backdoor> <sup>[[Wikiless]][832]</sup> <sup>[[Archive.org]][833]</sup>

[^99]: Wikipedia, IME, Disabling the ME <https://en.wikipedia.org/wiki/Intel_Management_Engine#Disabling_the_ME> <sup>[[Wikiless]][832]</sup> <sup>[[Archive.org]][833]</sup>

[^100]: Libreboot, <https://libreboot.org/> <sup>[[Archive.org]][836]</sup>

[^101]: Apple, Differential Privacy White Paper <https://www.apple.com/privacy/docs/Differential_Privacy_Overview.pdf> <sup>[[Archive.org]][837]</sup>

[^102]: Wikipedia, Differential Privacy <https://en.wikipedia.org/wiki/Differential_privacy> <sup>[[Wikiless]][838]</sup> <sup>[[Archive.org]][839]</sup>

[^103]: Continuing Ed, The All-Seeing "i": Apple Just Declared War on Your Privacy <https://edwardsnowden.substack.com/p/all-seeing-i> <sup>[[Archive.org]][840]</sup>

[^104]: Trinity College Dublin, Mobile Handset Privacy: Measuring The Data iOS and Android Send to Apple And Google <https://www.scss.tcd.ie/doug.leith/apple_google.pdf> <sup>[[Archive.org]][85]</sup>

[^105]: Reuters, Exclusive: Apple dropped plan for encrypting backups after FBI complained -- sources <https://www.reuters.com/article/us-apple-fbi-icloud-exclusive-idUSKBN1ZK1CT> <sup>[[Archive.org]][841]</sup>

[^106]: ZDnet, I asked Apple for all my data. Here's what was sent back <https://www.zdnet.com/article/apple-data-collection-stored-request/> <sup>[[Archive.org]][842]</sup>

[^107]: De Correspondent, Here's how we found the names and addresses of soldiers and secret agents using a simple fitness app <https://decorrespondent.nl/8481/heres-how-we-found-the-names-and-addresses-of-soldiers-and-secret-agents-using-a-simple-fitness-app/412999257-6756ba27> <sup>[[Archive.org]][843]</sup>

[^108]: Wired, The Strava Heat Map and the End of Secrets <https://www.wired.com/story/strava-heat-map-military-bases-fitness-trackers-privacy/> <sup>[[Archive.org]][844]</sup>

[^109]: Bellingcat, How to Use and Interpret Data from Strava's Activity Map <https://www.bellingcat.com/resources/how-tos/2018/01/29/strava-interpretation-guide/> <sup>[[Archive.org]][845]</sup>

[^110]: The Guardian, Fitness tracking app Strava gives away location of secret US army bases <https://www.theguardian.com/world/2018/jan/28/fitness-tracking-app-gives-away-location-of-secret-us-army-bases> <sup>[[Archive.org]][846]</sup>

[^111]: Telegraph, Running app reveals locations of secret service agents in MI6 and GCHQ <https://www.telegraph.co.uk/technology/2018/07/08/running-app-exposes-mi6-gchq-workers-whereabouts/> <sup>[[Archive.org]][847]</sup>

[^112]: Washington Post, Alexa has been eavesdropping on you this whole time <https://www.washingtonpost.com/technology/2019/05/06/alexa-has-been-eavesdropping-you-this-whole-time/?itid=lk_interstitial_manual_59> <sup>[[Archive.org]][848]</sup>

[^113]: Washington Post, What does your car know about you? We hacked a Chevy to find out <https://www.washingtonpost.com/technology/2019/12/17/what-does-your-car-know-about-you-we-hacked-chevy-find-out/> <sup>[[Archive.org]][849]</sup>

[^114]: Using Metadata to find Paul Revere (<https://kieranhealy.org/blog/archives/2013/06/09/using-metadata-to-find-paul-revere/> <sup>[[Archive.org]][850]</sup>)

[^115]: Wikipedia, Google SensorVault, <https://en.wikipedia.org/wiki/Sensorvault> <sup>[[Wikiless]][851]</sup> <sup>[[Archive.org]][852]</sup>

[^116]: NRKBeta, My Phone Was Spying on Me, so I Tracked Down the Surveillants <https://nrkbeta.no/2020/12/03/my-phone-was-spying-on-me-so-i-tracked-down-the-surveillants/> <sup>[[Archive.org]][853]</sup>

[^117]: New York Times <https://www.nytimes.com/interactive/2019/12/19/opinion/location-tracking-cell-phone.html> <sup>[[Archive.org]][854]</sup>

[^118]: Sophos, Google data puts innocent man at the scene of a crime <https://nakedsecurity.sophos.com/2020/03/10/google-data-puts-innocent-man-at-the-scene-of-a-crime/> <sup>[[Archive.org]][855]</sup>

[^119]: Wikipedia, Geofence Warrant <https://en.wikipedia.org/wiki/Geo-fence_warrant> <sup>[[Wikiless]][856]</sup> <sup>[[Archive.org]][857]</sup>

[^120]: Vice.com, Military Unit That Conducts Drone Strikes Bought Location Data From Ordinary Apps <https://www.vice.com/en/article/y3g97x/location-data-apps-drone-strikes-iowa-national-guard> <sup>[[Archive.org]][858]</sup>

[^121]: TechCrunch, Google says geofence warrants make up one-quarter of all US demands <https://techcrunch.com/2021/08/19/google-geofence-warrants/> <sup>[[Archive.org]][859]</sup>

[^122]: TechDirt, Google Report Shows 'Reverse Warrants' Are Swiftly Becoming Law Enforcement's Go-To Investigative Tool <https://www.techdirt.com/articles/20210821/10494847401/google-report-shows-reverse-warrants-are-swiftly-becoming-law-enforcements-go-to-investigative-tool.shtml> <sup>[[Archive.org]][860]</sup>

[^123]: Wikipedia, Room 641A <https://en.wikipedia.org/wiki/Room_641A> <sup>[[Wikiless]][861]</sup> <sup>[[Archive.org]][862]</sup>

[^124]: Wikipedia, Edward Snowden <https://en.wikipedia.org/wiki/Edward_Snowden> <sup>[[Wikiless]][863]</sup> <sup>[[Archive.org]][864]</sup>

[^125]: Wikipedia, Permanent Record <https://en.wikipedia.org/wiki/Permanent_Record_(autobiography)> <sup>[[Wikiless]][546]</sup> <sup>[[Archive.org]][547]</sup>

[^126]: Wikipedia, XKEYSCORE <https://en.wikipedia.org/wiki/XKeyscore> <sup>[[Wikiless]][865]</sup> <sup>[[Archive.org]][866]</sup>

[^127]: ElectroSpaces, Danish military intelligence uses XKEYSCORE to tap cables in cooperation with the NSA <https://www.electrospaces.net/2020/10/danish-military-intelligence-uses.html> <sup>[[Archive.org]][867]</sup>

[^128]: Wikipedia, MUSCULAR <https://en.m.wikipedia.org/wiki/MUSCULAR_(surveillance_program)> <sup>[[Archive.org]][868]</sup>

[^129]: Wikipedia, SORM <https://en.wikipedia.org/wiki/SORM> <sup>[[Wikiless]][869]</sup> <sup>[[Archive.org]][870]</sup>

[^130]: Wikipedia, Tempora <https://en.wikipedia.org/wiki/Tempora> <sup>[[Wikiless]][871]</sup> <sup>[[Archive.org]][872]</sup>

[^131]: Wikipedia, PRISM <https://en.wikipedia.org/wiki/PRISM_(surveillance_program)> <sup>[[Wikiless]][873]</sup> <sup>[[Archive.org]][874]</sup>

[^132]: Justsecurity, General Hayden <https://www.justsecurity.org/10318/video-clip-director-nsa-cia-we-kill-people-based-metadata/> <sup>[[Archive.org]][875]</sup>

[^133]: IDMB, The Social Dilemma <https://www.imdb.com/title/tt11464826/> <sup>[[Archive.org]][876]</sup>

[^134]: ArsTechnica, How the way you type can shatter anonymity---even on Tor <https://arstechnica.com/information-technology/2015/07/how-the-way-you-type-can-shatter-anonymity-even-on-tor/> <sup>[[Archive.org]][877]</sup>

[^135]: Wikipedia, Stylometry <https://en.wikipedia.org/wiki/Stylometry> <sup>[[Wikiless]][682]</sup> <sup>[[Archive.org]][878]</sup>

[^136]: Paul Moore Blog, Behavioral Profiling: The password you can't change. <https://paul.reviews/behavioral-profiling-the-password-you-cant-change/> <sup>[[Archive.org]][879]</sup>

[^137]: Wikipedia, Sentiment Analysis, <https://en.wikipedia.org/wiki/Sentiment_analysis> <sup>[[Wikiless]][880]</sup> <sup>[[Archive.org]][881]</sup>

[^138]: EFF CoverYourTracks, <https://coveryourtracks.eff.org/> <sup>[[Archive.org]][882]</sup>

[^139]: Berkeley.edu, On the Feasibility of Internet-Scale Author Identification <https://people.eecs.berkeley.edu/~dawnsong/papers/2012%20On%20the%20Feasibility%20of%20Internet-Scale%20Author%20Identification.pdf> <sup>[[Archive.org]][883]</sup>

[^140]: SecuredTouch Blog, Behavioral Biometrics 101: Behavioral Biometrics vs. Behavioral Analytics <https://blog.securedtouch.com/behavioral-biometrics-101-an-in-depth-look-at-behavioral-biometrics-vs-behavioral-analytics> <sup>[[Archive.org]][884]</sup>

[^141]: ArsTechnica, Stakeout: how the FBI tracked and busted a Chicago Anon <https://arstechnica.com/tech-policy/2012/03/stakeout-how-the-fbi-tracked-and-busted-a-chicago-anon/> <sup>[[Archive.org]][885]</sup>

[^142]: Bellingcat MH17 - Russian GRU Commander 'Orion' Identified as Oleg Ivannikov <https://www.bellingcat.com/news/uk-and-europe/2018/05/25/mh17-russian-gru-commander-orion-identified-oleg-ivannikov/> <sup>[[Archive.org]][886]</sup>

[^143]: Facebook Research, Deepface <https://research.fb.com/publications/deepface-closing-the-gap-to-human-level-performance-in-face-verification/> <sup>[[Archive.org]][887]</sup>

[^144]: Privacy News Online, Putting the "face" in Facebook: how Mark Zuckerberg is building a world without public anonymity <https://www.privateinternetaccess.com/blog/putting-face-facebook-mark-zuckerberg-building-world-without-public-anonymity/> <sup>[[Archive.org]][888]</sup>

[^145]: CNBC, "Facebook has mapped populations in 23 countries as it explores satellites to expand internet" <https://www.cnbc.com/2017/09/01/facebook-has-mapped-human-population-building-internet-in-space.html> <sup>[[Archive.org]][889]</sup>

[^146]: MIT Technology Review, This is how we lost control of our faces, <https://www.technologyreview.com/2021/02/05/1017388/ai-deep-learning-facial-recognition-data-history/> <sup>[[Archive.org]][890]</sup>

[^147]: Bellingcat, Shadow of a Doubt: Crowdsourcing Time Verification of the MH17 Missile Launch Photo <https://www.bellingcat.com/resources/case-studies/2015/08/07/shadow-of-a-doubt/> <sup>[[Archive.org]][891]</sup>

[^148]: Brown Institute, Open-Source Investigation, <https://brown.columbia.edu/open-source-investigation/> <sup>[[Archive.org]][892]</sup>

[^149]: NewScientist, Facebook can recognize you in photos even if you're not looking <https://www.newscientist.com/article/dn27761-facebook-can-recognise-you-in-photos-even-if-youre-not-looking/> <sup>[[Archive.org]][893]</sup>

[^150]: Google Patent, Techniques for emotion detection and content delivery <https://patents.google.com/patent/US20150242679> <sup>[[Archive.org]][894]</sup>

[^151]: APNews, Chinese 'gait recognition' tech IDs people by how they walk <https://apnews.com/article/bf75dd1c26c947b7826d270a16e2658a> <sup>[[Archive.org]][895]</sup>

[^152]: The Sun, New CCTV technology could now identify you just by the WAY you walk and your body shape <https://www.thesun.co.uk/news/7684204/cctv-technology-identify-body-shape-way-walk/> <sup>[[Archive.org]][896]</sup>

[^153]: City Security Magazine, Gait recognition: a useful identification tool <https://citysecuritymagazine.com/security-management/gait-recognition-identification-tool/> <sup>[[Archive.org]][897]</sup>

[^154]: Vice.com, Tech Companies Are Training AI to Read Your Lips <https://www.vice.com/en/article/bvzvdw/tech-companies-are-training-ai-to-read-your-lips> <sup>[[Archive.org]][898]</sup>

[^155]: New Atlas, Eye tracking can reveal an unbelievable amount of information about you <https://newatlas.com/science/science/eye-tracking-privacy/> <sup>[[Archive.org]][899]</sup>

[^156]: TechCrunch, Facial recognition reveals political party in troubling new research <https://techcrunch.com/2021/01/13/facial-recognition-reveals-political-party-in-troubling-new-research/> <sup>[[Archive.org]][900]</sup>

[^157]: Nature.com, Facial recognition technology can expose political orientation from naturalistic facial images <https://www.nature.com/articles/s41598-020-79310-1> <sup>[[Archive.org]][114]</sup>

[^158]: Slate <https://slate.com/technology/2018/04/facebook-collects-data-on-non-facebook-users-if-they-want-to-delete-it-they-have-to-sign-up.html> <sup>[[Archive.org]][901]</sup>

[^159]: The Conversation <https://theconversation.com/shadow-profiles-facebook-knows-about-you-even-if-youre-not-on-facebook-94804> <sup>[[Archive.org]][902]</sup>

[^160]: The Verge <https://www.theverge.com/2018/4/11/17225482/facebook-shadow-profiles-zuckerberg-congress-data-privacy> <sup>[[Archive.org]][903]</sup>

[^161]: ZDNET <https://www.zdnet.com/article/anger-mounts-after-facebooks-shadow-profiles-leak-in-bug/> <sup>[[Archive.org]][904]</sup>

[^162]: CNET <https://www.cnet.com/news/shadow-profiles-facebook-has-information-you-didnt-hand-over/> <sup>[[Archive.org]][905]</sup>

[^163]: Anyvision <https://www.anyvision.co/> <sup>[[Archive.org]][906]</sup>

[^164]: BuzzFeed.news, Surveillance Nation <https://www.buzzfeednews.com/article/ryanmac/clearview-ai-local-police-facial-recognition> <sup>[[Archive.org]][907]</sup>

[^165]: NEC, Neoface <https://www.nec.com/en/global/solutions/biometrics/face/neofacewatch.html> <sup>[[Archive.org]][908]</sup>

[^166]: The Guardian, Met police deploy live facial recognition technology <https://www.theguardian.com/uk-news/2020/feb/11/met-police-deploy-live-facial-recognition-technology> <sup>[[Archive.org]][909]</sup>

[^167]: YouTube, The Economist, China: facial recognition and state control <https://www.youtube.com/watch?v=lH2gMNrUuEY> <sup>[[Invidious]][910]</sup>

[^168]: CNN, Want your unemployment benefits? You may have to submit to facial recognition first <https://edition.cnn.com/2021/07/23/tech/idme-unemployment-facial-recognition/index.html> <sup>[[Archive.org]][911]</sup>

[^169]: Washington Post, Huawei tested AI software that could recognize Uighur minorities and alert police, report says <https://www.washingtonpost.com/technology/2020/12/08/huawei-tested-ai-software-that-could-recognize-uighur-minorities-alert-police-report-says/> <sup>[[Archive.org]][912]</sup>

[^170]: The Intercept, How a Facial Recognition Mismatch Can Ruin Your Life <https://theintercept.com/2016/10/13/how-a-facial-recognition-mismatch-can-ruin-your-life/> <sup>[[Tor Mirror]][913]</sup> <sup>[[Archive.org]][914]</sup>

[^171]: Vice, Facial Recognition Failures Are Locking People Out of Unemployment Systems <https://www.vice.com/en/article/5dbywn/facial-recognition-failures-are-locking-people-out-of-unemployment-systems> <sup>[[Archive.org]][915]</sup>

[^172]: BBC, WhatsApp photo drug dealer caught by 'groundbreaking' work <https://www.bbc.com/news/uk-wales-43711477> <sup>[[Archive.org]][916]</sup>

[^173]: CNN, Drug dealer jailed after sharing a photo of cheese that included his fingerprints <https://edition.cnn.com/2021/05/25/uk/drug-dealer-cheese-sentenced-scli-gbr-intl/index.html> <sup>[[Archive.org]][917]</sup>

[^174]: Vice.com, Cops Got a Drug Dealer's Fingerprints From Photos of His Hand on WhatsApp <https://www.vice.com/en/article/evqk9e/photo-of-fingerprints-used-to-arrest-drug-dealers> <sup>[[Archive.org]][918]</sup>

[^175]: JUSTIA Patent, Identification of taste attributes from an audio signal <https://patents.justia.com/patent/10891948> <sup>[[Archive.org]][919]</sup>

[^176]: PYMNTS, Iris Scan Serves As Traveler ID At Dubai Airport <https://www.pymnts.com/news/biometrics/2021/iris-scan-traveler-identification-dubai-airport/> <sup>[[Archive.org]][920]</sup>

[^177]: IMDB, Gattaca 1997, <https://www.imdb.com/title/tt0119177/> <sup>[[Archive.org]][921]</sup>

[^178]: IMDB, Person of Interest 2011 <https://www.imdb.com/title/tt1839578> <sup>[[Archive.org]][922]</sup>

[^179]: IMDB, Minority Report 2002, <https://www.imdb.com/title/tt0181689> <sup>[[Archive.org]][923]</sup>

[^180]: Wikipedia, Deepfake <https://en.wikipedia.org/wiki/Deepfake> <sup>[[Wikiless]][924]</sup> <sup>[[Archive.org]][925]</sup>

[^181]: Econotimes, Deepfake Voice Technology: The Good. The Bad. The Future <https://www.econotimes.com/Deepfake-Voice-Technology-The-Good-The-Bad-The-Future-1601278> <sup>[[Archive.org]][926]</sup>

[^182]: Wikipedia, Deepfake Events <https://en.wikipedia.org/wiki/Deepfake#Example_events> <sup>[[Wikiless]][924]</sup> <sup>[[Archive.org]][925]</sup>

[^183]: Forbes, A Voice Deepfake Was Used To Scam A CEO Out Of $243,000 <https://www.forbes.com/sites/jessedamiani/2019/09/03/a-voice-deepfake-was-used-to-scam-a-ceo-out-of-243000/> <sup>[[Archive.org]][927]</sup>

[^184]: Joseph Steinberg, How To Prevent Facial Recognition Technology From Identifying You <https://josephsteinberg.com/how-to-prevent-facial-recognition-technology-from-identifying-you/> <sup>[[Archive.org]][928]</sup>

[^185]: NIST, Face recognition accuracy with masks using pre-COVID-19 algorithms <https://nvlpubs.nist.gov/nistpubs/ir/2020/NIST.IR.8311.pdf> <sup>[[Archive.org]][929]</sup>

[^186]: BBC, Facial recognition identifies people wearing masks <https://www.bbc.com/news/technology-55573802> <sup>[[Archive.org]][930]</sup>

[^187]: University of Wisconsin, Exploring Reflectacles As Anti-Surveillance Glasses and for Adversarial Machine Learning in Computer Vision <http://diglib.uwgb.edu/digital/api/collection/p17003coll4/id/71/download> <sup>[[Archive.org]][931]</sup>

[^188]: Wikipedia, Phishing <https://en.wikipedia.org/wiki/Phishing> <sup>[[Wikiless]][932]</sup> <sup>[[Archive.org]][933]</sup>

[^189]: Wikipedia, Social Engineering <https://en.wikipedia.org/wiki/Social_engineering_(security)> <sup>[[Wikiless]][934]</sup> <sup>[[Archive.org]][935]</sup>

[^190]: BBC, Spy pixels in emails have become endemic <https://www.bbc.com/news/technology-56071437> <sup>[[Archive.org]][936]</sup>

[^191]: Vice, Facebook Helped the FBI Hack a Child Predator <https://www.vice.com/en/article/v7gd9b/facebook-helped-fbi-hack-child-predator-buster-hernandez> <sup>[[Archive.org]][652]</sup>

[^192]: Wikipedia, Exploit <https://en.wikipedia.org/wiki/Exploit_(computer_security)> <sup>[[Wikiless]][937]</sup> <sup>[[Archive.org]][938]</sup>

[^193]: Wikipedia, Freedom Hosting <https://en.wikipedia.org/wiki/Freedom_Hosting> <sup>[[Wikiless]][939]</sup> <sup>[[Archive.org]][940]</sup>

[^194]: Wired, 2013 FBI Admits It Controlled Tor Servers Behind Mass Malware Attack <https://www.wired.com/2013/09/freedom-hosting-fbi/> <sup>[[Archive.org]][941]</sup>

[^195]: Wikipedia, 2020 United States federal government data breach <https://en.wikipedia.org/wiki/2020_United_States_federal_government_data_breach> <sup>[[Wikiless]][942]</sup> <sup>[[Archive.org]][943]</sup>

[^196]: BBC, China social media: WeChat and the Surveillance State <https://www.bbc.com/news/blogs-china-blog-48552907> <sup>[[Archive.org]][944]</sup>

[^197]: The Intercept, Revealed: Massive Chinese Police Database <https://theintercept.com/2021/01/29/china-uyghur-muslim-surveillance-police/> <sup>[[Tor Mirror]][945]</sup> <sup>[[Archive.org]][946]</sup>

[^198]: Wikipedia, Sandbox <https://en.wikipedia.org/wiki/Sandbox_(computer_security)> <sup>[[Wikiless]][947]</sup> <sup>[[Archive.org]][948]</sup>

[^199]: Wired, Why the Security of USB Is Fundamentally Broken <https://www.wired.com/2014/07/usb-security/> <sup>[[Archive.org]][949]</sup>

[^200]: Wikipedia, Stuxnet <https://en.wikipedia.org/wiki/Stuxnet> <sup>[[Wikiless]][950]</sup> <sup>[[Archive.org]][951]</sup>

[^201]: Superuser.com, How do I safely investigate a USB stick found in the parking lot at work? <https://superuser.com/questions/1206321/how-do-i-safely-investigate-a-usb-stick-found-in-the-parking-lot-at-work> <sup>[[Archive.org]][952]</sup>

[^202]: The Guardian, Glenn Greenwald: how the NSA tampers with US-made internet routers <https://www.theguardian.com/books/2014/may/12/glenn-greenwald-nsa-tampers-us-internet-routers-snowden> <sup>[[Archive.org]][953]</sup>

[^203]: Wikipedia, Rootkit <https://en.wikipedia.org/wiki/Rootkit> <sup>[[Wikiless]][954]</sup> <sup>[[Archive.org]][955]</sup>

[^204]: Wikipedia, Userspace <https://en.wikipedia.org/wiki/User_space> <sup>[[Wikiless]][956]</sup> <sup>[[Archive.org]][957]</sup>

[^205]: Wikipedia, Firmware <https://en.wikipedia.org/wiki/Firmware> <sup>[[Wikiless]][958]</sup> <sup>[[Archive.org]][959]</sup>

[^206]: Wikipedia, BIOS <https://en.wikipedia.org/wiki/BIOS> <sup>[[Wikiless]][960]</sup> <sup>[[Archive.org]][961]</sup>

[^207]: Wikipedia, UEFI <https://en.wikipedia.org/wiki/Unified_Extensible_Firmware_Interface> <sup>[[Wikiless]][962]</sup> <sup>[[Archive.org]][963]</sup>

[^208]: Bellingcat, Joseph Mifsud: Rush for the EXIF <https://www.bellingcat.com/news/americas/2018/10/26/joseph-mifsud-rush-exif/> <sup>[[Archive.org]][964]</sup>

[^209]: Zoom Support, Adding a watermark <https://support.zoom.us/hc/en-us/articles/209605273-Adding-a-Watermark> <sup>[[Archive.org]][965]</sup>

[^210]: Zoom Support, Audio Watermark <https://support.zoom.us/hc/en-us/articles/360021839031-Audio-Watermark> <sup>[[Archive.org]][966]</sup>

[^211]: CreativeCloud Extension, IMATAG <https://exchange.adobe.com/creativecloud.details.101789.imatag-invisible-watermark-and-image-monitoring.html> <sup>[[Archive.org]][967]</sup>

[^212]: NexGuard, <https://dtv.nagra.com/nexguard-forensic-watermarking> <sup>[[Archive.org]][968]</sup>

[^213]: Vobile Solutions, <https://www.vobilegroup.com/> <sup>[[Archive.org]][969]</sup>

[^214]: Cinavia, <https://www.cinavia.com/languages/english/pages/technology.html> <sup>[[Archive.org]][970]</sup>

[^215]: Imatag, <https://www.imatag.com/> <sup>[[Archive.org]][971]</sup>

[^216]: Wikipedia, Steganography <https://en.wikipedia.org/wiki/Steganography> <sup>[[Wikiless]][972]</sup> <sup>[[Archive.org]][973]</sup>

[^217]: IEEExplore, A JPEG compression resistant steganography scheme for raster graphics images <https://ieeexplore.ieee.org/document/4428921> <sup>[[Archive.org]][974]</sup>

[^218]: ScienceDirect, Robust audio watermarking using perceptual masking <https://www.sciencedirect.com/science/article/abs/pii/S0165168498000140> <sup>[[Archive.org]][975]</sup>

[^219]: IEEExplore, Spread-spectrum watermarking of audio signals <https://ieeexplore.ieee.org/abstract/document/1188746> <sup>[[Archive.org]][976]</sup>

[^220]: Google Scholar, source camera identification <https://scholar.google.com/scholar?q=source+camera+identification> <sup>[[Archive.org]][977]</sup>

[^221]: Wikipedia, Printing Steganography <https://en.wikipedia.org/wiki/Machine_Identification_Code> <sup>[[Wikiless]][978]</sup> <sup>[[Archive.org]][979]</sup>

[^222]: MIT, SeeingYellow, <http://seeingyellow.com/> <sup>[[Archive.org]][980]</sup>

[^223]: arXiv, An Analysis of Anonymity in the Bitcoin System <https://arxiv.org/abs/1107.4524> <sup>[[Archive.org]][981]</sup>

[^224]: Bellingcat, How To Track Illegal Funding Campaigns Via Cryptocurrency, <https://www.bellingcat.com/resources/how-tos/2019/03/26/how-to-track-illegal-funding-campaigns-via-cryptocurrency/> <sup>[[Archive.org]][982]</sup>

[^225]: Wikipedia, KYC <https://en.wikipedia.org/wiki/Know_your_customer> <sup>[[Wikiless]][983]</sup> <sup>[[Archive.org]][984]</sup>

[^226]: arXiv.org, Probing the Mystery of Cryptocurrency Theft: An Investigation into Methods for Taint Analysis <https://arxiv.org/pdf/1906.05754.pdf> <sup>[[Archive.org]][985]</sup>

[^227]: YouTube, Breaking Monero <https://www.youtube.com/watch?v=WOyC6OB6ezA&list=PLsSYUeVwrHBnAUre2G_LYDsdo-tD0ov-y> <sup>[[Invidious]][986]</sup>

[^228]: Monero, Monero vs Princeton Researchers, <https://monero.org/monero-vs-princeton-researchers/> <sup>[[Archive.org]][987]</sup>

[^229]: Wikipedia, Cryptocurrency Tumbler <https://en.wikipedia.org/wiki/Cryptocurrency_tumbler> <sup>[[Wikiless]][988]</sup> <sup>[[Archive.org]][989]</sup>

[^230]: Wikipedia, Security Through Obscurity <https://en.wikipedia.org/wiki/Security_through_obscurity> <sup>[[Wikiless]][990]</sup> <sup>[[Archive.org]][991]</sup>

[^231]: ArXiv, Tracking Mixed Bitcoins, <https://arxiv.org/abs/2009.14007> <sup>[[Archive.org]][992]</sup>

[^232]: SSRN, The Cryptocurrency Tumblers: Risks, Legality and Oversight <https://papers.ssrn.com/sol3/papers.cfm?abstract_id=3080361> <sup>[[Archive.org]][993]</sup>

[^233]: Magnet Forensics, Magnet AXIOM <https://www.magnetforensics.com/products/magnet-axiom/cloud/> <sup>[[Archive.org]][994]</sup>

[^234]: Cellebrite, Unlock cloud-based evidence to solve the case sooner <https://www.cellebrite.com/en/ufed-cloud/> <sup>[[Archive.org]][995]</sup>

[^235]: Chromium Documentation, Technical analysis of client identification mechanisms <https://sites.google.com/a/chromium.org/dev/Home/chromium-security/client-identification-mechanisms#TOC-Machine-specific-characteristics> <sup>[[Archive.org]][996]</sup>

[^236]: Mozilla Wiki, Fingerprinting <https://wiki.mozilla.org/Fingerprinting> <sup>[[Archive.org]][997]</sup>

[^237]: Grayshirt, <https://www.grayshift.com/> <sup>[[Archive.org]][998]</sup>

[^238]: Securephones.io, Data Security on Mobile Devices: Current State of the Art, Open Problems, and Proposed Solutions <https://securephones.io/main.pdf> <sup>[[Archive.org]][999]</sup>

[^239]: Loup-Vaillant.fr, Rolling Your Own Crypto <https://loup-vaillant.fr/articles/rolling-your-own-crypto> <sup>[[Archive.org]][1000]</sup>

[^240]: Dhole Moments, Crackpot Cryptography and Security Theater <https://soatok.blog/2021/02/09/crackpot-cryptography-and-security-theater/> <sup>[[Archive.org]][1001]</sup>

[^241]: Vice.com, Why You Don't Roll Your Own Crypto <https://www.vice.com/en/article/wnx8nq/why-you-dont-roll-your-own-crypto> <sup>[[Archive.org]][1002]</sup>

[^242]: arXiv, MIT, You Really Shouldn't Roll Your Own Crypto: An Empirical Study of Vulnerabilities in Cryptographic Libraries <https://arxiv.org/pdf/2107.04940.pdf> <sup>[[Archive.org]][1003]</sup>

[^243]: YouTube, Great Crypto Failures <https://www.youtube.com/watch?v=loy84K3AJ5Q> <sup>[[Invidious]][1004]</sup>

[^244]: Cryptography Dispatches, The Most Backdoor-Looking Bug I've Ever Seen <https://buttondown.email/cryptography-dispatches/archive/cryptography-dispatches-the-most-backdoor-looking/> <sup>[[Archive.org]][165]</sup>

[^245]: Citizenlab.ca, Move Fast and Roll Your Own Crypto <https://citizenlab.ca/2020/04/move-fast-roll-your-own-crypto-a-quick-look-at-the-confidentiality-of-zoom-meetings/> <sup>[[Archive.org]][1005]</sup>

[^246]: Jack Poon, The myth of military grade encryption <https://medium.com/@atcipher/the-myth-of-military-grade-encryption-292313ae6369> <sup>[[Archive.org]][1006]</sup>

[^247]: Congruent Labs, Stop calling it "Military-Grade Encryption" <https://blog.congruentlabs.co/military-grade-encryption/> <sup>[[Archive.org]][1007]</sup>

[^248]: IronCoreLabs Blog, "Military Grade Encryption" <https://blog.ironcorelabs.com/military-grade-encryption-69aae0145588> <sup>[[Archive.org]][1008]</sup>

[^249]: Wikipedia, BLAKE2, <https://en.wikipedia.org/wiki/BLAKE_(hash_function)#BLAKE2> <sup>[[Wikiless]][1009]</sup> <sup>[[Archive.org]][1010]</sup>

[^250]: Wikipedia, AES Instruction Set, <https://en.wikipedia.org/wiki/AES_instruction_set> <sup>[[Wikiless]][1011]</sup> <sup>[[Archive.org]][1012]</sup>

[^251]: Wikipedia, ChaCha Variants, <https://en.wikipedia.org/wiki/Salsa20#ChaCha_variant> <sup>[[Wikiless]][1013]</sup> <sup>[[Archive.org]][1014]</sup>

[^252]: Wikipedia, Serpent, <https://en.wikipedia.org/wiki/Serpent_(cipher)> <sup>[[Wikiless]][1015]</sup> <sup>[[Archive.org]][1016]</sup>

[^253]: Wikipedia, TwoFish, <https://en.wikipedia.org/wiki/Twofish> <sup>[[Wikiless]][1017]</sup> <sup>[[Archive.org]][1018]</sup>

[^254]: Wikipedia, Shor's Algorithm, <https://en.wikipedia.org/wiki/Shor%27s_algorithm> <sup>[[Wikiless]][1019]</sup> <sup>[[Archive.org]][1020]</sup>

[^255]: Wikipedia, Gag Order, <https://en.wikipedia.org/wiki/Gag_order> <sup>[[Wikiless]][1021]</sup> <sup>[[Archive.org]][1022]</sup>

[^256]: Wikipedia, National Security Letter <https://en.wikipedia.org/wiki/National_security_letter> <sup>[[Wikiless]][1023]</sup> <sup>[[Archive.org]][1024]</sup>

[^257]: TechCrunch, ProtonMail logged IP address of French activist after order by Swiss authorities <https://techcrunch.com/2021/09/06/protonmail-logged-ip-address-of-french-activist-after-order-by-swiss-authorities/> <sup>[[Archive.org]][1025]</sup>

[^258]: ArsTechnica, VPN servers seized by Ukrainian authorities weren't encrypted <https://arstechnica.com/gadgets/2021/07/vpn-servers-seized-by-ukrainian-authorities-werent-encrypted/> <sup>[[Archive.org]][1026]</sup>

[^259]: BleepingComputer, DoubleVPN servers, logs, and account info seized by law enforcement <https://www.bleepingcomputer.com/news/security/doublevpn-servers-logs-and-account-info-seized-by-law-enforcement/> <sup>[[Archive.org]][1027]</sup>

[^260]: CyberScoop, Court rules encrypted email provider Tutanota must monitor messages in blackmail case <https://www.cyberscoop.com/court-rules-encrypted-email-tutanota-monitor-messages/> <sup>[[Archive.org]][1028]</sup>

[^261]: Heise Online (German), <https://www.heise.de/news/Gericht-zwingt-Mailprovider-Tutanota-zu-Ueberwachungsfunktion-4972460.html> <sup>[[Archive.org]][1029]</sup>

[^262]: PCMag, Did PureVPN Cross a Line When It Disclosed User Information? <https://www.pcmag.com/opinions/did-purevpn-cross-a-line-when-it-disclosed-user-information> <sup>[[Archive.org]][1030]</sup>

[^263]: Internet Archive, Wipeyourdata, "No logs" EarthVPN user arrested after police finds logs <https://archive.is/XNuVw#selection-230.0-230.1> <sup>[[Archive.org]][1031]</sup>

[^264]: Internet Archive, Invisibler, What Everybody Ought to Know About HideMyAss <https://archive.is/ag9w4#selection-136.0-136.1>

[^265]: Wikipedia, Lavabit Suspension and Gag order, <https://en.wikipedia.org/wiki/Lavabit#Suspension_and_gag_order> <sup>[[Wikiless]][1032]</sup> <sup>[[Archive.org]][1033]</sup>

[^266]: Wikipedia, Warrant Canary <https://en.wikipedia.org/wiki/Warrant_canary> <sup>[[Wikiless]][1034]</sup> <sup>[[Archive.org]][1035]</sup>

[^267]: Washington Post, The intelligence coup of the century <https://www.washingtonpost.com/graphics/2020/world/national-security/cia-crypto-encryption-machines-espionage/> <sup>[[Archive.org]][1036]</sup>

[^268]: Swissinfo.ch, Second Swiss firm allegedly sold encrypted spying devices <https://www.swissinfo.ch/eng/second-swiss-firm-allegedly-sold-encrypted-spying-devices/46186432> <sup>[[Archive.org]][1037]</sup>

[^269]: Wikipedia, Das Leben der Anderen <https://en.wikipedia.org/wiki/The_Lives_of_Others> <sup>[[Wikiless]][1038]</sup> <sup>[[Archive.org]][1039]</sup>

[^270]: Wired, Mind the Gap: This Researcher Steals Data With Noise, Light, and Magnets <https://www.wired.com/story/air-gap-researcher-mordechai-guri/> <sup>[[Archive.org]][1040]</sup>

[^271]: Ben Nassi, Lamphone, <https://www.nassiben.com/lamphone> <sup>[[Archive.org]][1041]</sup>

[^272]: The Guardian, Laser spying: is it really practical? <https://www.theguardian.com/world/2013/aug/22/gchq-warned-laser-spying-guardian-offices> <sup>[[Archive.org]][1042]</sup>

[^273]: ArsTechnica, Photos of an NSA "upgrade" factory show Cisco router getting implant <https://arstechnica.com/tech-policy/2014/05/photos-of-an-nsa-upgrade-factory-show-cisco-router-getting-implant/> <sup>[[Archive.org]][1043]</sup>

[^274]: Wikipedia, Rubber-hose Cryptanalysis https://en.wikipedia.org/wiki/Rubber-hose_cryptanalysis

[^275]: Defuse.ca, TrueCrypt's Plausible Deniability is Theoretically Useless <https://defuse.ca/truecrypt-plausible-deniability-useless-by-game-theory.htm> <sup>[[Archive.org]][240]</sup>

[^276]: Wikipedia, OONI, <https://en.wikipedia.org/wiki/OONI> <sup>[[Wikiless]][1044]</sup> <sup>[[Archive.org]][1045]</sup>

[^277]: Privacy International, Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws> <sup>[[Archive.org]][1046]</sup>

[^278]: NYTimes, Lost Passwords Lock Millionaires Out of Their Bitcoin Fortunes <https://www.nytimes.com/2021/01/12/technology/bitcoin-passwords-wallets-fortunes.html> <sup>[[Archive.org]][1047]</sup>

[^279]: Usenix.org, Shedding too much Light on a Microcontroller's Firmware Protection <https://www.usenix.org/system/files/conference/woot17/woot17-paper-obermaier.pdf> <sup>[[Archive.org]][1048]</sup>

[^280]: TorProject.org, Can I run Tor Browser on an iOS device? <https://support.torproject.org/tormobile/tormobile-3/> <sup>[[Archive.org]][1049]</sup>

[^281]: Wikipedia, Tails, <https://en.wikipedia.org/wiki/Tails_(operating_system)> <sup>[[Wikiless]][1050]</sup> <sup>[[Archive.org]][1051]</sup>

[^282]: Vice.com, Facebook Helped the FBI Hack a Child Predator <https://www.vice.com/en/article/v7gd9b/facebook-helped-fbi-hack-child-predator-buster-hernandez> <sup>[[Archive.org]][652]</sup>

[^283]: Veracrypt Documentation, Trim Operations <https://www.veracrypt.fr/en/Trim%20Operation.html> <sup>[[Archive.org]][1052]</sup>

[^284]: Coreboot, <https://www.coreboot.org/> <sup>[[Archive.org]][1053]</sup>

[^285]: YouTube, 36C3 - Uncover, Understand, Own - Regaining Control Over Your AMD CPU <https://www.youtube.com/watch?v=bKH5nGLgi08&t=2834s> <sup>[[Invidious]][78]</sup>

[^286]: Qubes OS, Anti-Evil Maid, <https://github.com/QubesOS/qubes-antievilmaid> <sup>[[Archive.org]][265]</sup>

[^287]: QubesOS FAQ, <https://www.qubes-os.org/faq/#is-secure-boot-supported> <sup>[[Archive.org]][361]</sup>

[^288]: Wikipedia, Secure Boot, <https://en.wikipedia.org/wiki/Unified_Extensible_Firmware_Interface#Secure_boot> <sup>[[Wikiless]][962]</sup> <sup>[[Archive.org]][963]</sup>

[^289]: Wikipedia, Booting <https://en.wikipedia.org/wiki/Booting> <sup>[[Wikiless]][1054]</sup> <sup>[[Archive.org]][1055]</sup>

[^290]: Wired <https://www.wired.com/2013/12/better-data-security-nail-polish/> <sup>[[Archive.org]][1056]</sup>

[^291]: Wikipedia, Virtual Machine <https://en.wikipedia.org/wiki/Virtual_machine> <sup>[[Wikiless]][1057]</sup> <sup>[[Archive.org]][1058]</sup>

[^292]: Wikipedia, Plausible Deniability <https://en.wikipedia.org/wiki/Plausible_deniability> <sup>[[Wikiless]][1059]</sup> <sup>[[Archive.org]][1060]</sup>

[^293]: Wikipedia, Deniable Encryption <https://en.wikipedia.org/wiki/Deniable_encryption> <sup>[[Wikiless]][1061]</sup> <sup>[[Archive.org]][1062]</sup>

[^294]: Privacytools.io, Don't use Windows 10 - It's a privacy nightmare <https://privacytools.io/operating-systems/#win10> <sup>[[Archive.org]][1063]</sup>

[^295]: Wikipedia, Deniable Encryption <https://en.wikipedia.org/wiki/Deniable_encryption> <sup>[[Wikiless]][1061]</sup> <sup>[[Archive.org]][1062]</sup>

[^296]: Wikipedia, Key Disclosure Laws <https://en.wikipedia.org/wiki/Key_disclosure_law> <sup>[[Wikiless]][533]</sup> <sup>[[Archive.org]][534]</sup>

[^297]: GP Digital, World map of encryption laws and policies <https://www.gp-digital.org/world-map-of-encryption/> <sup>[[Archive.org]][535]</sup>

[^298]: Wikipedia, Bitlocker <https://en.wikipedia.org/wiki/BitLocker> <sup>[[Wikiless]][1064]</sup> <sup>[[Archive.org]][1065]</sup>

[^299]: Wikipedia, Evil Maid Attack <https://en.wikipedia.org/wiki/Evil_maid_attack> <sup>[[Wikiless]][1066]</sup> <sup>[[Archive.org]][1067]</sup>

[^300]: Wikipedia, Cold Boot Attack <https://en.wikipedia.org/wiki/Cold_boot_attack> <sup>[[Wikiless]][1068]</sup> <sup>[[Archive.org]][1069]</sup>

[^301]: CITP 2008 (<https://www.youtube.com/watch?v=JDaicPIgn9U>) <sup>[[Invidious]][1070]</sup>

[^302]: ResearchGate, Defeating Plausible Deniability of VeraCrypt Hidden Operating Systems <https://www.researchgate.net/publication/318155607_Defeating_Plausible_Deniability_of_VeraCrypt_Hidden_Operating_Systems> <sup>[[Archive.org]][1071]</sup>

[^303]: SANS.org, Mission Implausible: Defeating Plausible Deniability with Digital Forensics <https://www.sans.org/reading-room/whitepapers/forensics/mission-implausible-defeating-plausible-deniability-digital-forensics-39500> <sup>[[Archive.org]][1072]</sup>

[^304]: SourceForge, Veracrypt Forum <https://sourceforge.net/p/veracrypt/discussion/technical/thread/53f33faf/> <sup>[[Archive.org]][1073]</sup>

[^305]: Microsoft, BitLocker Countermeasures <https://docs.microsoft.com/en-us/windows/security/information-protection/bitlocker/bitlocker-countermeasures> <sup>[[Archive.org]][1074]</sup>

[^306]: SANS, Windows ShellBag Forensics in-depth <https://www.sans.org/reading-room/whitepapers/forensics/windows-shellbag-forensics-in-depth-34545> <sup>[[Archive.org]][1075]</sup>

[^307]: University of York, Forensic data recovery from the Windows Search Database <https://eprints.whiterose.ac.uk/75046/1/Forensic_Data_Recovery_From_The_Windows_Search_Database_preprint_DIIN328.pdf> <sup>[[Archive.org]][1076]</sup>

[^308]: A forensic insight into Windows 10 Jump Lists <https://cyberforensicator.com/wp-content/uploads/2017/01/1-s2.0-S1742287616300202-main.2-14.pdf> <sup>[[Archive.org]][1077]</sup>

[^309]: Wikipedia, Gatekeeper <https://en.wikipedia.org/wiki/Gatekeeper_(macOS)> <sup>[[Wikiless]][1078]</sup> <sup>[[Archive.org]][1079]</sup>

[^310]: Alpine Linux Wiki, Setting up a laptop <https://wiki.alpinelinux.org/wiki/Setting_up_a_laptop> <sup>[[Archive.org]][1080]</sup>

[^311]: Wikipedia Veracrypt <https://en.wikipedia.org/wiki/VeraCrypt> <sup>[[Wikiless]][1081]</sup> <sup>[[Archive.org]][1082]</sup>

[^312]: OSTIF Veracrypt Audit, 2016, <https://web.archive.org/web/https://ostif.org/the-veracrypt-audit-results/>

[^313]: Veracrypt Documentation, Unencrypted Data in RAM <https://www.veracrypt.fr/en/Unencrypted%20Data%20in%20RAM.html> <sup>[[Archive.org]][1083]</sup>

[^314]: Veracrypt Documentation, Data Leaks <https://www.veracrypt.fr/code/VeraCrypt/plain/doc/html/Data%20Leaks.html> <sup>[[Archive.org]][1084]</sup>

[^315]: Dolos Group, From Stolen Laptop to Inside the Company Network <https://dolosgroup.io/blog/2021/7/9/from-stolen-laptop-to-inside-the-company-network> <sup>[[Archive.org]][1085]</sup>

[^316]: Trammell Hudson's Projects, Understanding TPM Sniffing Attacks <https://trmm.net/tpm-sniffing/> <sup>[[Archive.org]][1086]</sup>

[^317]: Microsoft, BitLocker Countermeasures, Attacker countermeasures <https://docs.microsoft.com/en-us/windows/security/information-protection/bitlocker/bitlocker-countermeasures> <sup>[[Archive.org]][1074]</sup>

[^318]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Trim_(computing)> <sup>[[Wikiless]][467]</sup> <sup>[[Archive.org]][468]</sup>

[^319]: Veracrypt Documentation, Trim Operations <https://www.veracrypt.fr/en/Trim%20Operation.html> <sup>[[Archive.org]][1052]</sup>

[^320]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html> <sup>[[Archive.org]][1087]</sup>

[^321]: St Cloud State University, Forensic Research on Solid State Drives using Trim Analysis <https://repository.stcloudstate.edu/cgi/viewcontent.cgi?article=1141&context=msia_etds> <sup>[[Archive.org]][1088]</sup>

[^322]: WindowsCentral, Trim Tutorial <https://www.windowscentral.com/how-ensure-trim-enabled-windows-10-speed-ssd-performance> <sup>[[Archive.org]][1089]</sup>

[^323]: Veracrypt Documentation, Trim Operation <https://veracrypt.eu/en/docs/trim-operation/> <sup>[[Archive.org]][1090]</sup>

[^324]: Black Hat 2018, Perfectly Deniable Steganographic Disk Encryption <https://i.blackhat.com/eu-18/Thu-Dec-6/eu-18-Schaub-Perfectly-Deniable-Steganographic-Disk-Encryption.pdf> <sup>[[Archive.org]][1091]</sup>

[^325]: Milan Broz's Blog, TRIM & dm-crypt ... problems? <http://asalor.blogspot.com/2011/08/trim-dm-crypt-problems.html> <sup>[[Archive.org]][1092]</sup>

[^326]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html> <sup>[[Archive.org]][1087]</sup>

[^327]: Wikipedia, Virtualbox <https://en.wikipedia.org/wiki/VirtualBox> <sup>[[Wikiless]][1093]</sup> <sup>[[Archive.org]][1094]</sup>

[^328]: VirtualBox Ticket 17987 <https://www.virtualbox.org/ticket/17987> <sup>[[Archive.org]][1095]</sup>

[^329]: Whonix Documentation, Spectre Meltdown, <https://www.whonix.org/wiki/Spectre_Meltdown#VirtualBox> <sup>[[Archive.org]][83]</sup>

[^330]: Whonix Documentation, Stream Isolation <https://www.whonix.org/wiki/Stream_Isolation> <sup>[[Archive.org]][310]</sup>

[^331]: Whonix Documentation, Tunnels Comparison Table, <https://www.whonix.org/wiki/Tunnels/Introduction#Comparison_Table> <sup>[[Archive.org]][312]</sup>

[^332]: Wikipedia, Whonix <https://en.wikipedia.org/wiki/Whonix> <sup>[[Wikiless]][1096]</sup> <sup>[[Archive.org]][1097]</sup>

[^333]: Oracle Virtualbox Manual, Snapshots <https://docs.oracle.com/en/virtualization/virtualbox/6.0/user/snapshots.html> <sup>[[Archive.org]][1098]</sup>

[^334]: Utica College, FORENSIC RECOVERY OF EVIDENCE FROM DELETED ORACLE VIRTUALBOX VIRTUAL MACHINES <https://web.archive.org/web/https://programs.online.utica.edu/sites/default/files/Neal_6_Gonnella_Forensic_Recovery_of_Evidence_from_Deleted_Oracle_VirtualBox_Virtual_Machine.pdf>

[^335]: Wikipedia, Spectre <https://en.wikipedia.org/wiki/Spectre_(security_vulnerability)> <sup>[[Wikiless]][1099]</sup> <sup>[[Archive.org]][1100]</sup>

[^336]: Wikipedia, Meltdown <https://en.wikipedia.org/wiki/Meltdown_(security_vulnerability)> <sup>[[Wikiless]][1101]</sup> <sup>[[Archive.org]][1102]</sup>

[^337]: Whonix Documentation, Stream Isolation, By Settings <https://www.whonix.org/wiki/Stream_Isolation#By_Settings> <sup>[[Archive.org]][1103]</sup>

[^338]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm> <sup>[[Wikiless]][1104]</sup> <sup>[[Archive.org]][1105]</sup>

[^339]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication> <sup>[[Wikiless]][1106]</sup> <sup>[[Archive.org]][1107]</sup>

[^340]: Whonix Documentation, Bridged Adapters Warning <https://www.whonix.org/wiki/Whonix-Gateway_Security#Warning:_Bridged_Networking> <sup>[[Archive.org]][1108]</sup>

[^341]: Qubes OS, FAQ, <https://www.qubes-os.org/faq/#is-qubes-just-another-linux-distribution> <sup>[[Archive.org]][1109]</sup>

[^342]: Qubes OS, System Requirements <https://www.qubes-os.org/doc/system-requirements/> <sup>[[Archive.org]][1110]</sup>

[^343]: Whonix Documentation, Stream Isolation <https://www.whonix.org/wiki/Stream_Isolation> <sup>[[Archive.org]][310]</sup>

[^344]: Whonix Documentation, Tunnels Comparison Table, <https://www.whonix.org/wiki/Tunnels/Introduction#Comparison_Table> <sup>[[Archive.org]][312]</sup>

[^345]: Qubes OS Issues, Simulate Hibernation / Suspend-To-Disk #2414 <https://github.com/QubesOS/qubes-issues/issues/2414> <sup>[[Archive.org]][1111]</sup>

[^346]: Wikipedia, AppArmor <https://en.wikipedia.org/wiki/AppArmor> <sup>[[Wikiless]][1112]</sup> <sup>[[Archive.org]][1113]</sup>

[^347]: Wikipedia, SELinux <https://en.wikipedia.org/wiki/Security-Enhanced_Linux> <sup>[[Wikiless]][1114]</sup> <sup>[[Archive.org]][1115]</sup>

[^348]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm> <sup>[[Wikiless]][1104]</sup> <sup>[[Archive.org]][1105]</sup>

[^349]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication> <sup>[[Wikiless]][1106]</sup> <sup>[[Archive.org]][1107]</sup>

[^350]: Wikipedia, Captcha <https://en.wikipedia.org/wiki/CAPTCHA> <sup>[[Wikiless]][1116]</sup> <sup>[[Archive.org]][1117]</sup>

[^351]: Wikipedia, Turing Test <https://en.wikipedia.org/wiki/Turing_test> <sup>[[Wikiless]][1118]</sup> <sup>[[Archive.org]][1119]</sup>

[^352]: Google reCAPTCHA <https://www.google.com/recaptcha/about/> <sup>[[Archive.org]][1120]</sup>

[^353]: hCaptcha <https://www.hcaptcha.com/> <sup>[[Archive.org]][1121]</sup>

[^354]: hCaptcha, hCaptcha Is Now the Largest Independent CAPTCHA Service, Runs on 15% Of The Internet <https://www.hcaptcha.com/post/hcaptcha-now-the-largest-independent-captcha-service> <sup>[[Archive.org]][1122]</sup>

[^355]: Nearcyan.com, You (probably) don't need ReCAPTCHA <https://nearcyan.com/you-probably-dont-need-recaptcha/> <sup>[[Archive.org]][1123]</sup>

[^356]: ArsTechnica, "Google's reCAPTCHA turns "invisible," will separate bots from people without challenges" <https://arstechnica.com/gadgets/2017/03/googles-recaptcha-announces-invisible-background-captchas/> <sup>[[Archive.org]][1124]</sup>

[^357]: BlackHat Asia 2016, "I'm not a human: Breaking the Google reCAPTCHA", <https://www.blackhat.com/docs/asia-16/materials/asia-16-Sivakorn-Im-Not-a-Human-Breaking-the-Google-reCAPTCHA-wp.pdf> <sup>[[Archive.org]][1125]</sup>

[^358]: Google Blog <https://security.googleblog.com/2014/12/are-you-robot-introducing-no-captcha.html> <sup>[[Archive.org]][1126]</sup>

[^359]: Cloudflare Blog, Cloudflare supports Privacy Pass <https://blog.cloudflare.com/cloudflare-supports-privacy-pass/> <sup>[[Archive.org]][1127]</sup>

[^360]: Privacy International, Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws> <sup>[[Archive.org]][1046]</sup>

[^361]: Wikipedia, Device Fingerprinting <https://en.wikipedia.org/wiki/Device_fingerprint> <sup>[[Wikiless]][1128]</sup> <sup>[[Archive.org]][1129]</sup>

[^362]: Developers Google Blog,

    Guidance to developers affected by our effort to block less secure browsers and applications <https://developers.googleblog.com/2020/08/guidance-for-our-effort-to-block-less-secure-browser-and-apps.html> <sup>[[Archive.org]][1130]</sup>

[^363]: Google Help, Access age-restricted content & features <https://support.google.com/accounts/answer/10071085> <sup>[[Archive.org]][1131]</sup>

[^364]: Wikipedia, Dark Pattern <https://en.wikipedia.org/wiki/Dark_pattern> <sup>[[Wikiless]][1132]</sup> <sup>[[Archive.org]][1133]</sup>

[^365]: The Verge, Tinder will give you a verified blue check mark if you pass its catfishing test <https://www.theverge.com/2020/1/23/21077423/tinder-photo-verification-blue-checkmark-safety-center-launch-noonlight> <sup>[[Archive.org]][1134]</sup>

[^366]: DigitalInformationWorld, Facebook will now require you to Create a Video Selfie for Identity Verification <https://www.digitalinformationworld.com/2020/03/facebook-is-now-demanding-some-users-to-create-a-video-selfie-for-identity-verification.html> <sup>[[Archive.org]][1135]</sup>

[^367]: Vice.com, PornHub Announces 'Biometric Technology' to Verify Users <https://www.vice.com/en/article/m7a4eq/pornhub-new-verification-policy-biometric-id> <sup>[[Archive.org]][1136]</sup>

[^368]: Variety, China Launches Hotline to Report Online Comments That 'Distort' History or 'Deny' Its Cultural Excellence <https://variety.com/2021/digital/news/china-censorship-hotline-historical-nihilism-1234950554/> <sup>[[Archive.org]][1137]</sup>

[^369]: Wikipedia, Trust but verify <https://en.wikipedia.org/wiki/Trust,_but_verify> <sup>[[Wikiless]][741]</sup> <sup>[[Archive.org]][742]</sup>

[^370]: Wikipedia, Zero-trust Security Model <https://en.wikipedia.org/wiki/Zero_trust_security_model> <sup>[[Wikiless]][1138]</sup> <sup>[[Archive.org]][1139]</sup>

[^371]: Wikipedia, Espionage, Organization <https://en.wikipedia.org/wiki/Espionage#Organization> <sup>[[Wikiless]][1140]</sup> <sup>[[Archive.org]][1141]</sup>

[^372]: Medium.com, Kyle McDonald, How to recognize fake AI-generated images <https://kcimc.medium.com/how-to-recognize-fake-ai-generated-images-4d1f6f9a2842> <sup>[[Archive.org]][1142]</sup>

[^373]: Jayway Blog, Using ML to detect fake face images created by AI <https://blog.jayway.com/2020/03/06/using-ml-to-detect-fake-face-images-created-by-ai/> <sup>[[Archive.org]][1143]</sup>

[^374]: Wikipedia, Sim Swapping <https://en.wikipedia.org/wiki/SIM_swap_scam> <sup>[[Wikiless]][1144]</sup> <sup>[[Archive.org]][1145]</sup>

[^375]: Whonix Documentation, <https://www.whonix.org/wiki/Tor#Edit_Tor_Configuration> <sup>[[Archive.org]][1146]</sup>

[^376]: Tor Browser Documentation, <https://support.torproject.org/tbb/tbb-editing-torrc/> <sup>[[Archive.org]][1147]</sup>

[^377]: Facebook Onion Website <http://facebookwkhpilnemxj7asaniu7vnjjbiltxjqhye3mhbshg7kx5tfyd.onion/>

[^378]: Google Help <https://support.google.com/accounts/answer/114129?hl=en> <sup>[[Archive.org]][1148]</sup>

[^379]: Google Help <https://support.google.com/google-ads/answer/7474263?hl=en> <sup>[[Archive.org]][1149]</sup>

[^380]: Google, Your account is disabled <https://support.google.com/accounts/answer/40695> <sup>[[Archive.org]][1150]</sup>

[^381]: Google, Request to restore the account <https://support.google.com/accounts/contact/disabled2> <sup>[[Archive.org]][1151]</sup>

[^382]: Google Help, Update your account to meet age requirements <https://support.google.com/accounts/answer/1333913?hl=en> <sup>[[Archive.org]][1152]</sup>

[^383]: Jumio, ID verification features <https://www.jumio.com/features/> <sup>[[Archive.org]][1153]</sup>

[^384]: Privacytools.io Recommended E-mail Providers <https://privacytools.io/providers/email/> <sup>[[Archive.org]][1154]</sup>

[^385]: ProtonMail Human Verification System [https://ProtonMail.com/support/knowledge-base/human-verification/][] <sup>[[Archive.org]][1155]</sup>

[^386]: Twitter Appeal Form <https://help.twitter.com/forms/general>

[^387]: KnowYourMeme, Good Luck, I'm Behind 7 Proxies <https://knowyourmeme.com/memes/good-luck-im-behind-7-proxies> <sup>[[Archive.org]][1156]</sup>

[^388]: Wikipedia, end-to-end encryption, <https://en.wikipedia.org/wiki/End-to-end_encryption> <sup>[[Wikiless]][1157]</sup> <sup>[[Archive.org]][1158]</sup>

[^389]: Wikipedia, Forward Secrecy, <https://en.wikipedia.org/wiki/Forward_secrecy> <sup>[[Wikiless]][1159]</sup> <sup>[[Archive.org]][1160]</sup>

[^390]: Protonblog, What is zero-access encryption and why it is important for security <https://protonmail.com/blog/zero-access-encryption/> <sup>[[Archive.org]][1161]</sup>

[^391]: Wikipedia, Cambridge Analytica Scandal, <https://en.wikipedia.org/wiki/Facebook%E2%80%93Cambridge_Analytica_data_scandal> <sup>[[Wikiless]][1162]</sup> <sup>[[Archive.org]][1163]</sup>

[^392]: Signal Blog, Technology preview: Sealed sender for Signal <https://signal.org/blog/sealed-sender/> <sup>[[Archive.org]][1164]</sup>

[^393]: Signal Blog, Private Contact Discovery, <https://signal.org/blog/private-contact-discovery/> <sup>[[Archive.org]][1165]</sup>

[^394]: Signal Blog, Private Group System, <https://signal.org/blog/signal-private-group-system/> <sup>[[Archive.org]][1166]</sup>

[^395]: Privacytools.io, File-Sharing <https://privacytools.io/software/file-sharing/> <sup>[[Archive.org]][1167]</sup>

[^396]: Privacytools.io, Real-Time Communication <https://privacytools.io/software/real-time-communication/> <sup>[[Archive.org]][1168]</sup>

[^397]: GetSession.org, The Session Protocol: What's changing --- and why <https://getsession.org/session-protocol-explained/> <sup>[[Archive.org]][1169]</sup>

[^398]: Quarkslab, Audit of Session Secure Messaging Application <https://blog.quarkslab.com/audit-of-session-secure-messaging-application.html> <sup>[[Archive.org]][1170]</sup>

[^399]: Techlore, Top 5 BEST Messengers For Privacy <https://www.youtube.com/watch?v=aVwl892hqb4> <sup>[[Invidious]][1171]</sup>

[^400]: Praxis Films, Open Letter from Laura Poitras <https://www.praxisfilms.org/open-letter-from-laura-poitras/> <sup>[[Archive.org]][1172]</sup>

[^401]: Wikipedia, SecureDrop <https://en.wikipedia.org/wiki/SecureDrop> <sup>[[Wikiless]][1173]</sup> <sup>[[Archive.org]][1174]</sup>

[^402]: Wikipedia, TPM <https://en.wikipedia.org/wiki/Trusted_Platform_Module> <sup>[[Wikiless]][1175]</sup> <sup>[[Archive.org]][1176]</sup>

[^403]: Wikipedia, Pastebin <https://en.wikipedia.org/wiki/Pastebin> <sup>[[Wikiless]][1177]</sup> <sup>[[Archive.org]][1178]</sup>

[^404]: Wikipedia, Wear Leveling <https://en.wikipedia.org/wiki/Wear_leveling> <sup>[[Wikiless]][1179]</sup> <sup>[[Archive.org]][1180]</sup>

[^405]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Write_amplification#TRIM> <sup>[[Wikiless]][1181]</sup> <sup>[[Archive.org]][1182]</sup>

[^406]: Wikipedia, Write Amplification <https://en.wikipedia.org/wiki/Write_amplification> <sup>[[Wikiless]][1181]</sup> <sup>[[Archive.org]][1182]</sup>

[^407]: Wikipedia, Trim Disadvantages <https://en.wikipedia.org/wiki/Trim_(computing)#Disadvantages> <sup>[[Wikiless]][467]</sup> <sup>[[Archive.org]][468]</sup>

[^408]: Wikipedia, Garbage Collection <https://en.wikipedia.org/wiki/Write_amplification#Garbage_collection> <sup>[[Wikiless]][1181]</sup> <sup>[[Archive.org]][1182]</sup>

[^409]: Techgage, Too TRIM? When SSD Data Recovery is Impossible <https://techgage.com/article/too_trim_when_ssd_data_recovery_is_impossible/> <sup>[[Archive.org]][1183]</sup>

[^410]: ResearchGate, Live forensics method for acquisition on the Solid-State Drive (SSD) NVMe TRIM function <https://www.researchgate.net/publication/341761017_Live_forensics_method_for_acquisition_on_the_Solid_State_Drive_SSD_NVMe_TRIM_function> <sup>[[Archive.org]][1184]</sup>

[^411]: ElcomSoft, Life after Trim: Using Factory Access Mode for Imaging SSD Drives <https://blog.elcomsoft.com/2019/01/life-after-trim-using-factory-access-mode-for-imaging-ssd-drives/> <sup>[[Archive.org]][1185]</sup>

[^412]: Forensic Focus, Forensic Acquisition Of Solid State Drives With Open Source Tools <https://www.forensicfocus.com/articles/forensic-acquisition-of-solid-state-drives-with-open-source-tools/> <sup>[[Archive.org]][1186]</sup>

[^413]: ResearchGate, Solid State Drive Forensics: Where Do We Stand? <https://www.researchgate.net/publication/325976653_Solid_State_Drive_Forensics_Where_Do_We_Stand> <sup>[[Archive.org]][1187]</sup>

[^414]: Wikipedia, Parted Magic <https://en.wikipedia.org/wiki/Parted_Magic> <sup>[[Wikiless]][1188]</sup> <sup>[[Archive.org]][1189]</sup>

[^415]: Wikipedia, hdparm <https://en.wikipedia.org/wiki/Hdparm> <sup>[[Wikiless]][1190]</sup> <sup>[[Archive.org]][1191]</sup>

[^416]: GitHub, nvme-cli <https://github.com/linux-nvme/nvme-cli> <sup>[[Archive.org]][1192]</sup>

[^417]: PartedMagic Secure Erase, <https://partedmagic.com/secure-erase/> <sup>[[Archive.org]][1193]</sup>

[^418]: Partedmagic NVMe Secure Erase, <https://partedmagic.com/nvme-secure-erase/> <sup>[[Archive.org]][1194]</sup>

[^419]: UFSExplorer, Can I recover data from an encrypted storage? <https://www.ufsexplorer.com/solutions/data-recovery-on-encrypted-storage.php> <sup>[[Archive.org]][1195]</sup>

[^420]: Apple Developer Documentation, <https://developer.apple.com/library/archive/documentation/FileManagement/Conceptual/APFS_Guide/FAQ/FAQ.html> <sup>[[Archive.org]][1196]</sup>

[^421]: EFF, How to: Delete Your Data Securely on macOS <https://ssd.eff.org/en/module/how-delete-your-data-securely-macos> <sup>[[Archive.org]][490]</sup>

[^422]: Privacytools.io, Productivity tools <https://www.privacytools.io/software/productivity/> <sup>[[Archive.org]][1197]</sup>

[^423]: Whonix Documentation, Scrubbing Metadata <https://www.whonix.org/wiki/Metadata> <sup>[[Archive.org]][1198]</sup>

[^424]: Tails documentation, MAT <https://gitlab.tails.boum.org/tails/blueprints/-/wikis/doc/mat/> <sup>[[Archive.org]][1199]</sup>

[^425]: GitHub, Disable Gatekeeper on macOS Big Sur (11.x) <https://disable-gatekeeper.github.io/> <sup>[[Archive.org]][1200]</sup>

[^426]: DuckDuckGo help, Cache <https://help.duckduckgo.com/duckduckgo-help-pages/features/cache/> <sup>[[Archive.org]][1201]</sup>

[^427]: DuckDuckGo help, Sources <https://help.duckduckgo.com/duckduckgo-help-pages/results/sources/> <sup>[[Archive.org]][1202]</sup>

[^428]: Wikipedia, Dead Drop <https://en.wikipedia.org/wiki/Dead_drop> <sup>[[Wikiless]][1203]</sup> <sup>[[Archive.org]][1204]</sup>

[^429]: Wikipedia, Secure Communication Obfuscation <https://en.wikipedia.org/wiki/Obfuscation#Secure_communication> <sup>[[Wikiless]][1205]</sup> <sup>[[Archive.org]][1206]</sup>

[^430]: Wikipedia, Steganography <https://en.wikipedia.org/wiki/Steganography> <sup>[[Wikiless]][972]</sup> <sup>[[Archive.org]][973]</sup>

[^431]: Wikipedia, Kleptography <https://en.wikipedia.org/wiki/Kleptography> <sup>[[Wikiless]][1207]</sup> <sup>[[Archive.org]][1208]</sup>

[^432]: Wikipedia, Koalang <https://en.wikipedia.org/wiki/Koalang> <sup>[[Wikiless]][1209]</sup> <sup>[[Archive.org]][1210]</sup>

[^433]: Wikipedia, OPSEC <https://en.wikipedia.org/wiki/Operations_security> <sup>[[Wikiless]][1211]</sup> <sup>[[Archive.org]][1212]</sup>

[^434]: Quote Investigator, A Lie Can Travel Halfway Around the World While the Truth Is Putting On Its Shoes <https://quoteinvestigator.com/2014/07/13/truth/> <sup>[[Archive.org]][1213]</sup>

[^435]: Privacytools.io, Operating Systems <https://privacytools.io/operating-systems/> <sup>[[Archive.org]][1063]</sup>

[^436]: Brave Support, What is a Private Window with Tor? <https://support.brave.com/hc/en-us/articles/360018121491-What-is-a-Private-Window-with-Tor-> <sup>[[Archive.org]][1214]</sup>

[^437]: Medium.com, The Windows USN Journal <https://medium.com/velociraptor-ir/the-windows-usn-journal-f0c55c9010e> <sup>[[Archive.org]][1215]</sup>

[^438]: Medium.com, Digging into the System Resource Usage Monitor (SRUM) <https://medium.com/velociraptor-ir/digging-into-the-system-resource-usage-monitor-srum-afbadb1a375> <sup>[[Archive.org]][1216]</sup>

[^439]: SANS, Timestamped Registry & NTFS Artifacts from Unallocated Space <https://www.sans.org/blog/timestamped-registry-ntfs-artifacts-from-unallocated-space/> <sup>[[Archive.org]][1217]</sup>

[^440]: DBAN, <https://dban.org/> <sup>[[Archive.org]][1218]</sup>

[^441]: NYTimes, Lost Passwords Lock Millionaires Out of Their Bitcoin Fortunes <https://www.nytimes.com/2021/01/12/technology/bitcoin-passwords-wallets-fortunes.html> <sup>[[Archive.org]][1047]</sup>

[^442]: CrystalDiskInfo <https://crystalmark.info/en/software/crystaldiskinfo/> <sup>[[Archive.org]][1219]</sup>

[^443]: Wikipedia, Faraday Cage, <https://en.wikipedia.org/wiki/Faraday_cage> <sup>[[Wikiless]][1220]</sup> <sup>[[Archive.org]][1221]</sup>

[^444]: Edith Cowan University, A forensic examination of several mobile device Faraday bags & materials to test their effectiveness materials to test their effectiveness <https://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1165&context=adf> <sup>[[Archive.org]][1222]</sup>

[^445]: arXiv, Deep-Spying: Spying using Smartwatch and Deep Learning <https://arxiv.org/abs/1512.05616> <sup>[[Archive.org]][1223]</sup>

[^446]: Acm.org, Privacy Implications of Accelerometer Data: A Review of Possible Inferences <https://dl.acm.org/doi/pdf/10.1145/3309074.3309076> <sup>[[Archive.org]][1224]</sup>

[^447]: YouTube, Fingerprinting Paper - Forensic Education <https://www.youtube.com/watch?v=sO98kDLkh-M> <sup>[[Invidious]][1225]</sup>

[^448]: Wikipedia, Touch DNA, <https://en.wikipedia.org/wiki/Touch_DNA> <sup>[[Wikiless]][1226]</sup> <sup>[[Archive.org]][1227]</sup>

[^449]: TheDNAGuide, DNA from Postage Stamps or Hair Samples? Yeeesssss..... <https://www.yourdnaguide.com/ydgblog/dna-hair-samples-postage-stamps> <sup>[[Archive.org]][1228]</sup>

[^450]: GitHub, Mhinkie, OONI-Detection <https://github.com/mhinkie/ooni-detection> <sup>[[Archive.org]][1229]</sup>

[^451]: Wikipedia, File Verification <https://en.wikipedia.org/wiki/File_verification> <sup>[[Wikiless]][1230]</sup> <sup>[[Archive.org]][1231]</sup>

[^452]: Wikipedia, CRC <https://en.wikipedia.org/wiki/Cyclic_redundancy_check> <sup>[[Wikiless]][1232]</sup> <sup>[[Archive.org]][1233]</sup>

[^453]: Wikipedia, MD5 <https://en.wikipedia.org/wiki/MD5> <sup>[[Wikiless]][1234]</sup> <sup>[[Archive.org]][1235]</sup>

[^454]: Wikipedia, MD5 Security <https://en.wikipedia.org/wiki/MD5#Security> <sup>[[Wikiless]][1234]</sup> <sup>[[Archive.org]][1235]</sup>

[^455]: Wikipedia, Collisions <https://en.wikipedia.org/wiki/Collision_(computer_science)> <sup>[[Wikiless]][1236]</sup> <sup>[[Archive.org]][1237]</sup>

[^456]: Wikipedia, SHA <https://en.wikipedia.org/wiki/Secure_Hash_Algorithms> <sup>[[Wikiless]][1238]</sup> <sup>[[Archive.org]][1239]</sup>

[^457]: Wikipedia, SHA-2 <https://en.wikipedia.org/wiki/SHA-2> <sup>[[Wikiless]][1240]</sup> <sup>[[Archive.org]][1241]</sup>

[^458]: Wikipedia, Collision Resistance <https://en.wikipedia.org/wiki/Collision_resistance> <sup>[[Wikiless]][1242]</sup> <sup>[[Archive.org]][1243]</sup>

[^459]: GnuPG Gpg4win Wiki, Check integrity of Gpg4win packages <https://wiki.gnupg.org/Gpg4win/CheckIntegrity> <sup>[[Archive.org]][1244]</sup>

[^460]: Medium.com, How to verify checksum on Mac <https://medium.com/@EvgeniIvanov/how-to-verify-checksum-on-mac-988f166b0c4f> <sup>[[Archive.org]][1245]</sup>

[^461]: Wikipedia, GPG <https://en.wikipedia.org/wiki/GNU_Privacy_Guard> <sup>[[Wikiless]][1246]</sup> <sup>[[Archive.org]][1247]</sup>

[^462]: Wikipedia, Public-Key Cryptography <https://en.wikipedia.org/wiki/Public-key_cryptography> <sup>[[Wikiless]][1248]</sup> <sup>[[Archive.org]][1249]</sup>

[^463]: Wikipedia, Polymorphic Code <https://en.wikipedia.org/wiki/Polymorphic_code> <sup>[[Wikiless]][1250]</sup> <sup>[[Archive.org]][1251]</sup>

[^464]: Whonix Documentation, Use of AV, <https://www.whonix.org/wiki/Malware_and_Firmware_Trojans#The_Utility_of_Antivirus_Tools> <sup>[[Archive.org]][1252]</sup>

[^465]: Whonix Forums, <https://forums.whonix.org/t/installation-of-antivirus-scanners-by-default/9755/8> <sup>[[Archive.org]][1253]</sup>

[^466]: AV-Test Security Report 2018-2019, <https://www.av-test.org/fileadmin/pdf/security_report/AV-TEST_Security_Report_2018-2019.pdf> <sup>[[Archive.org]][1254]</sup>

[^467]: ZDNet, ESET discovers 21 new Linux malware families <https://www.zdnet.com/article/eset-discovers-21-new-linux-malware-families/> <sup>[[Archive.org]][1255]</sup>

[^468]: NakeSecurity, EvilGnome -- Linux malware aimed at your desktop, not your servers <https://nakedsecurity.sophos.com/2019/07/25/evilgnome-linux-malware-aimed-at-your-laptop-not-your-servers/> <sup>[[Archive.org]][1256]</sup>

[^469]: Immunify, HiddenWasp: How to detect malware hidden on Linux & IoT <https://blog.imunify360.com/hiddenwasp-how-to-detect-malware-hidden-on-linux-iot> <sup>[[Archive.org]][1257]</sup>

[^470]: Wikipedia, Linux Malware <https://en.wikipedia.org/wiki/Linux_malware> <sup>[[Wikiless]][1258]</sup> <sup>[[Archive.org]][1259]</sup>

[^471]: Wikipedia, macOS Malware [https://en.wikipedia.org/wiki/macOS_malware][] <sup>[[Wikiless]][1260]</sup> <sup>[[Archive.org]][1261]</sup>

[^472]: MacWorld, List of Mac viruses, malware and security flaws <https://www.macworld.co.uk/feature/mac-viruses-list-3668354/> <sup>[[Archive.org]][1262]</sup>

[^473]: JAMF, The Mac Malware of 2020 <https://resources.jamf.com/documents/macmalware-2020.pdf> <sup>[[Archive.org]][1263]</sup>

[^474]: macOS Security and Privacy Guide, <https://github.com/drduh/macOS-Security-and-Privacy-Guide#viruses-and-malware> <sup>[[Archive.org]][288]</sup>

[^475]: ImageTragick.com, <https://imagetragick.com/> <sup>[[Archive.org]][1264]</sup>

[^476]: Oracle Virtualbox Documentation, <https://docs.oracle.com/en/virtualization/virtualbox/6.0/admin/hyperv-support.html> <sup>[[Archive.org]][1265]</sup>

[^477]: Oracle Virtualbox Documentation, <https://docs.oracle.com/en/virtualization/virtualbox/6.0/admin/hyperv-support.html> <sup>[[Archive.org]][1265]</sup>

[^478]: Lenny Zeltser, Analyzing Malicious Documents Cheat Sheet <https://zeltser.com/analyzing-malicious-documents/> <sup>[[Archive.org]][1266]</sup>

[^479]: Wikipedia, Portable Applications <https://en.wikipedia.org/wiki/Portable_application> <sup>[[Wikiless]][1267]</sup> <sup>[[Archive.org]][1268]</sup>

[^480]: Brave Help, What is a Private Window with Tor Connectivity? <https://support.brave.com/hc/en-us/articles/360018121491-What-is-a-Private-Window-with-Tor-> <sup>[[Archive.org]][1214]</sup>

[^481]: BlackGNU, <https://ebin.city/~werwolf/posts/brave-is-shit/> <sup>[[Archive.org]][1269]</sup>

[^482]: Brave Help Center, <https://support.brave.com/hc/en-us/articles/360022973471-What-is-Shields> <sup>[[Archive.org]][1270]</sup>

[^483]: VentureBeat, Browser benchmark battle January 2020: Chrome vs. Firefox vs. Edge vs. Brave <https://venturebeat.com/2020/01/15/browser-benchmark-battle-january-2020-chrome-firefox-edge-brave/view-all/> <sup>[[Archive.org]][1271]</sup>

[^484]: Brave.com, Brave, Fingerprinting, and Privacy Budgets <https://brave.com/brave-fingerprinting-and-privacy-budgets/> <sup>[[Archive.org]][156]</sup>

[^485]: Madaidan Insecurities, Firefox and Chromium <https://madaidans-insecurities.github.io/firefox-chromium.html> <sup>[[Archive.org]][1272]</sup>

[^486]: GrapheneOS, Web Browsing <https://grapheneos.org/usage#web-browsing> <sup>[[Archive.org]][1273]</sup>

[^487]: ResearchGate, Web Browser Privacy: What Do Browsers Say When They Phone Home? <https://www.researchgate.net/publication/349979628_Web_Browser_Privacy_What_Do_Browsers_Say_When_They_Phone_Home> <sup>[[Archive.org]][1274]</sup>

[^488]: Madaidan Insecurities, Firefox and Chromium <https://madaidans-insecurities.github.io/firefox-chromium.html> <sup>[[Archive.org]][1272]</sup>

[^489]: GrapheneOS, Web Browsing <https://grapheneos.org/usage#web-browsing> <sup>[[Archive.org]][1273]</sup>

[^490]: Madaidan Insecurities, Firefox and Chromium <https://madaidans-insecurities.github.io/firefox-chromium.html> <sup>[[Archive.org]][1272]</sup>

[^491]: GrapheneOS, Web Browsing <https://grapheneos.org/usage#web-browsing> <sup>[[Archive.org]][1273]</sup>

[^492]: Microsoft.com, Microsoft Edge support for Microsoft Defender Application Guard <https://docs.microsoft.com/en-us/deployedge/microsoft-edge-security-windows-defender-application-guard> <sup>[[Archive.org]][1275]</sup>

[^493]: PcMag, Mozilla Signs Lucrative 3-Year Google Search Deal for Firefox <https://www.pcmag.com/news/mozilla-signs-lucrative-3-year-google-search-deal-for-firefox> <sup>[[Archive.org]][1276]</sup>

[^494]: Madaidan Insecurities, Firefox and Chromium <https://madaidans-insecurities.github.io/firefox-chromium.html> <sup>[[Archive.org]][1272]</sup>

[^495]: Wikipedia, Virtualization <https://en.wikipedia.org/wiki/Virtualization> <sup>[[Wikiless]][1277]</sup> <sup>[[Archive.org]][1278]</sup>

[^496]: Tor Project, Project Snowflake <https://snowflake.torproject.org/> <sup>[[Archive.org]][542]</sup>

[^497]: GitHub, Obfs4 Repository <https://github.com/Yawning/obfs4/> <sup>[[Archive.org]][1279]</sup>

[^498]: Tor Browser Manual, Pluggable Transport <https://tb-manual.torproject.org/circumvention/> <sup>[[Archive.org]][1280]</sup>

[^499]: Tor Browser Manual, Pluggable Transport <https://tb-manual.torproject.org/circumvention/> <sup>[[Archive.org]][1280]</sup>

[^500]: Wikipedia, Domain Fronting <https://en.wikipedia.org/wiki/Domain_fronting> <sup>[[Wikiless]][1281]</sup> <sup>[[Archive.org]][1282]</sup>

[^501]: GitLab, Tor Browser Issues, Add uBlock Origin to the Tor Browser <https://gitlab.torproject.org/tpo/applications/tor-browser/-/issues/17569> <sup>[[Archive.org]][1283]</sup>

[^502]: Europol Wasabi Wallet Report, <https://www.tbstat.com/wp/uploads/2020/06/Europol-Wasabi-Wallet-Report.pdf> <sup>[[Archive.org]][1284]</sup>

[^503]: NIST, <https://www.sans.org/blog/nist-has-spoken-death-to-complexity-long-live-the-passphrase/> <sup>[[Archive.org]][1285]</sup>

[^504]: ZDnet, FBI recommends passphrases over password complexity <https://www.zdnet.com/article/fbi-recommends-passphrases-over-password-complexity/> <sup>[[Archive.org]][1286]</sup>

[^505]: The Intercept, Passphrases That You Can Memorize --- But That Even the NSA Can't Guess <https://theintercept.com/2015/03/26/passphrases-can-memorize-attackers-cant-guess/> <sup>[[Tor Mirror]][1287]</sup> <sup>[[Archive.org]][1288]</sup>

[^506]: ProtonMail Blog, Let's settle the password vs. passphrase debate once and for all <https://protonmail.com/blog/protonmail-com-blog-password-vs-passphrase/> <sup>[[Archive.org]][1289]</sup>

[^507]: YouTube, Edward Snowden on Passwords: Last Week Tonight with John Oliver (HBO) <https://www.youtube.com/watch?v=yzGzB-yYKcc> <sup>[[Invidious]][1290]</sup>

[^508]: YouTube, How to Choose a Password -- Computerphile <https://www.youtube.com/watch?v=3NjQ9b3pgIg> <sup>[[Invidious]][674]</sup>

[^509]: Wikipedia, Passphrase <https://en.wikipedia.org/wiki/Passphrase#Passphrase_selection> <sup>[[Wikiless]][1291]</sup> <sup>[[Archive.org]][1292]</sup>

[^510]: Monero Research Lab, Evaluating cryptocurrency security and privacy in a post-quantum world <https://github.com/insight-decentralized-consensus-lab/post-quantum-monero/blob/master/writeups/technical_note.pdf> <sup>[[Archive.org]][1293]</sup>
  ...
  [143]: https://web.archive.org/web/https://medium.com/@somdevsangwan/unblurring-images-for-osint-and-more-part-1-5ee36db6a70b
  [144]: https://web.archive.org/web/https://medium.com/@somdevsangwan/deblurring-images-for-osint-part-2-ba564af8eb5d
  [145]: media/image15.jpeg 
  [146]: https://web.archive.org/web/https://github.com/subeeshvasu/Awesome-Deblurring
  [147]: https://web.archive.org/web/https://www.myheritage.com/photo-enhancer
  [148]: media/image16.jpeg 
  [149]: https://web.archive.org/web/https://bitcoin.org/en/you-need-to-know
  [150]: https://web.archive.org/web/https://bitcoin.org/en/protect-your-privacy
  [151]: https://web.archive.org/web/https://support.apple.com/en-us/HT202303
  [152]: https://web.archive.org/web/https://faq.whatsapp.com/android/chats/about-google-drive-backups/
  [153]: https://web.archive.org/web/https://www.dropbox.com/privacy
  [154]: https://web.archive.org/web/https://privacy.microsoft.com/en-us/privacystatement
  [155]: https://web.archive.org/web/https://amiunique.org/links
  [156]: https://web.archive.org/web/https://brave.com/brave-fingerprinting-and-privacy-budgets/
  [157]: https://web.archive.org/web/https://palant.info/2020/12/10/how-anti-fingerprinting-extensions-tend-to-make-fingerprinting-easier/
  [158]: https://web.archive.org/web/https://www.upturn.org/reports/2020/mass-extraction/
  [159]: https://web.archive.org/web/https://www.nytimes.com/2020/10/21/technology/iphone-encryption-police.html
  [160]: https://web.archive.org/web/https://www.vice.com/en/article/vbxxxd/unlock-iphone-ios11-graykey-grayshift-police
  [161]: https://web.archive.org/web/http://encase-docs.opentext.com/documentation/encase/forensic/8.07/Content/Resources/External%20Files/EnCase%20Forensic%20v8.07%20User%20Guide.pdf
  [162]: https://web.archive.org/web/https://accessdata.com/products-services/forensic-toolkit-ftk
  [163]: https://web.archive.org/web/https://latacora.micro.blog/2018/04/03/cryptographic-right-answers.html
  ...
  [186]: https://web.archive.org/web/http://www.gcomtech.com/ccp0-prodshow/laser-surveillance-laser-listening.html
  [187]: https://yewtu.be/watch?v=YKRtFgunyj4
  [188]: https://web.archive.org/web/https://www.whonix.org/wiki/Data_Collection_Techniques
  [189]: https://web.archive.org/web/https://tosdr.org/
  [190]: https://web.archive.org/web/https://www.eff.org/issues/privacy
  [191]: https://wikiless.org/wiki/List_of_government_mass_surveillance_projects
  [192]: https://web.archive.org/web/https://en.wikipedia.org/wiki/List_of_government_mass_surveillance_projects
  [193]: https://web.archive.org/web/https://www.gwern.net/Death-Note-Anonymity
  ...
  [220]: http://cryptonx6nsmspsnpicuihgmbbz3qvro4na35od3eht4vojdo7glm6yd.onion
  ...
  [225]: https://web.archive.org/web/https://smspva.com/
  [226]: https://web.archive.org/web/https://cryptwerk.com/companies/sms/xmr/
  [227]: https://web.archive.org/web/https://syscall.eu/blog/2018/03/12/aigo_part1/
  [228]: https://web.archive.org/web/https://www.torproject.org/
  [229]: https://yewtu.be/watch?v=JWII85UlzKw
  [230]: https://web.archive.org/web/https://2019.www.torproject.org/about/overview.html.en
  [231]: https://web.archive.org/web/https://support.torproject.org/tormobile/tormobile-7/
  [232]: https://web.archive.org/web/https://tb-manual.torproject.org/security-settings/
  [233]: https://web.archive.org/web/https://tails.boum.org/doc/about/warnings/index.en.html
  [234]: https://web.archive.org/web/https://tails.boum.org/install/index.en.html
  [235]: https://web.archive.org/web/https://tails.boum.org/doc/anonymous_internet/tor/index.en.html
  [236]: https://web.archive.org/web/https://2019.www.torproject.org/docs/bridges
  [237]: https://web.archive.org/web/https://github.com/aforensics/HiddenVM
  [238]: media/image19.jpeg 
  [Tor over VPN]: #tor-over-vpn
  [239]: https://web.archive.org/web/https://www.whonix.org/wiki/Whonix-Host
  [240]: https://web.archive.org/web/https://defuse.ca/truecrypt-plausible-deniability-useless-by-game-theory.htm
  [241]: https://wikiless.org/wiki/Rubber-hose_cryptanalysis
  [242]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Rubber-hose_cryptanalysis
  [243]: https://web.archive.org/web/https://github.com/aforensics/HiddenVM/releases
  [244]: https://web.archive.org/web/https://www.whonix.org/wiki/VirtualBox/XFCE
  [245]: https://web.archive.org/web/https://puri.sm/
  [246]: https://web.archive.org/web/https://system76.com/
  [247]: https://web.archive.org/web/https://freundschafter.com/research/system-alternatives-without-intel-me-iamt-and-amd-psp-secure-technology/
  [248]: https://web.archive.org/web/https://libreboot.org/docs/hardware/
  [249]: https://web.archive.org/web/https://coreboot.org/status/board-status.html
  [250]: https://web.archive.org/web/https://store.hp.com/us/en/tech-takes/how-to-enter-bios-setup-windows-pcs
  [251]: https://yewtu.be/watch?v=QDSlWa9xQuA
  [252]: https://yewtu.be/watch?v=0fZdL3ufVOI
  [253]: https://web.archive.org/web/https://support.apple.com/en-au/HT204455
  [254]: https://web.archive.org/web/https://support.apple.com/en-gb/guide/security/sec28382c9ca/web
  [255]: https://web.archive.org/web/https://mullvad.net/en/help/how-tamper-protect-laptop/
  [256]: media/image20.jpeg
  [257]: media/image21.jpeg
  [258]: https://web.archive.org/web/https://www.whonix.org/wiki/Cold_Boot_Attack_Defense
  [259]: https://web.archive.org/web/https://www.whonix.org/wiki/Protection_Against_Physical_Attacks
  [260]: https://web.archive.org/web/https://github.com/0xPoly/Centry
  [261]: https://web.archive.org/web/https://github.com/AnonymousPlanet/Centry
  [262]: https://web.archive.org/web/https://github.com/hephaest0s/usbkill
  [263]: https://web.archive.org/web/https://github.com/Lvl4Sword/Killer
  [264]: https://web.archive.org/web/https://askubuntu.com/questions/153245/how-to-wipe-ram-on-shutdown-prevent-cold-boot-attacks
  [265]: https://web.archive.org/web/https://github.com/QubesOS/qubes-antievilmaid
  [266]: https://web.archive.org/web/https://www.sans.org/security-resources/posters/windows-forensic-analysis/170/download
  [267]: https://web.archive.org/web/https://www.whonix.org/wiki/Full_Disk_Encryption
  [268]: https://web.archive.org/web/https://madaidans-insecurities.github.io/linux.html
  [269]: https://web.archive.org/web/https://ubuntu.com/tutorials/install-ubuntu-desktop
  [270]: https://web.archive.org/web/https://help.ubuntu.com/community/ManualFullSystemEncryption
  [The Detached Headers Way]: #the-detached-headers-way
  [The Veracrypt Way]: #the-veracrypt-way
  [271]: https://web.archive.org/web/https://wiki.archlinux.org/title/Dm-crypt/Specialties#Encrypted_system_using_a_detached_LUKS_header
  [272]: https://web.archive.org/web/https://vitux.com/how-to-force-ubuntu-to-stop-collecting-your-data-from-your-pc/
  [273]: https://web.archive.org/web/https://www.addictivetips.com/ubuntu-linux-tips/disable-bluetooth-in-ubuntu/
  [274]: https://web.archive.org/web/https://www.linuxuprising.com/2019/07/how-to-completely-disable-tracker.html
  [275]: https://web.archive.org/web/https://www.how2shout.com/linux/how-to-hibernate-ubuntu-20-04-lts-focal-fossa/
  [276]: https://web.archive.org/web/http://www.lorenzobettini.it/2020/07/enabling-hibernation-on-ubuntu-20-04/
  [277]: https://web.archive.org/web/https://help.ubuntu.com/16.04/ubuntu-help/power-hibernate.html
  [278]: https://web.archive.org/web/http://ubuntuhandbook.org/index.php/2020/05/lid-close-behavior-ubuntu-20-04/
  [279]: https://web.archive.org/web/https://tipsonubuntu.com/2018/04/28/change-lid-close-action-ubuntu-18-04-lts/
  [280]: https://web.archive.org/web/https://help.ubuntu.com/community/EnableHibernateWithEncryptedSwap
  [281]: https://web.archive.org/web/https://help.ubuntu.com/community/AnonymizingNetworkMACAddresses
  [282]: https://web.archive.org/web/https://josh.works/shell-script-basics-change-mac-address
  [283]: https://yewtu.be/watch?v=Sa0KqbpLye4
  [284]: https://web.archive.org/web/https://madaidans-insecurities.github.io/guides/linux-hardening.html
  [285]: https://web.archive.org/web/https://wiki.archlinux.org/title/Security
  [286]: https://web.archive.org/web/https://codeberg.org/SalamanderSecurity/PARSEC
  [287]: https://yewtu.be/watch?v=lFx5icuE6Io
  [288]: https://web.archive.org/web/https://github.com/drduh/macOS-Security-and-Privacy-Guide
  [289]: https://web.archive.org/web/https://support.apple.com/en-us/HT204455
  [290]: https://web.archive.org/web/https://sneak.berlin/20201112/your-computer-isnt-yours/
  [291]: https://web.archive.org/web/https://blog.jacopo.io/en/post/apple-ocsp/
  [292]: https://yewtu.be/watch?v=vNRics7tlqw
  [293]: https://web.archive.org/web/https://technitium.com/tmac/
  [294]: https://web.archive.org/web/https://www.veracrypt.fr/en/Downloads.html
  ...
  [295]: https://web.archive.org/web/https://support.microsoft.com/en-us/windows/turn-on-device-encryption-0c453637-bc88-5f74-5105-741561aae838
  [296]: https://web.archive.org/web/https://docs.microsoft.com/en-us/troubleshoot/windows-client/deployment/disable-and-re-enable-hibernation
  [297]: https://web.archive.org/web/https://www.veracrypt.fr/en/VeraCrypt%20Hidden%20Operating%20System.html
  [298]: https://web.archive.org/web/https://www.veracrypt.fr/en/Security%20Requirements%20for%20Hidden%20Volumes.html
  [299]: media/image22.jpeg
  [300]: #_Appendix_C:_Windows
  [301]: https://web.archive.org/web/https://www.veracrypt.fr/en/Protection%20of%20Hidden%20Volumes.html
  [302]: https://web.archive.org/web/https://www.whonix.org/wiki/KVM
  [303]: https://web.archive.org/web/https://www.whonix.org/wiki/KVM#Why_Use_KVM_Over_VirtualBox.3F
  [304]: media/image23.jpeg 
  [305]: media/image24.jpeg 
  [306]: https://web.archive.org/web/https://gitlab.torproject.org/legacy/trac/-/wikis/org/doc/ListOfServicesBlockingTor
  [307]: media/image25.jpeg 
  [308]: media/image26.jpeg 
  [309]: https://web.archive.org/web/https://stakey.club/en/decred-via-tor-network/
  [310]: https://web.archive.org/web/https://www.whonix.org/wiki/Stream_Isolation
  [311]: https://web.archive.org/web/https://tails.boum.org/contribute/design/stream_isolation/
  [312]: https://web.archive.org/web/https://www.whonix.org/wiki/Tunnels/Introduction#Comparison_Table
  [313]: https://web.archive.org/web/https://www.whonix.org/wiki/Tunnels/Connecting_to_a_VPN_before_Tor
  [314]: https://web.archive.org/web/https://www.whonix.org/wiki/Comparison_Of_Tor_with_CGI_Proxies,_Proxy_Chains,_and_VPN_Services
  [315]: https://web.archive.org/web/https://www.whonix.org/wiki/Why_does_Whonix_use_Tor
  [316]: https://web.archive.org/web/https://www.researchgate.net/publication/324251041_Anonymity_communication_VPN_and_Tor_a_comparative_study
  [317]: https://web.archive.org/web/https://gist.github.com/joepie91/5a9909939e6ce7d09e29
  [318]: https://web.archive.org/web/https://schub.wtf/blog/2019/04/08/very-precarious-narrative.html
  [Adversaries (threats)]: #threats
  [319]: https://web.archive.org/web/https://gitlab.torproject.org/legacy/trac/-/wikis/doc/TorPlusVPN
  [320]: https://web.archive.org/web/https://gitlab.tails.boum.org/tails/blueprints/-/wikis/vpn_support/
  [321]: https://web.archive.org/web/https://tails.boum.org/support/faq/index.en.html
  [322]: https://web.archive.org/web/https://www.whonix.org/wiki/Tunnels/Introduction
  [323]: https://web.archive.org/web/https://www.whonix.org/wiki/Tunnels/Connecting_to_Tor_before_a_VPN
  [324]: media/image27.jpeg 
  [325]: media/image28.jpeg 
  [326]: https://web.archive.org/web/https://www.virtualbox.org/wiki/Downloads
  [327]: https://web.archive.org/web/https://www.whonix.org/wiki/Download
  [328]: https://web.archive.org/web/https://www.whonix.org/wiki/Virtualization_Platform_Security
  [329]: https://web.archive.org/web/https://www.whonix.org/wiki/Network_Time_Synchronization
  [330]: https://web.archive.org/web/https://www.virtualbox.org/manual/ch13.html
  [331]: https://web.archive.org/web/https://www.whonix.org/wiki/Bridges
  [332]: https://web.archive.org/web/https://www.whonix.org/wiki/Operating_System_Software_and_Updates
  [333]: https://web.archive.org/web/https://www.whonix.org/wiki/DoNot
  [334]: https://web.archive.org/web/https://www.whonix.org/wiki/Documentation
  [335]: https://web.archive.org/web/https://www.whonix.org/wiki/Install_Software
  [336]: https://web.archive.org/web/https://www.whonix.org/wiki/Anti-Forensics_Precautions
  [Virtualbox Hardening recommendations]: #virtualbox-hardening-recommendations
  [337]: https://web.archive.org/web/https://www.whonix.org/wiki/AppArmor
  [338]: https://web.archive.org/web/https://www.whonix.org/wiki/VM_Fingerprinting
  [339]: https://web.archive.org/web/https://www.whonix.org/wiki/Other_Operating_Systems
  [Hardening Linux]: #hardening-linux
  [340]: https://web.archive.org/web/https://ameliorated.info/
  [341]: https://web.archive.org/web/https://www.android-x86.org/documentation/virtualbox.html
  [342]: https://web.archive.org/web/https://www.wikigain.com/install-macos-catalina-on-virtualbox-on-windows/
  [343]: https://web.archive.org/web/https://www.wikigain.com/how-to-install-macos-big-sur-on-virtualbox-on-windows-pc/
  [344]: https://web.archive.org/web/https://github.com/myspaghetti/macos-virtualbox
  [Hardening macOS]: #hardening-macos
  [https://www.whonix.org/wiki/KeePassXC]: https://www.whonix.org/wiki/Keepassxc
  [345]: https://web.archive.org/web/https://www.whonix.org/wiki/Keepassxc
  [346]: https://web.archive.org/web/https://keepassxc.org/download/
  [347]: https://web.archive.org/web/https://keepassxc.org/docs/KeePassXC_GettingStarted.html
  ...
  [348]: media/image29.jpeg 
  [349]: media/image30.jpeg 
  [350]: media/image31.jpeg 
  [351]: https://web.archive.org/web/https://www.qubes-os.org/intro/
  [352]: https://web.archive.org/web/https://www.qubes-os.org/video-tours/
  [353]: https://web.archive.org/web/https://www.qubes-os.org/doc/getting-started/
  [354]: https://yewtu.be/watch?v=8cU4hQg6GvU
  [355]: https://yewtu.be/watch?v=sbN5Bz3v-uA
  [356]: https://yewtu.be/watch?v=YPAvoFsvSbg
  [357]: https://web.archive.org/web/https://www.qubes-os.org/hcl/
  [358]: media/image32.jpeg 
  [359]: media/image33.jpeg
  [Note about plausible deniability on Linux]: #note-about-plausible-deniability-on-linux
  [360]: https://web.archive.org/web/https://www.qubes-os.org/doc/installation-guide/
  [361]: https://web.archive.org/web/https://www.qubes-os.org/faq/
  [362]: https://web.archive.org/web/https://github.com/Qubes-Community/Contents/blob/master/docs/privacy/anonymizing-your-mac-address.md
  [363]: https://web.archive.org/web/https://wiki.debian.org/AppArmor
  [364]: https://web.archive.org/web/https://wiki.archlinux.org/title/AppArmor
  [365]: https://web.archive.org/web/https://www.whonix.org/wiki/Qubes/AppArmor
  [366]: https://yewtu.be/watch?v=_WOKRaM-HI4
  [367]: https://web.archive.org/web/https://docs.fedoraproject.org/en-US/quick-docs/getting-started-with-selinux/
  [368]: https://web.archive.org/web/https://github.com/Qubes-Community/Contents/blob/master/docs/configuration/vpn.md
  [369]: https://web.archive.org/web/https://check.torproject.org/
  [370]: https://web.archive.org/web/https://linuxconfig.org/how-to-create-a-vpn-killswitch-using-iptables-on-linux
  [371]: https://web.archive.org/web/https://mullvad.net/en/check/
  [372]: https://web.archive.org/web/https://www.ivpn.net/
  [373]: https://web.archive.org/web/https://protonvpn.com/support/vpn-ip-change/
  [374]: https://web.archive.org/web/https://www.whonix.org/wiki/Qubes/DisposableVM
  [375]: https://web.archive.org/web/https://brave.com/linux/
  [376]: https://web.archive.org/web/https://github.com/anbox/anbox-modules
  [377]: https://web.archive.org/web/https://github.com/anbox/anbox/blob/master/docs/install.md
  [378]: media/image34.jpeg 
  [379]: media/image35.jpeg 
  [380]: https://web.archive.org/web/https://github.com/dessant/buster
  [381]: https://web.archive.org/web/https://www.hcaptcha.com/accessibility
  [382]: https://web.archive.org/web/https://privacypass.github.io/
  [383]: http://vww6ybal4bd7szmgncyruucpgfkqahzddi37ktceo3ah7ngmcopnpyyd.onion/
  [384]: media/image36.jpeg
  [385]: media/image37.jpeg
  [386]: media/image38.jpeg
  [387]: https://web.archive.org/web/https://github.com/deepfakes/faceswap
  [Online Phone Number (less recommended)]: #online-phone-number
  [388]: https://web.archive.org/web/https://thispersondoesnotexist.com/
  [389]: https://web.archive.org/web/https://github.com/NVlabs/stylegan2
  [390]: https://web.archive.org/web/https://www.myheritage.com/deep-nostalgia
  [391]: media/image39.jpeg 
  [392]: https://web.archive.org/web/https://github.com/AliaksandrSiarohin/first-order-model
  [393]: https://web.archive.org/web/https://gendersec.tacticaltech.org/wiki/index.php/Complete_manual
  [394]: https://web.archive.org/web/https://www.bigdatacloud.com/insights/tor-exit-nodes
  [395]: https://wikiless.org/wiki/Facebook_real-name_policy_controversy
  [396]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Facebook_real-name_policy_controversy
  [397]: https://web.archive.org/web/https://slate.com/technology/2018/02/why-some-americans-are-cheering-germany-for-taking-on-facebooks-real-name-policy.html
  [398]: https://web.archive.org/web/https://www.theverge.com/2018/2/12/17005746/facebook-real-name-policy-illegal-german-court-rules
  [399]: https://web.archive.org/web/https://www.pcmag.com/news/german-court-rules-facebooks-real-name-policy-is-illegal
  [400]: https://web.archive.org/web/https://www.vzbv.de/sites/default/files/downloads/2018/02/14/18-02-12_vzbv_pm_facebook-urteil_en.pdf
  [401]: https://web.archive.org/web/https://www.reuters.com/article/us-germany-facebook/german-court-rules-facebook-use-of-personal-data-illegal-idUSKBN1FW1FI
  [402]: https://wikiless.org/wiki/Real-name_system
  [403]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Real-name_system
  [404]: https://web.archive.org/web/https://privacytools.io/
  [405]: https://web.archive.org/web/https://www.whonix.org/wiki/E-Mail#Anonymity_Friendly_Email_Provider_List
  [406]: https://web.archive.org/web/https://www.amazon.com/gp/help/customer/display.html?nodeId=202140280
  [407]: https://web.archive.org/web/https://www.apple.com/legal/internet-services/icloud/en/terms.html
  [408]: https://web.archive.org/web/https://www.binance.com/en/terms
  [409]: https://web.archive.org/web/https://briarproject.org/privacy-policy/
  [410]: https://web.archive.org/web/https://discord.com/terms
  [411]: https://web.archive.org/web/https://element.io/terms-of-service
  [412]: https://web.archive.org/web/https://www.facebook.com/terms.php
  [413]: https://web.archive.org/web/https://docs.github.com/en/free-pro-team@latest/github/site-policy/github-terms-of-service
  [414]: https://web.archive.org/web/https://about.gitlab.com/handbook/legal/subscription-agreement/
  [415]: https://web.archive.org/web/https://policies.google.com/terms
  [416]: https://web.archive.org/web/https://www.ycombinator.com/legal/
  [417]: https://web.archive.org/web/https://help.instagram.com/581066165581870?ref=dp
  [418]: https://web.archive.org/web/https://jami.net/privacy-policy/
  [419]: https://web.archive.org/web/https://www.ivpn.net/tos/
  [420]: https://web.archive.org/web/https://www.kraken.com/legal
  ...
  [426]: https://web.archive.org/web/https://onlyfans.com/terms
  [427]: https://web.archive.org/web/https://protonmail.com/terms-and-conditions
  [428]: https://web.archive.org/web/https://protonvpn.com/terms-and-conditions
  [429]: https://web.archive.org/web/https://www.redditinc.com/policies
  [430]: https://web.archive.org/web/https://slashdotmedia.com/terms-of-use/
  [431]: https://web.archive.org/web/https://telegram.org/tos
  [432]: https://web.archive.org/web/mailto:recover@telegram.org
  [433]: https://web.archive.org/web/https://tutanota.com/terms/
  [434]: https://web.archive.org/web/https://www.twitch.tv/p/en/legal/terms-of-service/
  [435]: https://web.archive.org/web/https://www.whatsapp.com/legal/updates/terms-of-service-eea
  [436]: media/image40.jpeg
  [437]: https://web.archive.org/web/https://www.bellingcat.com/resources/how-tos/2018/08/23/creating-android-open-source-research-device-pc/
  [438]: https://yewtu.be/watch?v=zSQtyW_ywZc
  [439]: https://wikiless.org/wiki/Comparison_of_instant_messaging_protocols
  [440]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Comparison_of_instant_messaging_protocols
  [441]: https://wikiless.org/wiki/Comparison_of_cross-platform_instant_messaging_clients
  [442]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Comparison_of_cross-platform_instant_messaging_clients
  [443]: https://web.archive.org/web/https://www.securemessagingapps.com/
  [444]: https://web.archive.org/web/https://protonmail.com/blog/whatsapp-alternatives/
  [445]: https://web.archive.org/web/https://www.whonix.org/wiki/Chat
  [446]: https://web.archive.org/web/https://securechatguide.org/featuresmatrix.html
  [447]: http://lldan5gahapx5k7iafb3s4ikijc4ni7gx5iywdflkba5y2ezyg6sjgyd.onion/
  [448]: https://web.archive.org/web/https://onionshare.org/
  [449]: https://web.archive.org/web/https://element.io/
  [450]: https://web.archive.org/web/https://jami.net/
  [451]: https://web.archive.org/web/https://briarproject.org/
  [452]: https://web.archive.org/web/https://getsession.org/
  [453]: https://web.archive.org/web/https://securedrop.org/
  [454]: https://web.archive.org/web/https://github.com/alecmuffett/real-world-onion-sites#securedrop
  [455]: https://web.archive.org/web/https://haveibeenpwned.com/
  [456]: https://web.archive.org/web/https://www.veracrypt.fr/en/Beginner%27s%20Tutorial.html
  [457]: https://web.archive.org/web/https://www.qubes-os.org/doc/backup-restore/
  [458]: https://web.archive.org/web/https://support.apple.com/en-ie/guide/mac-help/mh21241/mac
  [459]: https://web.archive.org/web/https://support.apple.com/en-ie/guide/mac-help/mh11421/11.0/mac/11.0
  [460]: https://web.archive.org/web/https://support.apple.com/en-ie/guide/disk-utility/dskutl1010/mac
  [461]: https://web.archive.org/web/https://privacytools.io/providers/cloud-storage/
  [462]: https://web.archive.org/web/https://privacytools.io/providers/paste/
  [463]: https://web.archive.org/web/https://syncthing.net/
  [464]: media/image41.jpeg
  [465]: media/image42.jpeg
  [466]: media/image43.jpeg
  [467]: https://wikiless.org/wiki/Trim_(computing)
  [468]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Trim_(computing)
  [469]: media/image44.jpeg
  [470]: https://yewtu.be/watch?v=-bpX8YvNg6Y
  [471]: https://web.archive.org/web/https://wiki.archlinux.org/index.php/Solid_state_drive
  [472]: https://web.archive.org/web/https://www.bleachbit.org/download/linux
  [473]: https://web.archive.org/web/https://superuser.com/questions/19326/how-to-wipe-free-disk-space-in-linux
  [474]: https://web.archive.org/web/https://linuxhint.com/completely_wipe_hard_drive_ubuntu/
  [475]: https://web.archive.org/web/https://linoxide.com/linux-command/commands-wipe-disk-linux/
  [476]: https://web.archive.org/web/https://wiki.archlinux.org/index.php/Securely_wipe_disk
  [477]: https://web.archive.org/web/https://ssd.eff.org/en/module/how-delete-your-data-securely-linux
  [Extra Tools Cleaning]: #extra-tools-cleaning
  [478]: https://web.archive.org/web/https://eraser.heidi.ie/download/
  [479]: https://web.archive.org/web/http://killdisk.com/killdisk-freeware.htm
  [480]: https://web.archive.org/web/https://support.apple.com/en-gb/guide/disk-utility/dskutl14079/mac
  [481]: https://web.archive.org/web/https://support.apple.com/guide/disk-utility/erase-and-reformat-a-storage-device-dskutl14079/mac
  [482]: https://web.archive.org/web/https://support.apple.com/guide/disk-utility/encrypt-protect-a-storage-device-password-dskutl35612/mac
  [483]: media/image45.jpeg
  [484]: https://web.archive.org/web/https://ssd.eff.org/en/module/how-delete-your-data-securely-windows
  [485]: https://web.archive.org/web/https://wiki.archlinux.org/index.php/dm-crypt/Device_encryption
  [486]: https://web.archive.org/web/https://github.com/Qubes-Community/Contents/blob/master/docs/configuration/disk-trim.md
  [487]: https://web.archive.org/web/https://support.apple.com/en-us/HT210898
  [488]: media/image46.jpeg
  [489]: https://web.archive.org/web/http://www.edenwaith.com/products/permanent%20eraser/
  [490]: https://web.archive.org/web/https://ssd.eff.org/en/module/how-delete-your-data-securely-macos
  [491]: https://web.archive.org/web/https://www.whonix.org/wiki/System_Hardening_Checklist
  [492]: https://web.archive.org/web/https://exiftool.org/
  [493]: https://web.archive.org/web/https://exifcleaner.com/
  [494]: https://web.archive.org/web/https://www.purevpn.com/internet-privacy/how-to-remove-metadata-from-photos
  [495]: https://web.archive.org/web/https://sandlab.cs.uchicago.edu/fawkes/
  [496]: https://web.archive.org/web/https://github.com/Shawn-Shan/fawkes
  [497]: https://web.archive.org/web/https://lowkey.umiacs.umd.edu/
  [498]: https://web.archive.org/web/https://adversarial.io/
  [499]: https://web.archive.org/web/https://github.com/kanzure/pdfparanoia
  [500]: https://web.archive.org/web/https://support.microsoft.com/en-us/office/remove-hidden-data-and-personal-information-by-inspecting-documents-presentations-or-workbooks-356b7b5d-77af-44fe-a07f-9aa4d085966f
  [501]: https://web.archive.org/web/https://0xacab.org/jvoisin/mat2
  [502]: media/image47.jpeg 
  [503]: https://web.archive.org/web/https://www.whonix.org/wiki/VM_Live_Mode
  [504]: https://web.archive.org/web/https://www.titanium-software.fr/en/onyx.html
  [505]: https://web.archive.org/web/https://github.com/Qubes-Community/Contents/blob/master/docs/security/security-guidelines.md
  [506]: https://web.archive.org/web/https://www.whonix.org/wiki/Linux_Kernel_Runtime_Guard_LKRG
  [507]: https://web.archive.org/web/https://support.microsoft.com/en-us/windows/how-to-open-registry-editor-in-windows-10-deab38e6-91d6-e0aa-4b7c-8878d9e07b11
  [508]: https://web.archive.org/web/https://privazer.com/en/download-shellbag-analyzer-shellbag-cleaner.php
  [509]: https://web.archive.org/web/https://privazer.com/en/download.php
  [510]: https://web.archive.org/web/https://www.bleachbit.org/download
  [511]: https://web.archive.org/web/https://justdeleteme.xyz/
  [512]: https://web.archive.org/web/https://justgetmydata.com/
  [513]: https://web.archive.org/web/https://search.google.com/search-console/remove-outdated-content
  [514]: https://web.archive.org/web/https://www.bing.com/webmasters/tools/contentremoval
  [515]: https://web.archive.org/web/https://webmaster.yandex.com/
  [516]: https://web.archive.org/web/https://webmaster.yandex.com/tools/del-url/
  [517]: https://web.archive.org/web/https://help.yahoo.com/kb/SLN4530.html
  [518]: https://wikiless.org/wiki/Wikipedia:Courtesy_vanishing
  [519]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Wikipedia:Courtesy_vanishing
  [520]: https://web.archive.org/web/https://inteltechniques.com/workbook.html
  ...
  [539]: media/image48.jpeg 
  [540]: media/image49.jpeg 
  [541]: media/image50.jpeg 
  ...
  [546]: https://wikiless.org/wiki/Permanent_Record_(autobiography)
  [547]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Permanent_Record_(autobiography)
  [548]: https://web.archive.org/web/https://www.w10privacy.de/english-home/
  [549]: https://web.archive.org/web/https://crazymax.dev/WindowsSpyBlocker/download/
  [550]: https://web.archive.org/web/https://www.oo-software.com/en/shutup10
  [551]: https://web.archive.org/web/https://github.com/beerisgood/windows10_hardening
  [552]: https://web.archive.org/web/https://www.microsoft.com/en-us/software-download/windows10
  [553]: https://web.archive.org/web/https://www.system-rescue.org/Installing-SystemRescue-on-a-USB-memory-stick/
  [554]: https://web.archive.org/web/https://wiki.archlinux.org/index.php/Solid_state_drive/Memory_cell_clearing
  [555]: https://web.archive.org/web/https://ata.wiki.kernel.org/index.php/ATA_Secure_Erase
  [556]: https://web.archive.org/web/https://tinyapps.org/docs/wipe_drives_hdparm.html
  [557]: https://web.archive.org/web/https://tinyapps.org/docs/ata_sanitize_hdparm.html
  [558]: https://web.archive.org/web/https://tinyapps.org/docs/nvme-secure-erase.html
  [559]: https://web.archive.org/web/https://tinyapps.org/docs/nvme-sanitize.html
  [560]: https://web.archive.org/web/https://clonezilla.org/liveusb.php
  [561]: https://web.archive.org/web/https://clonezilla.org/show-live-doc-content.php?topic=clonezilla-live/doc/01_Save_disk_image
  [562]: https://web.archive.org/web/https://clonezilla.org/show-live-doc-content.php?topic=clonezilla-live/doc/02_Restore_disk_image
  [563]: https://web.archive.org/web/https://bridges.torproject.org/
  [564]: https://web.archive.org/web/https://support.microsoft.com/en-us/windows/disk-cleanup-in-windows-10-8a96ff42-5751-39ad-23d6-434b4d5b9a68
  [565]: https://web.archive.org/web/https://support.microsoft.com/en-us/windows/defragment-your-windows-10-pc-048aefac-7f1f-4632-d48a-9700c4ec702a
  [566]: https://web.archive.org/web/https://www.bleachbit.org/
  [567]: https://web.archive.org/web/https://privazer.com/
  [568]: https://web.archive.org/web/https://www.system-rescue.org/
  [569]: https://web.archive.org/web/https://www.lifewire.com/how-to-erase-a-hard-drive-using-dban-2619148
  [570]: https://web.archive.org/web/https://github.com/PartialVolume/shredos.2020.02
  [571]: https://web.archive.org/web/https://rufus.ie/
  [572]: https://web.archive.org/web/https://kb.sandisk.com/app/answers/detail/a_id/15108/~/dashboard-support-information
  [573]: https://web.archive.org/web/https://www.seagate.com/support/downloads/seatools/
  [574]: https://web.archive.org/web/https://www.samsung.com/semiconductor/minisite/ssd/download/tools/
  [575]: https://web.archive.org/web/https://www.kingston.com/unitedstates/en/support/technical/ssdmanager
  [576]: https://web.archive.org/web/https://support.lenovo.com/us/en/downloads/ds019026-thinkpad-drive-erase-utility-for-resetting-the-cryptographic-key-and-erasing-the-solid-state-drive-thinkpad
  [577]: https://web.archive.org/web/https://www.crucial.com/support/storage-executive
  [578]: https://web.archive.org/web/https://support.wdc.com/downloads.aspx?p=279
  [579]: https://web.archive.org/web/https://store.hp.com/us/en/tech-takes/how-to-secure-erase-ssd
  [580]: https://web.archive.org/web/https://www.transcend-info.com/Support/Software-10/
  [581]: https://web.archive.org/web/https://www.dell.com/support/kbdoc/en-us/000134997/using-the-dell-bios-data-wipe-function-for-optiplex-precision-and-latitude-systems-built-after-november-2015?lwp=rt
  [582]: https://web.archive.org/web/https://www.toshiba-storage.com/downloads/
  [583]: https://web.archive.org/web/https://www.glump.net/howto/desktop/enable-trim-on-an-external-ssd-on-linux
  [584]: https://web.archive.org/web/https://code.mendhak.com/securely-wipe-ssd/
  [585]: https://web.archive.org/web/https://www.lifewire.com/enable-trim-for-ssd-in-os-x-yosemite-2260789
  [586]: https://web.archive.org/web/https://cindori.org/trimenabler/
  [587]: https://web.archive.org/web/https://www.debian.org/CD/netinst/
  [588]: https://web.archive.org/web/https://support.lenovo.com/be/en/solutions/migr-68369
  [589]: https://web.archive.org/web/https://support.hp.com/gb-en/document/c06204100
  [590]: https://web.archive.org/web/https://www.dell.com/support/kbdoc/en-us/000146892/dell-data-wipe
  [591]: https://web.archive.org/web/https://us.answers.acer.com/app/answers/detail/a_id/41567/~/how-to-use-disk-sanitizer-on-acer-travelmate-notebooks
  [592]: https://translate.google.com/translate?hl=&sl=ru&tl=en&u=https%3A%2F%2Fbiboroda.livejournal.com%2F4894724.html&anno=2
  [593]: https://web.archive.org/web/https://biboroda.livejournal.com/4894724.html
  [594]: https://wikiless.org/wiki/EncroChat
  [595]: https://web.archive.org/web/https://en.wikipedia.org/wiki/EncroChat
  [596]: https://wikiless.org/wiki/Sky_ECC
  [597]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Sky_ECC
  [598]: https://web.archive.org/web/https://privacytools.io/providers/vpn/
  [599]: https://web.archive.org/web/https://safing.io/
  [Printing Watermarking]: #printing-watermarking
  [600]: https://web.archive.org/web/https://www.getmonero.org/community/merchants/
  [601]: https://web.archive.org/web/https://proprivacy.com/vpn/guides/create-your-own-vpn-server
  [602]: https://web.archive.org/web/https://linuxize.com/post/how-to-setup-ssh-socks-tunnel-for-private-browsing/
  [603]: https://web.archive.org/web/https://www.digitalocean.com/community/tutorials/how-to-route-web-traffic-securely-without-a-vpn-using-a-socks-tunnel
  [604]: https://web.archive.org/web/https://www.forwardproxy.com/2018/12/using-putty-to-setup-a-quick-socks-proxy/
  [605]: https://web.archive.org/web/https://ma.ttias.be/socks-proxy-linux-ssh-bypass-content-filters/
  [606]: https://web.archive.org/web/https://www.putty.org/
  [607]: media/image51.jpeg 
  [608]: https://web.archive.org/web/https://tails.boum.org/contribute/design/Unsafe_Browser/
  [609]: https://web.archive.org/web/https://archive.flossmanuals.net/bypassing-censorship/index.html
  [610]: media/image52.jpeg 
  [611]: https://web.archive.org/web/https://www.alfa.com.tw/
  [612]: https://web.archive.org/web/https://www.tp-link.com/us/home-networking/usb-adapter/tl-wn722n/
  [613]: https://web.archive.org/web/https://www.wirelesshack.org/best-kali-linux-compatible-usb-adapter-dongles.html
  [614]: https://yewtu.be/watch?v=8FV2QZ1BPnw
  [615]: https://web.archive.org/web/https://www.netally.com/products/
  [616]: https://web.archive.org/web/https://mullvad.net/en/help/install-mullvad-app-windows/
  [617]: https://web.archive.org/web/https://www.ivpn.net/apps-windows
  [618]: https://web.archive.org/web/https://docs.safing.io/portmaster/install/windows
  [619]: https://web.archive.org/web/https://protonvpn.com/support/protonvpn-windows-vpn-application/
  [620]: https://web.archive.org/web/https://mullvad.net/en/help/install-and-use-mullvad-app-macos/
  [621]: https://web.archive.org/web/https://www.ivpn.net/apps-macos/
  [622]: https://web.archive.org/web/https://protonvpn.com/support/protonvpn-mac-vpn-application/
  [623]: https://web.archive.org/web/https://mullvad.net/en/help/install-mullvad-app-linux/
  [624]: https://web.archive.org/web/https://www.ivpn.net/apps-linux/
  [625]: https://web.archive.org/web/https://docs.safing.io/portmaster/install/linux
  [626]: https://web.archive.org/web/https://protonvpn.com/support/linux-vpn-setup/
  [627]: https://web.archive.org/web/https://www.ivpn.net/knowledgebase/general/do-you-offer-a-kill-switch-or-vpn-firewall/
  [628]: https://web.archive.org/web/https://protonvpn.com/support/what-is-kill-switch/
  [629]: https://web.archive.org/web/https://mullvad.net/en/help/using-mullvad-vpn-app/
  [630]: https://web.archive.org/web/https://protonvpn.com/blog/macos-vpn-kill-switch/
  [631]: https://web.archive.org/web/https://mullvad.net/en/help/wireguard-and-mullvad-vpn/
  [632]: https://web.archive.org/web/https://mullvad.net/en/help/linux-openvpn-installation/
  [633]: https://web.archive.org/web/https://github.com/ProtonVPN/linux-cli/blob/master/USAGE.md
  [634]: https://web.archive.org/web/https://www.ivpn.net/knowledgebase/linux/linux-wireguard-kill-switch/
  [635]: https://web.archive.org/web/https://www.ivpn.net/knowledgebase/linux/linux-kill-switch-using-the-uncomplicated-firewall-ufw/
  [636]: https://web.archive.org/web/https://ooni.org/install/
  [637]: https://web.archive.org/web/https://www.gpg4win.org/
  [638]: https://web.archive.org/web/https://gpgtools.org/
  [639]: https://web.archive.org/web/https://support.torproject.org/tbb/how-to-verify-signature/
  [640]: https://web.archive.org/web/https://tails.boum.org/install/vm-download/index.en.html
  [641]: https://web.archive.org/web/https://www.whonix.org/wiki/Verify_the_Whonix_images
  [642]: https://web.archive.org/web/https://www.clamav.net/
  [643]: https://web.archive.org/web/https://github.com/rfxn/linux-malware-detect
  [644]: https://web.archive.org/web/http://www.chkrootkit.org/
  [645]: https://web.archive.org/web/https://developers.virustotal.com/v3.0/docs/search-by-hash
  [646]: https://web.archive.org/web/https://github.com/rshipp/awesome-malware-analysis
  [647]: https://web.archive.org/web/https://support.virustotal.com/hc/en-us/articles/115002168385-Privacy-Policy
  [648]: https://web.archive.org/web/https://blog.didierstevens.com/programs/pdf-tools/
  [649]: https://web.archive.org/web/https://github.com/QubesOS/qubes-app-linux-pdf-converter
  [650]: https://web.archive.org/web/https://github.com/firstlookmedia/pdf-redact-tools
  [651]: https://web.archive.org/web/https://github.com/firstlookmedia/dangerzone
  [652]: https://web.archive.org/web/https://www.vice.com/en/article/v7gd9b/facebook-helped-fbi-hack-child-predator-buster-hernandez
  ...
  [657]: https://web.archive.org/web/https://wiki.archlinux.org/title/Firefox/Privacy
  [658]: https://web.archive.org/web/https://www.privacytools.io/browsers/#about_config
  [659]: https://web.archive.org/web/https://proprivacy.com/privacy-service/guides/firefox-privacy-security-guide
  [660]: https://web.archive.org/web/https://chrisx.xyz/blog/yet-another-firefox-hardening-guide/
  [661]: https://web.archive.org/web/https://ebin.city/~werwolf/posts/firefox-hardening-guide/
  [662]: media/image53.jpeg 
  [663]: media/image54.jpeg 
  [664]: https://web.archive.org/web/https://www.torproject.org/download/
  [665]: media/image55.jpeg 
  [666]: media/image56.jpeg 
  [667]: media/image57.jpeg 
  [668]: https://web.archive.org/web/https://wasabiwallet.io/
  [669]: https://web.archive.org/web/https://privacytools.io/providers/hosting/
  [670]: https://web.archive.org/web/https://community.torproject.org/relay/community-resources/good-bad-isps/
  [671]: https://web.archive.org/web/https://xkcd.com/936/
  [672]: https://web.archive.org/web/https://www.explainxkcd.com/wiki/index.php/936:_Password_Strength
  [673]: media/image58.jpeg 
  [674]: https://yewtu.be/watch?v=3NjQ9b3pgIg
  [675]: https://web.archive.org/web/https://privacytools.io/providers/search-engines
  [676]: http://www.privacy2zbidut4m4jyj3ksdqidzkw3uoip2vhvhbvwxbqux5xy5obyd.onion/providers/search-engines/
  [677]: https://web.archive.org/web/https://arstechnica.com/tech-policy/2018/06/dark-web-vendor-oxymonster-turns-out-to-be-a-frenchman-with-luscious-beard/
  [678]: https://wikiless.org/wiki/Forensic_linguistics
  [679]: https://web.archive.org/web/https://wikipedia.org/wiki/Forensic_linguistics
  [680]: https://wikiless.org/wiki/Writeprint
  [681]: https://web.archive.org/web/https://wikipedia.org/wiki/Writeprint
  [682]: https://wikiless.org/wiki/Stylometry
  [683]: https://web.archive.org/web/https://wikipedia.org/wiki/Stylometry
  [684]: https://wikiless.org/wiki/Content_similarity_detection
  [685]: https://web.archive.org/web/https://wikipedia.org/wiki/Content_similarity_detection
  [686]: https://wikiless.org/wiki/Author_profiling
  [687]: https://web.archive.org/web/https://wikipedia.org/wiki/Author_profiling
  [688]: https://wikiless.org/wiki/Native-language_identification
  [689]: https://web.archive.org/web/https://wikipedia.org/wiki/Native-language_identification
  [690]: https://wikiless.org/wiki/Computational_linguistics
  [691]: https://web.archive.org/web/https://wikipedia.org/wiki/Computational_linguistics
  [692]: https://web.archive.org/web/https://regmedia.co.uk/2017/09/27/gal_vallerius.pdf
  [693]: https://wikiless.org/wiki/Ted_Kaczynski#After_publication
  [694]: https://web.archive.org/web/https://wikipedia.org/wiki/Ted_Kaczynski#After_publication
  [695]: https://web.archive.org/web/https://i.blackhat.com/USA-19/Wednesday/us-19-Wixey-Im-Unique-Just-Like-You-Human-Side-Channels-And-Their-Implications-For-Security-And-Privacy.pdf
  [696]: https://web.archive.org/web/https://media.defcon.org/DEF%20CON%2026/DEF%20CON%2026%20presentations/DEFCON-26-Matt-Wixey-Betrayed-by-the-Keyboard-Updated.pdf
  [697]: https://web.archive.org/web/https://i.blackhat.com/us-18/Wed-August-8/us-18-Wixey-Every-ROSE-Has-Its-Thorn-The-Dark-Art-Of-Remote-Online-Social-Engineering.pdf
  [698]: https://web.archive.org/web/https://github.com/spikecodes/libreddit
  [699]: https://web.archive.org/web/https://git.sr.ht/~metalune/simplytranslate_web
  [https://anonymousplanet.org]: https://anonymousplanet.org/
  [700]: https://web.archive.org/web/https://anonymousplanet.org
  [701]: https://archive.fo/anonymousplanet.org/guide.html
  [702]: https://mirror.anonymousplanet.org/guide.pdf
  [703]: https://web.archive.org/web/https://anonymousplanet.org/guide.pdf
  [704]: http://thgtoa7imksbg7rit4grgijl2ef6kc7b56bp56pmtta4g354lydlzkqd.onion/guide.pdf
  [705]: https://mirror.anonymousplanet.org/guide-dark.pdf
  [706]: https://web.archive.org/web/https://anonymousplanet.org/guide-dark.pdf
  [707]: http://thgtoa7imksbg7rit4grgijl2ef6kc7b56bp56pmtta4g354lydlzkqd.onion/guide-dark.pdf
  [708]: https://mirror.anonymousplanet.org/guide.odt
  [709]: https://web.archive.org/web/https://anonymousplanet.org/guide.odt
  [710]: http://thgtoa7imksbg7rit4grgijl2ef6kc7b56bp56pmtta4g354lydlzkqd.onion/guide.odt
  [711]: https://web.archive.org/web/https://help.libreoffice.org/7.1/en-US/text/shared/guide/redlining_doccompare.html
  [**https://www.youtube.com/watch?v=j02QoI4ZlnU**]: https://www.youtube.com/watch?v=j02QoI4ZlnU
  [712]: https://yewtu.be/watch?v=j02QoI4ZlnU
  [713]: https://web.archive.org/web/https://github.com/monero-project/monero/blob/master/docs/ANONYMITY_NETWORKS.md#privacy-limitations
  [714]: https://web.archive.org/web/https://www.huntonprivacyblog.com/wp-content/uploads/sites/28/2016/02/Telemedia_Act__TMA_.pdf
  [715]: https://wikiless.org/wiki/Don%27t_be_evil
  [716]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Don%27t_be_evil
  [717]: https://yewtu.be/watch?v=6DGNZnfKYnU
  [718]: https://wikiless.org/wiki/Open-source_intelligence
  [719]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Open-source_intelligence
  [720]: https://yewtu.be/playlist?list=PLna1KTNJu3y09Tu70U6yPn28sekaNhOMY
  [721]: https://wikiless.org/wiki/4chan
  [722]: https://web.archive.org/web/https://en.wikipedia.org/wiki/4chan
  [723]: https://web.archive.org/web/https://www.privateinternetaccess.com/blog/how-does-privacy-differ-from-anonymity-and-why-are-both-important/
  [724]: https://web.archive.org/web/https://scholar.harvard.edu/files/mickens/files/thisworldofours.pdf
  [725]: https://web.archive.org/web/https://xkcd.com/538/
  [726]: https://wikiless.org/wiki/Threat_model
  [727]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Threat_model
  [728]: https://web.archive.org/web/https://www.bellingcat.com/
  [729]: https://wikiless.org/wiki/Doxing
  [730]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Doxing
  [731]: https://yewtu.be/watch?v=muoR8Td44UE
  [732]: https://web.archive.org/web/https://www.bbc.com/news/technology-50150981
  [733]: https://web.archive.org/web/https://github.com/alecmuffett/real-world-onion-sites
  [734]: https://web.archive.org/web/https://2019.www.torproject.org/about/torusers.html.en
  [735]: https://web.archive.org/web/https://www.whonix.org/wiki/Anonymity
  [736]: https://web.archive.org/web/https://geekfeminism.wikia.org/wiki/Who_is_harmed_by_a_%22Real_Names%22_policy%3F
  [737]: https://web.archive.org/web/https://www.cyberghostvpn.com/privacyhub/internet-privacy-surveillance/
  [738]: https://web.archive.org/web/https://www.privateinternetaccess.com/blog/internet-freedom-around-the-world-in-50-stats/
  [739]: https://wikiless.org/wiki/IANAL
  [740]: https://web.archive.org/web/https://en.wikipedia.org/wiki/IANAL
  [741]: https://wikiless.org/wiki/Trust,_but_verify
  [742]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Trust,_but_verify
  [743]: https://wikiless.org/wiki/IP_address
  [744]: https://web.archive.org/web/https://en.wikipedia.org/wiki/IP_address
  [745]: https://wikiless.org/wiki/Data_retention
  [746]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Data_retention
  [747]: https://wikiless.org/wiki/Tor_(anonymity_network)
  [748]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Tor_(anonymity_network)
  [749]: https://wikiless.org/wiki/Virtual_private_network
  [750]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Virtual_private_network
  [751]: https://wikiless.org/wiki/Domain_Name_System
  [752]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Domain_Name_System
  [753]: https://wikiless.org/wiki/DNS_blocking
  [754]: https://web.archive.org/web/https://en.wikipedia.org/wiki/DNS_blocking
  [755]: https://web.archive.org/web/https://arxiv.org/pdf/2001.08288.pdf
  [756]: https://web.archive.org/web/https://labzilla.io/blog/force-dns-pihole
  [757]: https://wikiless.org/wiki/DNS_over_HTTPS
  [758]: https://web.archive.org/web/https://en.wikipedia.org/wiki/DNS_over_HTTPS
  [759]: https://wikiless.org/wiki/DNS_over_TLS
  [760]: https://web.archive.org/web/https://en.wikipedia.org/wiki/DNS_over_TLS
  [761]: https://wikiless.org/wiki/Pi-hole
  [762]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Pi-hole
  [763]: https://wikiless.org/wiki/Server_Name_Indication
  [764]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Server_Name_Indication
  [765]: https://web.archive.org/web/https://www.usenix.org/system/files/foci19-paper_chai_0.pdf
  [766]: https://wikiless.org/wiki/Content_delivery_network
  [767]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Content_delivery_network
  [768]: https://web.archive.org/web/https://blog.cloudflare.com/encrypted-client-hello/
  [769]: https://web.archive.org/web/https://www.zdnet.com/article/russia-wants-to-ban-the-use-of-secure-protocols-such-as-tls-1-3-doh-dot-esni/
  [770]: https://web.archive.org/web/https://www.zdnet.com/article/china-is-now-blocking-all-encrypted-https-traffic-using-tls-1-3-and-esni/
  [771]: https://wikiless.org/wiki/Online_Certificate_Status_Protocol
  [772]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Online_Certificate_Status_Protocol
  [773]: https://web.archive.org/web/https://madaidans-insecurities.github.io/encrypted-dns.html
  [774]: https://wikiless.org/wiki/OCSP_stapling
  [775]: https://web.archive.org/web/https://en.wikipedia.org/wiki/OCSP_stapling
  [776]: https://web.archive.org/web/https://dev.chromium.org/Home/chromium-security/crlsets
  [777]: https://web.archive.org/web/https://www.zdnet.com/article/chrome-does-certificate-revocation-better/
  [778]: https://web.archive.org/web/https://www.esat.kuleuven.be/cosic/publications/article-3153.pdf
  [779]: https://web.archive.org/web/https://www.researchgate.net/publication/332893422_Oblivious_DNS_Practical_Privacy_for_DNS_Queries
  [780]: https://web.archive.org/web/https://nymity.ch/tor-dns/
  [781]: https://wikiless.org/wiki/Near-field_communication
  [782]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Near-field_communication
  [783]: https://web.archive.org/web/https://shop.samsonite.com/accessories/rfid-accessories/
  [784]: https://web.archive.org/web/https://support.google.com/accounts/answer/3467281?hl=en
  [785]: https://web.archive.org/web/https://support.apple.com/en-us/HT207056
  [786]: https://web.archive.org/web/https://cse.buffalo.edu/~lusu/papers/MobiCom2020.pdf
  [787]: https://web.archive.org/web/https://digi.ninja/jasager/
  [788]: https://web.archive.org/web/https://shop.hak5.org/products/wifi-pineapple
  [789]: https://wikiless.org/wiki/Wi-Fi_deauthentication_attack
  [790]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Wi-Fi_deauthentication_attack
  [791]: https://wikiless.org/wiki/Captive_portal
  [792]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Captive_portal
  [793]: https://web.archive.org/web/https://www.hackerfactor.com/blog/index.php?/archives/868-Deanonymizing-Tor-Circuits.html
  [794]: https://web.archive.org/web/https://distrinet.cs.kuleuven.be/software/tor-wf-dl/
  [795]: https://web.archive.org/web/https://www.dailydot.com/unclick/tor-harvard-bomb-suspect/
  [796]: https://web.archive.org/web/https://arstechnica.com/information-technology/2015/10/how-the-nsa-can-break-trillions-of-encrypted-web-and-vpn-connections/
  [797]: https://web.archive.org/web/https://arstechnica.com/gadgets/2020/11/does-tor-provide-more-benefit-or-harm-new-paper-says-it-depends/
  [798]: https://web.archive.org/web/https://www.pnas.org/content/early/2020/11/24/2011893117
  [799]: https://web.archive.org/web/https://blog.cryptographyengineering.com/2019/06/05/how-does-apple-privately-find-your-offline-devices/
  [800]: https://web.archive.org/web/https://support.apple.com/en-us/HT210515
  [801]: https://web.archive.org/web/https://www.xda-developers.com/samsung-find-my-mobile-app-locate-galaxy-devices-offline/
  [802]: https://web.archive.org/web/https://support.apple.com/en-us/HT204756
  [803]: https://wikiless.org/wiki/Bluetooth_Low_Energy
  [804]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Bluetooth_Low_Energy
  [805]: https://wikiless.org/wiki/International_Mobile_Equipment_Identity
  [806]: https://web.archive.org/web/https://en.wikipedia.org/wiki/International_Mobile_Equipment_Identity
  [807]: https://wikiless.org/wiki/International_mobile_subscriber_identity
  [808]: https://web.archive.org/web/https://en.wikipedia.org/wiki/International_mobile_subscriber_identity
  [809]: https://web.archive.org/web/https://source.android.com/devices/tech/config/device-identifiers
  [810]: https://web.archive.org/web/https://policies.google.com/privacy/embedded?hl=en-US
  [811]: https://web.archive.org/web/https://www.bellingcat.com/news/uk-and-europe/2019/06/28/the-gru-globetrotters-mission-london/
  [812]: https://web.archive.org/web/https://www.bellingcat.com/news/uk-and-europe/2020/02/17/v-like-vympel-fsbs-secretive-department-v-behind-assassination-of-zelimkhan-khangoshvili/
  [813]: https://wikiless.org/wiki/Closed-circuit_television
  [814]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Closed-circuit_television
  [815]: https://web.archive.org/web/https://www.apple.com/legal/transparency/device-requests.html
  [816]: https://27m3p2uv7igmj6kvd4ql3cct5h3sdwrsajovkkndeufumzyfhlfev4qd.onion/2020/07/31/protests-surveillance-stingrays-dirtboxes-phone-tracking/
  [817]: https://web.archive.org/web/https://theintercept.com/2020/07/31/protests-surveillance-stingrays-dirtboxes-phone-tracking/
  [818]: https://wikiless.org/wiki/IMSI-catcher
  [819]: https://web.archive.org/web/https://en.wikipedia.org/wiki/IMSI-catcher
  [820]: https://wikiless.org/wiki/Stingray_phone_tracker
  [821]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Stingray_phone_tracker
  [822]: https://web.archive.org/web/https://gizmodo.com/american-cops-turns-to-canadian-phone-tracking-firm-aft-1845442778
  [823]: https://wikiless.org/wiki/Man-in-the-middle_attack
  [824]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Man-in-the-middle_attack
  [825]: https://web.archive.org/web/https://shop.puri.sm/shop/librem-5/
  [826]: https://wikiless.org/wiki/MAC_address
  [827]: https://web.archive.org/web/https://en.wikipedia.org/wiki/MAC_address
  [828]: https://web.archive.org/web/https://amsignalinc.com/data-sheets/Acyclica/Acyclica-RoadTrend-Product-Sheet.pdf
  [829]: https://web.archive.org/web/https://www.researchgate.net/publication/334590931_Tracking_Anonymized_Bluetooth_Devices/fulltext/5d3308db92851cd04675a469/Tracking-Anonymized-Bluetooth-Devices.pdf
  [830]: https://wikiless.org/wiki/Central_processing_unit
  [831]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Central_processing_unit
  [832]: https://wikiless.org/wiki/Intel_Management_Engine
  [833]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Intel_Management_Engine
  [834]: https://wikiless.org/wiki/AMD_Platform_Security_Processor
  [835]: https://web.archive.org/web/https://en.wikipedia.org/wiki/AMD_Platform_Security_Processor
  [836]: https://web.archive.org/web/https://libreboot.org/
  [837]: https://web.archive.org/web/https://www.apple.com/privacy/docs/Differential_Privacy_Overview.pdf
  [838]: https://wikiless.org/wiki/Differential_privacy
  [839]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Differential_privacy
  [840]: https://web.archive.org/web/https://edwardsnowden.substack.com/p/all-seeing-i
  [841]: https://web.archive.org/web/https://www.reuters.com/article/us-apple-fbi-icloud-exclusive-idUSKBN1ZK1CT
  [842]: https://web.archive.org/web/https://www.zdnet.com/article/apple-data-collection-stored-request/
  [843]: https://web.archive.org/web/https://decorrespondent.nl/8481/heres-how-we-found-the-names-and-addresses-of-soldiers-and-secret-agents-using-a-simple-fitness-app/412999257-6756ba27
  [844]: https://web.archive.org/web/https://www.wired.com/story/strava-heat-map-military-bases-fitness-trackers-privacy/
  [845]: https://web.archive.org/web/https://www.bellingcat.com/resources/how-tos/2018/01/29/strava-interpretation-guide/
  [846]: https://web.archive.org/web/https://www.theguardian.com/world/2018/jan/28/fitness-tracking-app-gives-away-location-of-secret-us-army-bases
  [847]: https://web.archive.org/web/https://www.telegraph.co.uk/technology/2018/07/08/running-app-exposes-mi6-gchq-workers-whereabouts/
  [848]: https://web.archive.org/web/https://www.washingtonpost.com/technology/2019/05/06/alexa-has-been-eavesdropping-you-this-whole-time/?itid=lk_interstitial_manual_59
  [849]: https://web.archive.org/web/https://www.washingtonpost.com/technology/2019/12/17/what-does-your-car-know-about-you-we-hacked-chevy-find-out/
  [850]: https://web.archive.org/web/https://kieranhealy.org/blog/archives/2013/06/09/using-metadata-to-find-paul-revere/
  [851]: https://wikiless.org/wiki/Sensorvault
  [852]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Sensorvault
  [853]: https://web.archive.org/web/https://nrkbeta.no/2020/12/03/my-phone-was-spying-on-me-so-i-tracked-down-the-surveillants/
  [854]: https://web.archive.org/web/https://www.nytimes.com/interactive/2019/12/19/opinion/location-tracking-cell-phone.html
  [855]: https://web.archive.org/web/https://nakedsecurity.sophos.com/2020/03/10/google-data-puts-innocent-man-at-the-scene-of-a-crime/
  [856]: https://wikiless.org/wiki/Geo-fence_warrant
  [857]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Geo-fence_warrant
  [858]: https://web.archive.org/web/https://www.vice.com/en/article/y3g97x/location-data-apps-drone-strikes-iowa-national-guard
  [859]: https://web.archive.org/web/https://techcrunch.com/2021/08/19/google-geofence-warrants/
  [860]: https://web.archive.org/web/https://www.techdirt.com/articles/20210821/10494847401/google-report-shows-reverse-warrants-are-swiftly-becoming-law-enforcements-go-to-investigative-tool.shtml
  [861]: https://wikiless.org/wiki/Room_641A
  [862]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Room_641A
  [863]: https://wikiless.org/wiki/Edward_Snowden
  [864]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Edward_Snowden
  [865]: https://wikiless.org/wiki/XKeyscore
  [866]: https://web.archive.org/web/https://en.wikipedia.org/wiki/XKeyscore
  [867]: https://web.archive.org/web/https://www.electrospaces.net/2020/10/danish-military-intelligence-uses.html
  [868]: https://web.archive.org/web/https://en.m.wikipedia.org/wiki/MUSCULAR_(surveillance_program)
  [869]: https://wikiless.org/wiki/SORM
  [870]: https://web.archive.org/web/https://en.wikipedia.org/wiki/SORM
  [871]: https://wikiless.org/wiki/Tempora
  [872]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Tempora
  [873]: https://wikiless.org/wiki/PRISM_(surveillance_program)
  [874]: https://web.archive.org/web/https://en.wikipedia.org/wiki/PRISM_(surveillance_program)
  [875]: https://web.archive.org/web/https://www.justsecurity.org/10318/video-clip-director-nsa-cia-we-kill-people-based-metadata/
  [876]: https://web.archive.org/web/https://www.imdb.com/title/tt11464826/
  [877]: https://web.archive.org/web/https://arstechnica.com/information-technology/2015/07/how-the-way-you-type-can-shatter-anonymity-even-on-tor/
  [878]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Stylometry
  [879]: https://web.archive.org/web/https://paul.reviews/behavioral-profiling-the-password-you-cant-change/
  [880]: https://wikiless.org/wiki/Sentiment_analysis
  [881]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Sentiment_analysis
  [882]: https://web.archive.org/web/https://coveryourtracks.eff.org/
  [883]: https://web.archive.org/web/https://people.eecs.berkeley.edu/~dawnsong/papers/2012%20On%20the%20Feasibility%20of%20Internet-Scale%20Author%20Identification.pdf
  [884]: https://web.archive.org/web/https://blog.securedtouch.com/behavioral-biometrics-101-an-in-depth-look-at-behavioral-biometrics-vs-behavioral-analytics
  [885]: https://web.archive.org/web/https://arstechnica.com/tech-policy/2012/03/stakeout-how-the-fbi-tracked-and-busted-a-chicago-anon/
  [886]: https://web.archive.org/web/https://www.bellingcat.com/news/uk-and-europe/2018/05/25/mh17-russian-gru-commander-orion-identified-oleg-ivannikov/
  [887]: https://web.archive.org/web/https://research.fb.com/publications/deepface-closing-the-gap-to-human-level-performance-in-face-verification/
  [888]: https://web.archive.org/web/https://www.privateinternetaccess.com/blog/putting-face-facebook-mark-zuckerberg-building-world-without-public-anonymity/
  [889]: https://web.archive.org/web/https://www.cnbc.com/2017/09/01/facebook-has-mapped-human-population-building-internet-in-space.html
  [890]: https://web.archive.org/web/https://www.technologyreview.com/2021/02/05/1017388/ai-deep-learning-facial-recognition-data-history/
  [891]: https://web.archive.org/web/https://www.bellingcat.com/resources/case-studies/2015/08/07/shadow-of-a-doubt/
  [892]: https://web.archive.org/web/https://brown.columbia.edu/open-source-investigation/
  [893]: https://web.archive.org/web/https://www.newscientist.com/article/dn27761-facebook-can-recognise-you-in-photos-even-if-youre-not-looking/
  [894]: https://web.archive.org/web/https://patents.google.com/patent/US20150242679
  [895]: https://web.archive.org/web/https://apnews.com/article/bf75dd1c26c947b7826d270a16e2658a
  [896]: https://web.archive.org/web/https://www.thesun.co.uk/news/7684204/cctv-technology-identify-body-shape-way-walk/
  [897]: https://web.archive.org/web/https://citysecuritymagazine.com/security-management/gait-recognition-identification-tool/
  [898]: https://web.archive.org/web/https://www.vice.com/en/article/bvzvdw/tech-companies-are-training-ai-to-read-your-lips
  [899]: https://web.archive.org/web/https://newatlas.com/science/science/eye-tracking-privacy/
  [900]: https://web.archive.org/web/https://techcrunch.com/2021/01/13/facial-recognition-reveals-political-party-in-troubling-new-research/
  [901]: https://web.archive.org/web/https://slate.com/technology/2018/04/facebook-collects-data-on-non-facebook-users-if-they-want-to-delete-it-they-have-to-sign-up.html
  [902]: https://web.archive.org/web/https://theconversation.com/shadow-profiles-facebook-knows-about-you-even-if-youre-not-on-facebook-94804
  [903]: https://web.archive.org/web/https://www.theverge.com/2018/4/11/17225482/facebook-shadow-profiles-zuckerberg-congress-data-privacy
  [904]: https://web.archive.org/web/https://www.zdnet.com/article/anger-mounts-after-facebooks-shadow-profiles-leak-in-bug/
  [905]: https://web.archive.org/web/https://www.cnet.com/news/shadow-profiles-facebook-has-information-you-didnt-hand-over/
  [906]: https://web.archive.org/web/https://www.anyvision.co/
  [907]: https://web.archive.org/web/https://www.buzzfeednews.com/article/ryanmac/clearview-ai-local-police-facial-recognition
  [908]: https://web.archive.org/web/https://www.nec.com/en/global/solutions/biometrics/face/neofacewatch.html
  [909]: https://web.archive.org/web/https://www.theguardian.com/uk-news/2020/feb/11/met-police-deploy-live-facial-recognition-technology
  [910]: https://yewtu.be/watch?v=lH2gMNrUuEY
  [911]: https://web.archive.org/web/https://edition.cnn.com/2021/07/23/tech/idme-unemployment-facial-recognition/index.html
  [912]: https://web.archive.org/web/https://www.washingtonpost.com/technology/2020/12/08/huawei-tested-ai-software-that-could-recognize-uighur-minorities-alert-police-report-says/
  [913]: https://27m3p2uv7igmj6kvd4ql3cct5h3sdwrsajovkkndeufumzyfhlfev4qd.onion/2016/10/13/how-a-facial-recognition-mismatch-can-ruin-your-life/
  [914]: https://web.archive.org/web/https://theintercept.com/2016/10/13/how-a-facial-recognition-mismatch-can-ruin-your-life/
  [915]: https://web.archive.org/web/https://www.vice.com/en/article/5dbywn/facial-recognition-failures-are-locking-people-out-of-unemployment-systems
  [916]: https://web.archive.org/web/https://www.bbc.com/news/uk-wales-43711477
  [917]: https://web.archive.org/web/https://edition.cnn.com/2021/05/25/uk/drug-dealer-cheese-sentenced-scli-gbr-intl/index.html
  [918]: https://web.archive.org/web/https://www.vice.com/en/article/evqk9e/photo-of-fingerprints-used-to-arrest-drug-dealers
  [919]: https://web.archive.org/web/https://patents.justia.com/patent/10891948
  [920]: https://web.archive.org/web/https://www.pymnts.com/news/biometrics/2021/iris-scan-traveler-identification-dubai-airport/
  [921]: https://web.archive.org/web/https://www.imdb.com/title/tt0119177/
  [922]: https://web.archive.org/web/https://www.imdb.com/title/tt1839578
  [923]: https://web.archive.org/web/https://www.imdb.com/title/tt0181689
  [924]: https://wikiless.org/wiki/Deepfake
  [925]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Deepfake
  [926]: https://web.archive.org/web/https://www.econotimes.com/Deepfake-Voice-Technology-The-Good-The-Bad-The-Future-1601278
  [927]: https://web.archive.org/web/https://www.forbes.com/sites/jessedamiani/2019/09/03/a-voice-deepfake-was-used-to-scam-a-ceo-out-of-243000/
  [928]: https://web.archive.org/web/https://josephsteinberg.com/how-to-prevent-facial-recognition-technology-from-identifying-you/
  [929]: https://web.archive.org/web/https://nvlpubs.nist.gov/nistpubs/ir/2020/NIST.IR.8311.pdf
  [930]: https://web.archive.org/web/https://www.bbc.com/news/technology-55573802
  [931]: https://web.archive.org/web/http://diglib.uwgb.edu/digital/api/collection/p17003coll4/id/71/download
  [932]: https://wikiless.org/wiki/Phishing
  [933]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Phishing
  [934]: https://wikiless.org/wiki/Social_engineering_(security)
  [935]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Social_engineering_(security)
  [936]: https://web.archive.org/web/https://www.bbc.com/news/technology-56071437
  [937]: https://wikiless.org/wiki/Exploit_(computer_security)
  [938]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Exploit_(computer_security)
  [939]: https://wikiless.org/wiki/Freedom_Hosting
  [940]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Freedom_Hosting
  [941]: https://web.archive.org/web/https://www.wired.com/2013/09/freedom-hosting-fbi/
  [942]: https://wikiless.org/wiki/2020_United_States_federal_government_data_breach
  [943]: https://web.archive.org/web/https://en.wikipedia.org/wiki/2020_United_States_federal_government_data_breach
  [944]: https://web.archive.org/web/https://www.bbc.com/news/blogs-china-blog-48552907
  [945]: https://27m3p2uv7igmj6kvd4ql3cct5h3sdwrsajovkkndeufumzyfhlfev4qd.onion/2021/01/29/china-uyghur-muslim-surveillance-police/
  [946]: https://web.archive.org/web/https://theintercept.com/2021/01/29/china-uyghur-muslim-surveillance-police/
  [947]: https://wikiless.org/wiki/Sandbox_(computer_security)
  [948]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Sandbox_(computer_security)
  [949]: https://web.archive.org/web/https://www.wired.com/2014/07/usb-security/
  [950]: https://wikiless.org/wiki/Stuxnet
  [951]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Stuxnet
  [952]: https://web.archive.org/web/https://superuser.com/questions/1206321/how-do-i-safely-investigate-a-usb-stick-found-in-the-parking-lot-at-work
  [953]: https://web.archive.org/web/https://www.theguardian.com/books/2014/may/12/glenn-greenwald-nsa-tampers-us-internet-routers-snowden
  [954]: https://wikiless.org/wiki/Rootkit
  [955]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Rootkit
  [956]: https://wikiless.org/wiki/User_space
  [957]: https://web.archive.org/web/https://en.wikipedia.org/wiki/User_space
  [958]: https://wikiless.org/wiki/Firmware
  [959]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Firmware
  [960]: https://wikiless.org/wiki/BIOS
  [961]: https://web.archive.org/web/https://en.wikipedia.org/wiki/BIOS
  [962]: https://wikiless.org/wiki/Unified_Extensible_Firmware_Interface
  [963]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Unified_Extensible_Firmware_Interface
  [964]: https://web.archive.org/web/https://www.bellingcat.com/news/americas/2018/10/26/joseph-mifsud-rush-exif/
  [965]: https://web.archive.org/web/https://support.zoom.us/hc/en-us/articles/209605273-Adding-a-Watermark
  [966]: https://web.archive.org/web/https://support.zoom.us/hc/en-us/articles/360021839031-Audio-Watermark
  [967]: https://web.archive.org/web/https://exchange.adobe.com/creativecloud.details.101789.imatag-invisible-watermark-and-image-monitoring.html
  [968]: https://web.archive.org/web/https://dtv.nagra.com/nexguard-forensic-watermarking
  [969]: https://web.archive.org/web/https://www.vobilegroup.com
  [970]: https://web.archive.org/web/https://www.cinavia.com/languages/english/pages/technology.html
  [971]: https://web.archive.org/web/https://www.imatag.com/
  [972]: https://wikiless.org/wiki/Steganography
  [973]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Steganography
  [974]: https://web.archive.org/web/https://ieeexplore.ieee.org/document/4428921
  [975]: https://web.archive.org/web/https://www.sciencedirect.com/science/article/abs/pii/S0165168498000140
  [976]: https://web.archive.org/web/https://ieeexplore.ieee.org/abstract/document/1188746
  [977]: https://web.archive.org/web/https://scholar.google.com/scholar?q=source+camera+identification
  [978]: https://wikiless.org/wiki/Machine_Identification_Code
  [979]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Machine_Identification_Code
  [980]: https://web.archive.org/web/http://seeingyellow.com/
  [981]: https://web.archive.org/web/https://arxiv.org/abs/1107.4524
  [982]: https://web.archive.org/web/https://www.bellingcat.com/resources/how-tos/2019/03/26/how-to-track-illegal-funding-campaigns-via-cryptocurrency/
  [983]: https://wikiless.org/wiki/Know_your_customer
  [984]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Know_your_customer
  [985]: https://web.archive.org/web/https://arxiv.org/pdf/1906.05754.pdf
  [986]: https://yewtu.be/playlist?list=PLsSYUeVwrHBnAUre2G_LYDsdo-tD0ov-y
  [987]: https://web.archive.org/web/https://monero.org/monero-vs-princeton-researchers/
  [988]: https://wikiless.org/wiki/Cryptocurrency_tumbler
  [989]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Cryptocurrency_tumbler
  [990]: https://wikiless.org/wiki/Security_through_obscurity
  [991]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Security_through_obscurity
  [992]: https://web.archive.org/web/https://arxiv.org/abs/2009.14007
  [993]: https://web.archive.org/web/https://papers.ssrn.com/sol3/papers.cfm?abstract_id=3080361
  [994]: https://web.archive.org/web/https://www.magnetforensics.com/products/magnet-axiom/cloud/
  [995]: https://web.archive.org/web/https://www.cellebrite.com/en/ufed-cloud/
  [996]: https://web.archive.org/web/https://sites.google.com/a/chromium.org/dev/Home/chromium-security/client-identification-mechanisms
  [997]: https://web.archive.org/web/https://wiki.mozilla.org/Fingerprinting
  [998]: https://web.archive.org/web/https://www.grayshift.com/
  [999]: https://web.archive.org/web/https://securephones.io/main.pdf
  [1000]: https://web.archive.org/web/https://loup-vaillant.fr/articles/rolling-your-own-crypto
  [1001]: https://web.archive.org/web/https://soatok.blog/2021/02/09/crackpot-cryptography-and-security-theater/
  [1002]: https://web.archive.org/web/https://www.vice.com/en/article/wnx8nq/why-you-dont-roll-your-own-crypto
  [1003]: https://web.archive.org/web/https://arxiv.org/pdf/2107.04940.pdf
  [1004]: https://yewtu.be/watch?v=loy84K3AJ5Q
  [1005]: https://web.archive.org/web/https://citizenlab.ca/2020/04/move-fast-roll-your-own-crypto-a-quick-look-at-the-confidentiality-of-zoom-meetings/
  [1006]: https://web.archive.org/web/https://medium.com/@atcipher/the-myth-of-military-grade-encryption-292313ae6369
  [1007]: https://web.archive.org/web/https://blog.congruentlabs.co/military-grade-encryption/
  [1008]: https://web.archive.org/web/https://blog.ironcorelabs.com/military-grade-encryption-69aae0145588
  [1009]: https://wikiless.org/wiki/BLAKE_(hash_function)#BLAKE2
  [1010]: https://web.archive.org/web/https://en.wikipedia.org/wiki/BLAKE_(hash_function)#BLAKE2
  [1011]: https://wikiless.org/wiki/AES_instruction_set
  [1012]: https://web.archive.org/web/https://en.wikipedia.org/wiki/AES_instruction_set
  [1013]: https://wikiless.org/wiki/Salsa20#ChaCha_variant
  [1014]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Salsa20#ChaCha_variant
  [1015]: https://wikiless.org/wiki/Serpent_(cipher)
  [1016]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Serpent_(cipher)
  [1017]: https://wikiless.org/wiki/Twofish
  [1018]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Twofish
  [1019]: https://wikiless.org/wiki/Shor%27s_algorithm
  [1020]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Shor%27s_algorithm
  [1021]: https://wikiless.org/wiki/Gag_order
  [1022]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Gag_order
  [1023]: https://wikiless.org/wiki/National_security_letter
  [1024]: https://web.archive.org/web/https://en.wikipedia.org/wiki/National_security_letter
  [1025]: https://web.archive.org/web/https://techcrunch.com/2021/09/06/protonmail-logged-ip-address-of-french-activist-after-order-by-swiss-authorities/
  [1026]: https://web.archive.org/web/https://arstechnica.com/gadgets/2021/07/vpn-servers-seized-by-ukrainian-authorities-werent-encrypted/
  [1027]: https://web.archive.org/web/https://www.bleepingcomputer.com/news/security/doublevpn-servers-logs-and-account-info-seized-by-law-enforcement/
  [1028]: https://web.archive.org/web/https://www.cyberscoop.com/court-rules-encrypted-email-tutanota-monitor-messages/
  [1029]: https://web.archive.org/web/https://www.heise.de/news/Gericht-zwingt-Mailprovider-Tutanota-zu-Ueberwachungsfunktion-4972460.html
  [1030]: https://web.archive.org/web/https://www.pcmag.com/opinions/did-purevpn-cross-a-line-when-it-disclosed-user-information
  [1031]: https://web.archive.org/web/https://archive.is/XNuVw
  [1032]: https://wikiless.org/wiki/Lavabit
  [1033]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Lavabit
  [1034]: https://wikiless.org/wiki/Warrant_canary
  [1035]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Warrant_canary
  [1036]: https://web.archive.org/web/https://www.washingtonpost.com/graphics/2020/world/national-security/cia-crypto-encryption-machines-espionage/
  [1037]: https://web.archive.org/web/https://www.swissinfo.ch/eng/second-swiss-firm-allegedly-sold-encrypted-spying-devices/46186432
  [1038]: https://wikiless.org/wiki/The_Lives_of_Others
  [1039]: https://web.archive.org/web/https://en.wikipedia.org/wiki/The_Lives_of_Others
  [1040]: https://web.archive.org/web/https://www.wired.com/story/air-gap-researcher-mordechai-guri/
  [1041]: https://web.archive.org/web/https://www.nassiben.com/lamphone
  [1042]: https://web.archive.org/web/https://www.theguardian.com/world/2013/aug/22/gchq-warned-laser-spying-guardian-offices
  [1043]: https://web.archive.org/web/https://arstechnica.com/tech-policy/2014/05/photos-of-an-nsa-upgrade-factory-show-cisco-router-getting-implant/
  [1044]: https://wikiless.org/wiki/OONI
  [1045]: https://web.archive.org/web/https://en.wikipedia.org/wiki/OONI
  [1046]: https://web.archive.org/web/https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws
  [1047]: https://web.archive.org/web/https://www.nytimes.com/2021/01/12/technology/bitcoin-passwords-wallets-fortunes.html
  [1048]: https://web.archive.org/web/https://www.usenix.org/system/files/conference/woot17/woot17-paper-obermaier.pdf
  [1049]: https://web.archive.org/web/https://support.torproject.org/tormobile/tormobile-3/
  [1050]: https://wikiless.org/wiki/Tails_(operating_system)
  [1051]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Tails_(operating_system)
  [1052]: https://web.archive.org/web/https://www.veracrypt.fr/en/Trim%20Operation.html
  [1053]: https://web.archive.org/web/https://www.coreboot.org/
  [1054]: https://wikiless.org/wiki/Booting
  [1055]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Booting
  [1056]: https://web.archive.org/web/https://www.wired.com/2013/12/better-data-security-nail-polish/
  [1057]: https://wikiless.org/wiki/Virtual_machine
  [1058]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Virtual_machine
  [1059]: https://wikiless.org/wiki/Plausible_deniability
  [1060]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Plausible_deniability
  [1061]: https://wikiless.org/wiki/Deniable_encryption
  [1062]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Deniable_encryption
  [1063]: https://web.archive.org/web/https://privacytools.io/operating-systems/
  [1064]: https://wikiless.org/wiki/BitLocker
  [1065]: https://web.archive.org/web/https://en.wikipedia.org/wiki/BitLocker
  [1066]: https://wikiless.org/wiki/Evil_maid_attack
  [1067]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Evil_maid_attack
  [1068]: https://wikiless.org/wiki/Cold_boot_attack
  [1069]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Cold_boot_attack
  [1070]: https://yewtu.be/watch?v=JDaicPIgn9U
  [1071]: https://web.archive.org/web/https://www.researchgate.net/publication/318155607_Defeating_Plausible_Deniability_of_VeraCrypt_Hidden_Operating_Systems
  [1072]: https://web.archive.org/web/https://www.sans.org/reading-room/whitepapers/forensics/mission-implausible-defeating-plausible-deniability-digital-forensics-39500
  [1073]: https://web.archive.org/web/https://sourceforge.net/p/veracrypt/discussion/technical/thread/53f33faf/
  [1074]: https://web.archive.org/web/https://docs.microsoft.com/en-us/windows/security/information-protection/bitlocker/bitlocker-countermeasures
  [1075]: https://web.archive.org/web/https://www.sans.org/reading-room/whitepapers/forensics/windows-shellbag-forensics-in-depth-34545
  [1076]: https://web.archive.org/web/https://eprints.whiterose.ac.uk/75046/1/Forensic_Data_Recovery_From_The_Windows_Search_Database_preprint_DIIN328.pdf
  [1077]: https://web.archive.org/web/https://cyberforensicator.com/wp-content/uploads/2017/01/1-s2.0-S1742287616300202-main.2-14.pdf
  [1078]: https://wikiless.org/wiki/Gatekeeper_(macOS)
  [1079]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Gatekeeper_(macOS)
  [1080]: https://web.archive.org/web/https://wiki.alpinelinux.org/wiki/Setting_up_a_laptop
  [1081]: https://wikiless.org/wiki/VeraCrypt
  [1082]: https://web.archive.org/web/https://en.wikipedia.org/wiki/VeraCrypt
  [1083]: https://web.archive.org/web/https://www.veracrypt.fr/en/Unencrypted%20Data%20in%20RAM.html
  [1084]: https://web.archive.org/web/https://www.veracrypt.fr/code/VeraCrypt/plain/doc/html/Data%20Leaks.html
  [1085]: https://web.archive.org/web/https://dolosgroup.io/blog/2021/7/9/from-stolen-laptop-to-inside-the-company-network
  [1086]: https://web.archive.org/web/https://trmm.net/tpm-sniffing/
  [1087]: https://web.archive.org/web/https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html
  [1088]: https://web.archive.org/web/https://repository.stcloudstate.edu/cgi/viewcontent.cgi?article=1141&context=msia_etds
  [1089]: https://web.archive.org/web/https://www.windowscentral.com/how-ensure-trim-enabled-windows-10-speed-ssd-performance
  [1090]: https://web.archive.org/web/https://veracrypt.eu/en/docs/trim-operation/
  [1091]: https://web.archive.org/web/https://i.blackhat.com/eu-18/Thu-Dec-6/eu-18-Schaub-Perfectly-Deniable-Steganographic-Disk-Encryption.pdf
  [1092]: https://web.archive.org/web/http://asalor.blogspot.com/2011/08/trim-dm-crypt-problems.html
  [1093]: https://wikiless.org/wiki/VirtualBox
  [1094]: https://web.archive.org/web/https://en.wikipedia.org/wiki/VirtualBox
  [1095]: https://web.archive.org/web/https://www.virtualbox.org/ticket/17987
  [1096]: https://wikiless.org/wiki/Whonix
  [1097]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Whonix
  [1098]: https://web.archive.org/web/https://docs.oracle.com/en/virtualization/virtualbox/6.0/user/snapshots.html
  [1099]: https://wikiless.org/wiki/Spectre_(security_vulnerability)
  [1100]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Spectre_(security_vulnerability)
  [1101]: https://wikiless.org/wiki/Meltdown_(security_vulnerability)
  [1102]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Meltdown_(security_vulnerability)
  [1103]: https://web.archive.org/web/https://www.whonix.org/wiki/Stream_Isolation#By_Settings
  [1104]: https://wikiless.org/wiki/Time-based_One-time_Password_algorithm
  [1105]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm
  [1106]: https://wikiless.org/wiki/Multi-factor_authentication
  [1107]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Multi-factor_authentication
  [1108]: https://web.archive.org/web/https://www.whonix.org/wiki/Whonix-Gateway_Security#Warning:_Bridged_Networking
  [1109]: https://web.archive.org/web/https://www.qubes-os.org/faq/#is-qubes-just-another-linux-distribution
  [1110]: https://web.archive.org/web/https://www.qubes-os.org/doc/system-requirements/
  [1111]: https://web.archive.org/web/https://github.com/QubesOS/qubes-issues/issues/2414
  [1112]: https://wikiless.org/wiki/AppArmor
  [1113]: https://web.archive.org/web/https://en.wikipedia.org/wiki/AppArmor
  [1114]: https://wikiless.org/wiki/Security-Enhanced_Linux
  [1115]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Security-Enhanced_Linux
  [1116]: https://wikiless.org/wiki/CAPTCHA
  [1117]: https://web.archive.org/web/https://en.wikipedia.org/wiki/CAPTCHA
  [1118]: https://wikiless.org/wiki/Turing_test
  [1119]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Turing_test
  [1120]: https://web.archive.org/web/https://www.google.com/recaptcha/about/
  [1121]: https://web.archive.org/web/https://www.hcaptcha.com/
  [1122]: https://web.archive.org/web/https://www.hcaptcha.com/post/hcaptcha-now-the-largest-independent-captcha-service
  [1123]: https://web.archive.org/web/https://nearcyan.com/you-probably-dont-need-recaptcha/
  [1124]: https://web.archive.org/web/https://arstechnica.com/gadgets/2017/03/googles-recaptcha-announces-invisible-background-captchas/
  [1125]: https://web.archive.org/web/https://www.blackhat.com/docs/asia-16/materials/asia-16-Sivakorn-Im-Not-a-Human-Breaking-the-Google-reCAPTCHA-wp.pdf
  [1126]: https://web.archive.org/web/https://security.googleblog.com/2014/12/are-you-robot-introducing-no-captcha.html
  [1127]: https://web.archive.org/web/https://blog.cloudflare.com/cloudflare-supports-privacy-pass/
  [1128]: https://wikiless.org/wiki/Device_fingerprint
  [1129]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Device_fingerprint
  [1130]: https://web.archive.org/web/https://developers.googleblog.com/2020/08/guidance-for-our-effort-to-block-less-secure-browser-and-apps.html
  [1131]: https://web.archive.org/web/https://support.google.com/accounts/answer/10071085
  [1132]: https://wikiless.org/wiki/Dark_pattern
  [1133]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Dark_pattern
  [1134]: https://web.archive.org/web/https://www.theverge.com/2020/1/23/21077423/tinder-photo-verification-blue-checkmark-safety-center-launch-noonlight
  [1135]: https://web.archive.org/web/https://www.digitalinformationworld.com/2020/03/facebook-is-now-demanding-some-users-to-create-a-video-selfie-for-identity-verification.html
  [1136]: https://web.archive.org/web/https://www.vice.com/en/article/m7a4eq/pornhub-new-verification-policy-biometric-id
  [1137]: https://web.archive.org/web/https://variety.com/2021/digital/news/china-censorship-hotline-historical-nihilism-1234950554/
  [1138]: https://wikiless.org/wiki/Zero_trust_security_model
  [1139]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Zero_trust_security_model
  [1140]: https://wikiless.org/wiki/Espionage
  [1141]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Espionage
  [1142]: https://web.archive.org/web/https://kcimc.medium.com/how-to-recognize-fake-ai-generated-images-4d1f6f9a2842
  [1143]: https://web.archive.org/web/https://blog.jayway.com/2020/03/06/using-ml-to-detect-fake-face-images-created-by-ai/
  [1144]: https://wikiless.org/wiki/SIM_swap_scam
  [1145]: https://web.archive.org/web/https://en.wikipedia.org/wiki/SIM_swap_scam
  [1146]: https://web.archive.org/web/https://www.whonix.org/wiki/Tor
  [1147]: https://web.archive.org/web/https://support.torproject.org/tbb/tbb-editing-torrc/
  [1148]: https://web.archive.org/web/https://support.google.com/accounts/answer/114129?hl=en
  [1149]: https://web.archive.org/web/https://support.google.com/google-ads/answer/7474263?hl=en
  [1150]: https://web.archive.org/web/https://support.google.com/accounts/answer/40695
  [1151]: https://web.archive.org/web/https://support.google.com/accounts/contact/disabled2
  [1152]: https://web.archive.org/web/https://support.google.com/accounts/answer/1333913?hl=en
  [1153]: https://web.archive.org/web/https://www.jumio.com/features/
  [1154]: https://web.archive.org/web/https://privacytools.io/providers/email/
  ...
  [1155]: https://web.archive.org/web/https://protonmail.com/support/knowledge-base/human-verification/
  [1156]: https://web.archive.org/web/https://knowyourmeme.com/memes/good-luck-im-behind-7-proxies
  [1157]: https://wikiless.org/wiki/End-to-end_encryption
  [1158]: https://web.archive.org/web/https://en.wikipedia.org/wiki/End-to-end_encryption
  [1159]: https://wikiless.org/wiki/Forward_secrecy
  [1160]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Forward_secrecy
  [1161]: https://web.archive.org/web/https://protonmail.com/blog/zero-access-encryption/
  [1162]: https://wikiless.org/wiki/Facebook%E2%80%93Cambridge_Analytica_data_scandal
  [1163]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Facebook%E2%80%93Cambridge_Analytica_data_scandal
  [1164]: https://web.archive.org/web/https://signal.org/blog/sealed-sender/
  [1165]: https://web.archive.org/web/https://signal.org/blog/private-contact-discovery/
  [1166]: https://web.archive.org/web/https://signal.org/blog/signal-private-group-system/
  [1167]: https://web.archive.org/web/https://privacytools.io/software/file-sharing/
  [1168]: https://web.archive.org/web/https://privacytools.io/software/real-time-communication/
  [1169]: https://web.archive.org/web/https://getsession.org/session-protocol-explained/
  [1170]: https://web.archive.org/web/https://blog.quarkslab.com/audit-of-session-secure-messaging-application.html
  [1171]: https://yewtu.be/watch?v=aVwl892hqb4
  [1172]: https://web.archive.org/web/https://www.praxisfilms.org/open-letter-from-laura-poitras/
  [1173]: https://wikiless.org/wiki/SecureDrop
  [1174]: https://web.archive.org/web/https://en.wikipedia.org/wiki/SecureDrop
  [1175]: https://wikiless.org/wiki/Trusted_Platform_Module
  [1176]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Trusted_Platform_Module
  [1177]: https://wikiless.org/wiki/Pastebin
  [1178]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Pastebin
  [1179]: https://wikiless.org/wiki/Wear_leveling
  [1180]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Wear_leveling
  [1181]: https://wikiless.org/wiki/Write_amplification
  [1182]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Write_amplification
  [1183]: https://web.archive.org/web/https://techgage.com/article/too_trim_when_ssd_data_recovery_is_impossible/
  [1184]: https://web.archive.org/web/https://www.researchgate.net/publication/341761017_Live_forensics_method_for_acquisition_on_the_Solid_State_Drive_SSD_NVMe_TRIM_function
  [1185]: https://web.archive.org/web/https://blog.elcomsoft.com/2019/01/life-after-trim-using-factory-access-mode-for-imaging-ssd-drives/
  [1186]: https://web.archive.org/web/https://www.forensicfocus.com/articles/forensic-acquisition-of-solid-state-drives-with-open-source-tools/
  [1187]: https://web.archive.org/web/https://www.researchgate.net/publication/325976653_Solid_State_Drive_Forensics_Where_Do_We_Stand
  [1188]: https://wikiless.org/wiki/Parted_Magic
  [1189]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Parted_Magic
  [1190]: https://wikiless.org/wiki/Hdparm
  [1191]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Hdparm
  [1192]: https://web.archive.org/web/https://github.com/linux-nvme/nvme-cli
  [1193]: https://web.archive.org/web/https://partedmagic.com/secure-erase/
  [1194]: https://web.archive.org/web/https://partedmagic.com/nvme-secure-erase/
  [1195]: https://web.archive.org/web/https://www.ufsexplorer.com/solutions/data-recovery-on-encrypted-storage.php
  [1196]: https://web.archive.org/web/https://developer.apple.com/library/archive/documentation/FileManagement/Conceptual/APFS_Guide/FAQ/FAQ.html
  [1197]: https://web.archive.org/web/https://www.privacytools.io/software/productivity/
  [1198]: https://web.archive.org/web/https://www.whonix.org/wiki/Metadata
  [1199]: https://web.archive.org/web/https://gitlab.tails.boum.org/tails/blueprints/-/wikis/doc/mat/
  [1200]: https://web.archive.org/web/https://disable-gatekeeper.github.io/
  [1201]: https://web.archive.org/web/https://help.duckduckgo.com/duckduckgo-help-pages/features/cache/
  [1202]: https://web.archive.org/web/https://help.duckduckgo.com/duckduckgo-help-pages/results/sources/
  [1203]: https://wikiless.org/wiki/Dead_drop
  [1204]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Dead_drop
  [1205]: https://wikiless.org/wiki/Obfuscation
  [1206]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Obfuscation
  [1207]: https://wikiless.org/wiki/Kleptography
  [1208]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Kleptography
  [1209]: https://wikiless.org/wiki/Koalang
  [1210]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Koalang
  [1211]: https://wikiless.org/wiki/Operations_security
  [1212]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Operations_security
  [1213]: https://web.archive.org/web/https://quoteinvestigator.com/2014/07/13/truth/
  [1214]: https://web.archive.org/web/https://support.brave.com/hc/en-us/articles/360018121491-What-is-a-Private-Window-with-Tor-
  [1215]: https://web.archive.org/web/https://medium.com/velociraptor-ir/the-windows-usn-journal-f0c55c9010e
  [1216]: https://web.archive.org/web/https://medium.com/velociraptor-ir/digging-into-the-system-resource-usage-monitor-srum-afbadb1a375
  [1217]: https://web.archive.org/web/https://www.sans.org/blog/timestamped-registry-ntfs-artifacts-from-unallocated-space/
  [1218]: https://web.archive.org/web/https://dban.org/
  [1219]: https://web.archive.org/web/https://crystalmark.info/en/software/crystaldiskinfo/
  [1220]: https://wikiless.org/wiki/Faraday_cage
  [1221]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Faraday_cage
  [1222]: https://web.archive.org/web/https://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1165&context=adf
  [1223]: https://web.archive.org/web/https://arxiv.org/abs/1512.05616
  [1224]: https://web.archive.org/web/https://dl.acm.org/doi/pdf/10.1145/3309074.3309076
  [1225]: https://yewtu.be/watch?v=sO98kDLkh-M
  [1226]: https://wikiless.org/wiki/Touch_DNA
  [1227]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Touch_DNA
  [1228]: https://web.archive.org/web/https://www.yourdnaguide.com/ydgblog/dna-hair-samples-postage-stamps
  [1229]: https://web.archive.org/web/https://github.com/mhinkie/ooni-detection
  [1230]: https://wikiless.org/wiki/File_verification
  [1231]: https://web.archive.org/web/https://en.wikipedia.org/wiki/File_verification
  [1232]: https://wikiless.org/wiki/Cyclic_redundancy_check
  [1233]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Cyclic_redundancy_check
  [1234]: https://wikiless.org/wiki/MD5
  [1235]: https://web.archive.org/web/https://en.wikipedia.org/wiki/MD5
  [1236]: https://wikiless.org/wiki/Collision_(computer_science)
  [1237]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Collision_(computer_science)
  [1238]: https://wikiless.org/wiki/Secure_Hash_Algorithms
  [1239]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Secure_Hash_Algorithms
  [1240]: https://wikiless.org/wiki/SHA-2
  [1241]: https://web.archive.org/web/https://en.wikipedia.org/wiki/SHA-2
  [1242]: https://wikiless.org/wiki/Collision_resistance
  [1243]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Collision_resistance
  [1244]: https://web.archive.org/web/https://wiki.gnupg.org/Gpg4win/CheckIntegrity
  [1245]: https://web.archive.org/web/https://medium.com/@EvgeniIvanov/how-to-verify-checksum-on-mac-988f166b0c4f
  [1246]: https://wikiless.org/wiki/GNU_Privacy_Guard
  [1247]: https://web.archive.org/web/https://en.wikipedia.org/wiki/GNU_Privacy_Guard
  [1248]: https://wikiless.org/wiki/Public-key_cryptography
  [1249]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Public-key_cryptography
  [1250]: https://wikiless.org/wiki/Polymorphic_code
  [1251]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Polymorphic_code
  [1252]: https://web.archive.org/web/https://www.whonix.org/wiki/Malware_and_Firmware_Trojans
  [1253]: https://web.archive.org/web/https://forums.whonix.org/t/installation-of-antivirus-scanners-by-default/9755/8
  [1254]: https://web.archive.org/web/https://www.av-test.org/fileadmin/pdf/security_report/AV-TEST_Security_Report_2018-2019.pdf
  [1255]: https://web.archive.org/web/https://www.zdnet.com/article/eset-discovers-21-new-linux-malware-families/
  [1256]: https://web.archive.org/web/https://nakedsecurity.sophos.com/2019/07/25/evilgnome-linux-malware-aimed-at-your-laptop-not-your-servers/
  [1257]: https://web.archive.org/web/https://blog.imunify360.com/hiddenwasp-how-to-detect-malware-hidden-on-linux-iot
  [1258]: https://wikiless.org/wiki/Linux_malware
  [1259]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Linux_malware
  [https://en.wikipedia.org/wiki/macOS_malware]: https://en.wikipedia.org/wiki/MacOS_malware
  [1260]: https://wikiless.org/wiki/MacOS_malware
  [1261]: https://web.archive.org/web/https://en.wikipedia.org/wiki/MacOS_malware
  [1262]: https://web.archive.org/web/https://www.macworld.co.uk/feature/mac-viruses-list-3668354/
  [1263]: https://web.archive.org/web/https://resources.jamf.com/documents/macmalware-2020.pdf
  [1264]: https://web.archive.org/web/https://imagetragick.com/
  [1265]: https://web.archive.org/web/https://docs.oracle.com/en/virtualization/virtualbox/6.0/admin/hyperv-support.html
  [1266]: https://web.archive.org/web/https://zeltser.com/analyzing-malicious-documents/
  [1267]: https://wikiless.org/wiki/Portable_application
  [1268]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Portable_application
  [1269]: https://web.archive.org/web/https://ebin.city/~werwolf/posts/brave-is-shit/
  [1270]: https://web.archive.org/web/https://support.brave.com/hc/en-us/articles/360022973471-What-is-Shields
  [1271]: https://web.archive.org/web/https://venturebeat.com/2020/01/15/browser-benchmark-battle-january-2020-chrome-firefox-edge-brave/view-all/
  [1272]: https://web.archive.org/web/https://madaidans-insecurities.github.io/firefox-chromium.html
  [1273]: https://web.archive.org/web/https://grapheneos.org/usage#web-browsing
  [1274]: https://web.archive.org/web/https://www.researchgate.net/publication/349979628_Web_Browser_Privacy_What_Do_Browsers_Say_When_They_Phone_Home
  [1275]: https://web.archive.org/web/https://docs.microsoft.com/en-us/deployedge/microsoft-edge-security-windows-defender-application-guard
  [1276]: https://web.archive.org/web/https://www.pcmag.com/news/mozilla-signs-lucrative-3-year-google-search-deal-for-firefox
  [1277]: https://wikiless.org/wiki/Virtualization
  [1278]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Virtualization
  [1279]: https://web.archive.org/web/https://github.com/Yawning/obfs4/
  [1280]: https://web.archive.org/web/https://tb-manual.torproject.org/circumvention/
  [1281]: https://wikiless.org/wiki/Domain_fronting
  [1282]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Domain_fronting
  [1283]: https://web.archive.org/web/https://gitlab.torproject.org/tpo/applications/tor-browser/-/issues/17569
  [1284]: https://web.archive.org/web/https://www.tbstat.com/wp/uploads/2020/06/Europol-Wasabi-Wallet-Report.pdf
  [1285]: https://web.archive.org/web/https://www.sans.org/blog/nist-has-spoken-death-to-complexity-long-live-the-passphrase/
  [1286]: https://web.archive.org/web/https://www.zdnet.com/article/fbi-recommends-passphrases-over-password-complexity/
  [1287]: https://27m3p2uv7igmj6kvd4ql3cct5h3sdwrsajovkkndeufumzyfhlfev4qd.onion/2015/03/26/passphrases-can-memorize-attackers-cant-guess/
  [1288]: https://web.archive.org/web/https://theintercept.com/2015/03/26/passphrases-can-memorize-attackers-cant-guess/
  [1289]: https://web.archive.org/web/https://protonmail.com/blog/protonmail-com-blog-password-vs-passphrase/
  [1290]: https://yewtu.be/watch?v=yzGzB-yYKcc
  [1291]: https://wikiless.org/wiki/Passphrase#Passphrase_selection
  [1292]: https://web.archive.org/web/https://en.wikipedia.org/wiki/Passphrase#Passphrase_selection
  [1293]: https://web.archive.org/web/https://github.com/insight-decentralized-consensus-lab/post-quantum-monero/blob/master/writeups/technical_note.pdf
